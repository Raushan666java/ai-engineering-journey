...
4. Repeat for every outer row → O(n Ã— m) complexity.
...
**Complexity:**
- **Time:** O(n Ã— m) worst case → inner query runs n times, each scanning m rows. Best case O(n Ã— log m) with index on correlation column.

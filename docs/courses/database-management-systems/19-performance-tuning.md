...
**Impact:** 10 orders Ã— 5 items = 11 queries vs 1 query. 10-50ms per query = 110-550ms vs 5-15ms.
...
| **Bad plan** | Estimated rows â‰  actual rows | Stale statistics | ANALYZE, increase STATISTICS target |
...
|-----------------|-----------|----------------|
| **BRIN Indexes** | IoT sensor data, log tables, audit trails | 1000Ã— smaller indexes for append-only timestamp data |

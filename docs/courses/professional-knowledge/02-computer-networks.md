# Chapter 2: Computer Networks â€” Exam Quick Revision
...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/professional-knowledge/02-computer-networks/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/professional-knowledge/02-computer-networks/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/professional-knowledge/02-computer-networks/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/professional-knowledge/02-computer-networks/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/professional-knowledge/02-computer-networks/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/professional-knowledge/02-computer-networks/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
| 6 | Presentation | (merged) | SSL/TLS, MIME â€” encryption, compression |
| 5 | Session | (merged) | NetBIOS, RPC â€” session management |
| 4 | Transport | Transport | TCP, UDP |
| 3 | Network | Internet | IP, ICMP, ARP, OSPF, RIP, BGP â€” Router |
| 2 | Data Link | Network Access | Ethernet, PPP, MAC â€” Bridge, Switch |
| 1 | Physical | (hardware) | Cables, hubs, repeaters, modems |

**Exam Mnemonic for OSI:**
- Layer 7-1 (topâ†’bottom): **A**ll **P**eople **S**eem **T**o **N**eed **D**ata **P**rocessing
- Layer 1-7 (bottomâ†’top): **P**lease **D**o **N**ot **T**hrow **S**ausage **P**izza **A**way
...
|-------|-------------|-------|----------|----------|-------------|-------------------|
| A | 0 | 1.0.0.0 â€“ 126.255.255.255 | 8 | 24 | 126 | 2^24 âˆ’ 2 = 16,777,214 |
| B | 10 | 128.0.0.0 â€“ 191.255.255.255 | 16 | 16 | 16,384 | 2^16 âˆ’ 2 = 65,534 |
| C | 110 | 192.0.0.0 â€“ 223.255.255.255 | 24 | 8 | 2,097,152 | 2^8 âˆ’ 2 = 254 |
| D | 1110 | 224.0.0.0 â€“ 239.255.255.255 | â€” | â€” | Multicast | â€” |
| E | 1111 | 240.0.0.0 â€“ 255.255.255.255 | â€” | â€” | Reserved | â€” |

### Classless Addressing (CIDR)

- Format: `IP_address/PrefixLength` â€” e.g., `192.168.10.0/24`
...
- **0.0.0.0/8:** "This network" â€” default route
...
**Formula:** Number of hosts per subnet = 2^(32 âˆ’ prefix) âˆ’ 2 = 2^6 âˆ’ 2 = 62
...
| SMTP | 25 | TCP | Email sending (MTA â†’ MTA) |
...
|----------|------------------|
| **ARP** | Broadcast "Who has IP X?" â†’ Host with IP X replies with MAC address. Cached in ARP table. |
| **RARP** | Reverse of ARP â€” given MAC, find IP (used by diskless workstations) |
| **DHCP (DORA)** | D: Discover (client broadcast), O: Offer (server), R: Request (client), A: Acknowledge (server) |
| **DNS** | Hierarchical: Root â†’ TLD (.com, .org) â†’ Authoritative. Recursive vs Iterative queries. |
...
| **SMTP** | Push protocol â€” uses port 25. MTA transfers between mail servers. |
...
```
Client (SYN=1, seq=100) ---------------â†’ Server
Client â†------ (SYN=1, ACK=1, seq=300, ack=101) Server
Client (ACK=1, seq=101, ack=301) -----â†’ Server
```
...
```
Client (FIN=1, seq=x) ----â†’ Server
Client â†--- (ACK=1, ack=x+1) ---- Server
Client â†--- (FIN=1, seq=y) ------ Server
Client (ACK=1, ack=y+1) ---â†’ Server
```
**States:** FIN_WAIT_1 â†’ FIN_WAIT_2 â†’ TIME_WAIT â†’ CLOSED
...
- **Receiver window (RWND):** Advertised by receiver â€” available buffer space
- **Congestion window (CWND):** Sender's estimate of network capacity
- **Effective window = min(RWND, CWND)**

---

## 7. Congestion Control â€” TCP States

```
          +----------+
          |  Slow    |  (exponential growth: CWND Ã— 2 each RTT
...
| Congestion Avoidance | CWND â‰¥ ssthresh | CWND += 1 per RTT (linear) |
...
- **Metric:** Hop count (max 15 â€” infinity)
...
- **Fast convergence** â€” no count-to-infinity
...
- **Path vector protocol** â€” exchanges reachability information between ASes
- Uses TCP (port 179)
...
**Answer:** (a) 14. 2^(32âˆ’28) âˆ’ 2 = 2^4 âˆ’ 2 = 16 âˆ’ 2 = 14.
...
**Answer:** (b) 4. Each ACK increases CWND by 1 MSS. Two ACKs = CWND + 2 = 4 MSS. (Slow start doubles CWND per RTT, not per ACK; but per-ACK increment is 1 MSS Ã— number of ACKs.)
...
**Answer:** (b) DHCP. Discover â†’ Offer â†’ Request â†’ Acknowledge.
...
```
1. Sense channel
2. If idle â†’ transmit; else wait
3. If collision detected â†’ send jam signal
4. Exponential backoff: wait random time (0 to 2^kâˆ’1 slots)
5. Retry
```

**Minimum frame size** requirement: Must be &gt; 2 Ã— propagation delay Ã— bandwidth (to detect all collisions)
...
- **ACK:** Receiver acknowledges successful frame (missing ACK â‡’ retransmit)
...
| Address count | ~4.3 billion | ~3.4Ã—10^38 |
...
- **No broadcast** in IPv6 â€” replaced by multicast
...
| **Gateway** | 3-7 | Protocol translation (network â†” network) | N/A | N/A |
| **Modem** | 1-2 | Modulates/demodulates analog â†” digital | N/A | N/A |
| **Access Point** | 2 | Wireless â†” wired bridge | Per radio | Single |
...
### TCP State Machine â€” Full Diagram
...
**Answer:** Server moves to FIN_WAIT_2, then receives FIN from client â†’ TIME_WAIT â†’ CLOSED after 2MSL timeout.
...
// calculateSubnet('192.168.1.0', 24, 4) â†’ 4 subnets, /26 each, 62 usable hosts each
```

### Routing Algorithm Simulator â€” Dijkstra's for OSPF

```typescript
interface Router {
  id: string;
  neighbors: Map<string, number>; // neighbor â†’ cost
}
...
### TCP Congestion Control â€” CWND Evolution with TypeScript
...
// simulateTCP(1, 16, 10) â†’ slow start: 1â†’2â†’4â†’8â†’16â†’32, then ssthresh=16, cwnd=16 â†’ +1 each RTT
```

### VLAN and Subnetting â€” Advanced Numerical
...
**Solution:**
- For 5000 hosts: need 2^h âˆ’ 2 â‰¥ 5000 â†’ 2^h â‰¥ 5002 â†’ h = 13 (2^13 = 8192)
- So prefix = 32 âˆ’ 13 = /19 per subnet
- For 6 subnets: need âŒˆlog2(6)âŒ‰ = 3 subnet bits
- Starting from /16, take 3 bits â†’ /19
- Subnet 0: 172.16.0.0/19 (hosts: 172.16.0.1 â€“ 172.16.31.254)
- Subnet 1: 172.16.32.0/19
- ... up to Subnet 7: 172.16.224.0/19
- Valid subnets: 0â€“5 (6 subnets)
- Each subnet has 2^13 âˆ’ 2 = 8190 usable hosts âœ“
...
**Solution:**
- Max payload per fragment = 1500 âˆ’ 20 = 1480 bytes
- Number of fragments = âŒˆ3980/1480âŒ‰ = 3
- Fragment 1: Offset = 0, MF = 1, size = 1500 (1480 data)
- Fragment 2: Offset = 1480/8 = 185, MF = 1, size = 1500 (1480 data)
- Fragment 3: Offset = 2960/8 = 370, MF = 0, size = 3980 âˆ’ 2960 + 20 = 1040 (1020 data)
...
**Answer:** (b) 1022. 2^(32âˆ’22) âˆ’ 2 = 2^10 âˆ’ 2 = 1024 âˆ’ 2 = 1022.
</details>
...
<details>
<summary>Q7: In CSMA/CD, if the propagation delay is 25 Î¼s and bandwidth is 100 Mbps, what is the minimum frame size?</summary>
(a) 2500 bits (b) 5000 bits (c) 10000 bits (d) 12500 bits
**Answer:** (b) 5000 bits. Minimum frame size = 2 Ã— propagation delay Ã— bandwidth = 2 Ã— 25Ã—10^âˆ’6 Ã— 100Ã—10^6 = 5000 bits.
</details>
...
**Answer:** (c) 128 bits. IPv6 = 128 bits (3.4Ã—10^38 addresses), IPv4 = 32 bits.
</details>
...
(a) 3 (b) 4 (c) 5 (d) 6
**Answer:** (b) 4. For detecting d-bit errors, Hamming distance must be at least d+1. So for d=3, Hamming distance â‰¥ 4.
</details>
...
2. 255.255.248.0 = /21. Contains 2^(24âˆ’21) = 8 /24 subnets
3. Clientâ†’FINâ†’Server: enters FIN_WAIT1. Serverâ†’ACKâ†’Client: enters FIN_WAIT2. Serverâ†’FINâ†’Client: enters CLOSE_WAIT. Clientâ†’ACKâ†’Server: enters TIME_WAIT (2MSL). Serverâ†’CLOSED
4. 6 fragments. Payload = 2980. Per fragment payload = 556 (576-20). Offsets: 0, 69.5... actually 556Ã—5 = 2780, last = 200. Fragments: 5 offset 0-4, MF=1, last offset=2780/8=347.5? No: 556Ã—5=2780, last fragment offset=2780/8=347.5 not integer! Recalculate: 556Ã—5=2780, remainder=200. All offsets = {0, 69, 139, 208, 278, 347} Ã— 8. Fragment sizes: 576, 576, 576, 576, 576, 220
...
7. Aâ†’B:2, Aâ†’C:3(via B), Aâ†’D:5(via B)
8. /23. 2^(32âˆ’23) = 512 total, 510 usable
...
11. 2^h âˆ’ 2 â‰¥ 500 â†’ h = 9 â†’ 2^9 âˆ’ 2 = 510. Prefix = /23. CIDR = /23
...
14. /28 â†’ mask 255.255.255.240. Network = 200.100.50.32. Broadcast = 200.100.50.47. Hosts: .33â€“.46 (14 hosts)
...
17. Handshake = 1 RTT = 100ms. Transfer = 1MB/10Mbps = 0.8s. Total â‰ˆ 0.9s (plus slow start overhead)
18. Hidden node: A and C both send to B but can't hear each other â†’ collision. RTS/CTS: A sends RTS, B replies CTS (heard by both A and C), C defers
...
21. 110101 Ã· 1011 â†’ remainder = 001. CRC-3 = 001
...
26. Circuit: dedicated path (telephone) â€” fixed bandwidth. Packet: store-and-forward (internet) â€” efficient utilization
27. Throughput â‰¤ Window/RTT = 65535Ã—8 / 0.2 = 2.6214 Mbps
...
29. Need 9 host bits (2^9âˆ’2=510 â‰¥ 200). Prefix = /23. 2^(16âˆ’9) = 128 subnets, but 500 needed. Use /23: 2^(8âˆ’3) = 32 subnets per /16... Actually from /8, take 9 bits â†’ /17. 2^9 = 512 subnets âœ“
...
**Solution:**
- Need 500 hosts â†’ 2^h âˆ’ 2 â‰¥ 500 â†’ h = 9 (2^9 âˆ’ 2 = 510)
- Prefix = 32 âˆ’ 9 = /23 per subnet
- From /16, borrow 7 bits for subnet â†’ 2^7 = 128 subnets (enough for 20)
- Subnet mask: 255.255.254.0
- Subnet 1: 172.24.0.0/23 (hosts: 172.24.0.1 â€“ 172.24.1.254)
- Subnet 2: 172.24.2.0/23 (hosts: 172.24.2.1 â€“ 172.24.3.254)
- ...up to Subnet 20: 172.24.38.0/23
- Each subnet has 510 usable hosts âœ“
...
**Answer:** 
- Initially slow start: 1â†’2â†’4â†’8 (doubling each RTT)
- At CWND=8, ssthresh was reached. Threshold = 8. Switched to congestion avoidance: 8â†’9â†’10 (linear increase)
- At CWND=10, 3 duplicate ACKs detected â†’ Fast Recovery triggered
- New ssthresh = CWND/2 = 5. New CWND = 5 (TCP Reno sets CWND = ssthresh)
- Then linear increase resumes: 5â†’6â†’7â†’8â†’9

> **PYQ 2026:** Compare HTTP/2 multiplexing with HTTP/1.1 pipelining. Which addresses head-of-line blocking better?

**Answer:** HTTP/1.1 pipelining allows multiple requests on one connection but responses must be returned in order (FIFO) â†’ head-of-line blocking if one response is slow. HTTP/2 multiplexing allows multiple concurrent streams over one TCP connection with interleaved responses â€” eliminates HOLB at HTTP level. However, TCP-level HOLB still exists in HTTP/2 (lost packet blocks all streams). HTTP/3 (QUIC) solves this with per-stream independent delivery over UDP.
...
- **IP addressing:** Classful (A/B/C default masks) â†’ Classless CIDR (/prefix)
- **Key protocols:** ARP (MAC resolution), DHCP (auto IP), DNS (name â†’ IP), HTTP/2 (multiplexed)
...
- **Congestion control:** Slow start â†’ congestion avoidance â†’ fast retransmit/recovery
...
1. Subnetting numericals â€” given IP block, find subnet mask, number of hosts, subnet addresses
2. OSI layer functions â€” which layer does encryption/segmentation/routing/error detection
3. TCP header fields â€” sequence number, acknowledgment number, window size, flags
4. Congestion control â€” slow start threshold calculation, CWND graph interpretation
5. Switching techniques (circuit vs packet vs message) with examples
6. IP fragmentation â€” MF flag, offset field, fragment sizes
7. CSMA/CD vs CSMA/CA â€” collision detection vs collision avoidance
8. DNS resolution process â€” recursive vs iterative query types
...
A2: Reset â€” used to abort a connection abruptly (e.g., connection refused, host unreachable, protocol error).
</details>

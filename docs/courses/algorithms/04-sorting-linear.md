...
> **Prerequisites:** [Chapter 3: Comparison-Based Sorting](./03-sorting-nlogn.md) â€” The Î©(n log n) lower bound for comparison sorts | **Next:** [Chapter 5: Divide and Conquer](./05-divide-conquer.md) â€” Beyond sorting: applying divide-and-conquer broadly
...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/04-sorting-linear/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/04-sorting-linear/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/04-sorting-linear/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/04-sorting-linear/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/04-sorting-linear/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/04-sorting-linear/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
Imagine you are a teacher with 1,000 exam papers, each graded 0â€“100. You need to sort them by score so you can enter grades. You *could* compare every pair of papers (comparison sort, O(n log n) â‰ˆ 10,000 comparisons). Or you could make 101 piles on a table, drop each paper into its score's pile, and simply stack the piles in order. That is exactly what counting sort does â€” and it runs in O(n + k) time, completely bypassing the Î©(n log n) comparison barrier.

This is the central insight of this chapter: **when keys have structure (small ranges, fixed widths, uniform distributions), you do not need comparisons.** You can sort in linear time by exploiting that structure directly. The practical impact is enormous â€” from GPU-accelerated radix sort in database engines to counting-sort-based suffix array construction in bioinformatics.
...
|-------|-------------|-------------------|
| Counting Sort | Count frequencies, compute prefix sums, place elements | O(n + k) time â€” blistering fast when key range k is small |
| Radix Sort | Sort digit by digit using stable sort | O(d(n + r)) â€” ideal for fixed-width integers and strings |
| Bucket Sort | Distribute into buckets, sort individually | Expected O(n) â€” excellent on uniformly distributed data |
| Comparison Bypass | Key structure replaces comparisons | Non-comparison sorts beat Î©(n log n) by exploiting key properties |
...
The \( \Omega(n \log n) \) lower bound for comparison sorting (Theorem 3.1) applies only to algorithms that use comparisons as their sole means of gaining information about element order. By exploiting the structure of keys â€” their range, digit composition, or distribution â€” we can achieve sorting in \( O(n) \) time.

### 4.1 Counting Sort


**Real-World Analogy:** You are sorting 500 student answer sheets by roll number (1â€“100). Instead of comparing roll numbers pairwise, you prepare 100 slots, place each sheet into the slot matching its roll number, then collect the slots in order. No comparisons needed â€” just distribution and collection.
...
**Why It Beats Î©(n log n):** Counting sort never compares two elements. It uses the element values as indices into a count array. The Î©(n log n) bound applies only to algorithms that rely on pairwise comparisons to determine order. By using direct array indexing, counting sort escapes this bound entirely.

#### Algorithm Steps

```
Step 1: Create count array C[0..k-1], initialized to 0.
Step 2: Count frequencies â€” for each element x in A, increment C[x].
Step 3: Compute prefix sums â€” for i = 1 to k-1, set C[i] = C[i] + C[i-1].
Step 4: Build output â€” for i = n-1 down to 0:
...
**Input:** `A = [4, 2, 2, 8, 3, 3, 1]`, Range `k = 9` (values 0â€“8), `n = 7`

**Step 2 â€” Frequency Count:**

| Element | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|----------|---|---|---|---|---|---|---|---|---|
| Count    | 0 | 1 | 2 | 2 | 1 | 0 | 0 | 0 | 1 |

**Step 3 â€” Prefix Sums (Cumulative Count):**

| Index    | 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|----------|---|---|---|---|---|---|---|---|---|
| Prefix   | 0 | 1 | 3 | 5 | 6 | 6 | 6 | 6 | 7 |

Each prefix value C[i] = number of elements â‰¤ i.

**Step 4 â€” Placement (right-to-left traversal):**
...
| Metric | Value | Why? |
|--------|-------|------|
| **Time (Best)** | O(n + k) | One pass to count, one for prefix sums, one to place â€” all linear. |
| **Time (Worst)** | O(n + k) | Same as best â€” runtime depends only on n and k, not on data order. |
...
**Why O(n + k) beats O(n log n):** When k = O(n), counting sort runs in O(n) â€” strictly faster than any comparison sort's Î©(n log n) lower bound. The catch is that k must be small enough that the count array fits in memory.

#### Advantages & Disadvantages

| Advantages | Disadvantages |
|------------|---------------|
| Linear O(n + k) time when k = O(n) | Impractical when k is huge (e.g., sorting 10â¶ integers in range [0, 10Â¹Â²]) |
...
| No data-dependent branching â€” constant speed | Cannot sort by fractional keys directly |
| Parallelizable â€” frequency counting is embarrassingly parallel | Not in-place |
...
| **Duplicate-heavy data** | Handled naturally â€” counts track multiplicity | Stable sort preserves original order of duplicates |
| **Empty array** | Loops run zero times | Output is empty â€” trivial base case |
| **Single element** | Correctly placed in 1 step | Always works |

> **Pro Tip:** Counting sort is your fastest option when keys are integers with a limited range (e.g., ages 0â€“150, grades 0â€“100, ASCII 0â€“255). For k â‰¤ n, it beats any comparison sort.
>
> **Remember:** The right-to-left pass is what makes counting sort stable â€” reversing the direction breaks stability.
>
> **Warning:** Do NOT use counting sort when k is large relative to n. Sorting 10 integers in range [0, 10â¶] wastes O(10â¶) space for 10 elements.
...
**Real-World Analogy:** A librarian sorts books by call number. Instead of comparing full call numbers, they look at only the first digit and sort into 10 piles (0â€“9). Within each pile, they look at the second digit and sort further. This is MSD radix sort. A mail sorter works the other way: sort packages by the last digit of the ZIP code first, then the second-to-last, and so on â€” LSD radix sort. Both avoid ever comparing two full numbers.
...
**Why It Beats Î©(n log n):** Radix sort reduces the sorting problem to d passes of a stable sort with a small range (the digit range r). Each pass is O(n + r), and since d is a constant for fixed-width keys (e.g., d = 10 for decimal 32-bit ints when r = 10, or d = 4 when r = 256), the total is O(n) â€” bypassing the Î©(n log n) comparison barrier.
...
**Pass 1 â€” Sort by units digit (exp = 1):**

| Original | Digit (Ã·1 % 10) |
|----------|------------------|
| 329      | 9                |
| 457      | 7                |
| 657      | 7                |
...
Counting sort by digit â†’ `[720, 355, 436, 457, 657, 329, 839]`

**Pass 2 â€” Sort by tens digit (exp = 10):**

| After Pass 1 | Digit (Ã·10 % 10) |
|--------------|------------------|
| 720          | 2                |
| 355          | 5                |
| 436          | 3                |
...
Counting sort by digit â†’ `[720, 329, 436, 839, 355, 457, 657]`

**Pass 3 â€” Sort by hundreds digit (exp = 100):**

| After Pass 2 | Digit (Ã·100 % 10) |
|--------------|-------------------|
| 720          | 7                 |
| 329          | 3                 |
| 436          | 4                 |
...
Counting sort by digit â†’ `[329, 355, 436, 457, 657, 720, 839]` âœ“
...
| **Time (Worst)** | O(d(n + r)) | Same as best â€” no data-dependent branches. |
...
**Choosing the base r:** Larger r means fewer passes (smaller d) but a larger count array (O(r) space). Optimal r balances d and r â€” typically base 256 (byte-by-byte) is the practical sweet spot for integer sorting.
...
| Highly parallelizable â€” each digit pass is independent | Not in-place |
...
> **Pro Tip:** Choose radix base wisely â€” base 256 (byte-by-byte) on 32-bit integers needs only 4 passes, each with r = 256 counters. This usually beats base 10's 10 passes.
>
> **Warning:** Radix sort only works with LSD-first for general inputs. MSD-first requires recursion and can degrade.
>
> **Remember:** Radix sort is the fastest integer sorting algorithm in practice for large arrays â€” GPU implementations sort 10â¸ integers in under a second.

**One-Sentence Takeaway:** Radix sort achieves O(n) for fixed-width keys by processing digits one at a time with a stable auxiliary sort, bypassing the Î©(n log n) comparison bound.
...
**Real-World Analogy:** You need to sort 1,000 exam scores (between 0 and 100) that are uniformly distributed. You create 10 buckets labeled 0â€“9, 10â€“19, ..., 90â€“100. You drop each score into its bucket. Then you sort each bucket individually (with a simple sort like insertion sort) and concatenate the buckets. Since scores are uniformly distributed, each bucket has roughly 100 items, and sorting 100 items 10 times is much faster than sorting 1,000 items at once.
...
**Why It Beats Î©(n log n):** The expected number of elements per bucket is O(1) when the input is uniformly distributed. Sorting O(1) elements per bucket costs O(1) per bucket using insertion sort, giving O(n) total. The Î©(n log n) bound assumes pairwise comparisons across all n elements â€” bucket sort avoids this by restricting comparisons to within small groups.
...
**Distribution into buckets (bucket index = floor(10 Ã— A[i])):**
...
|----------|----------|----------|----------|----------|----------|----------|----------|----------|----------|
| â€”        | 0.17, 0.12 | 0.26, 0.21, 0.23 | 0.39 | â€” | â€” | 0.68 | 0.78, 0.72 | â€” | 0.94 |

**After Insertion Sort (within each bucket):**

| Bucket 0 | Bucket 1 | Bucket 2 | Bucket 3 | Bucket 4 | Bucket 5 | Bucket 6 | Bucket 7 | Bucket 8 | Bucket 9 |
|----------|----------|----------|----------|----------|----------|----------|----------|----------|----------|
| â€”        | 0.12, 0.17 | 0.21, 0.23, 0.26 | 0.39 | â€” | â€” | 0.68 | 0.72, 0.78 | â€” | 0.94 |

**Final Concatenated Output:**
`[0.12, 0.17, 0.21, 0.23, 0.26, 0.39, 0.68, 0.72, 0.78, 0.94]` âœ“
...
| **Time (Worst)** | O(nÂ²) | All elements land in one bucket â€” insertion sort on n elements. |
| **Time (Average)** | Î˜(n) | Uniform distribution gives expected O(1) elements per bucket. |
...
| Advantages | Disadvantages |
|------------|---------------|
| Expected O(n) time â€” fastest for uniformly distributed data | Worst-case O(nÂ²) â€” catastrophic on non-uniform data |
...
| Easy to parallelize â€” each bucket sorts independently | Extra overhead for bucket creation and concatenation |
...
| Edge Case | Behavior | Mitigation |
|-----------|----------|------------|
| **All elements in one bucket** | Degrades to O(nÂ²) | Use a balanced hash function or fallback to comparison sort |
...
| **Negative values** | floor(n Ã— x) can be negative | Shift range to [0, 1) via normalization |
| **Very small n** | Overhead of bucket creation dominates | Use insertion sort directly for n &lt; threshold |
| **All equal values** | All go to same bucket â†’ O(nÂ²) | Fallback to counting sort if duplicates are heavy |

> **Pro Tip:** Bucket sort needs a good hash function that distributes elements uniformly. If your data is not uniform, use a different algorithm â€” bucket sort degrades to O(nÂ²).
>
> **Remember:** Bucket sort is efficient only when the number of buckets is proportional to n and the distribution is uniform.
>
> **Warning:** Never assume uniform distribution without checking â€” test your data first or add a fallback to a comparison sort.
...
| **Time (Average)** | O(n + k) | O(d(n + r)) | Î˜(n) expected |
| **Time (Worst)** | O(n + k) | O(d(n + r)) | O(nÂ²) |
...
- **Counting Sort** wins when the key range k is small relative to n (e.g., sorting 10â¶ people by age 0â€“150). If k â‰¤ 10n and k â‰ˆ n, counting sort is simplest and fastest.
- **Radix Sort** wins when the key range is huge but the keys have a digit structure (e.g., sorting 10â¶ 64-bit integers). Counting sort would need O(10Â¹â¹) space, but radix sort with base 2Â¹â¶ needs only 4 passes of O(n + 65536) each.
- **Rule of thumb:** If k â‰¤ 10n, use counting sort. If k is unbounded but keys are fixed-width integers, use radix sort.

### Sorting with Limited Range

When the interviewer says "sort in O(n) time," they are testing whether you recognize that non-comparison sorts exist. The follow-up is always: *What assumption about the data makes this possible?* The answer must include the key property (small range, digit structure, uniform distribution) that circumvents the Î©(n log n) bound.
...
1. *"Can you sort integers in O(n) time?"* â€” Yes, with counting sort (if range is small) or radix sort (if fixed-width).
2. *"What is the space trade-off of increasing the radix base?"* â€” Fewer passes (smaller d) but larger count array (larger r). Optimize for r Ã— d.
3. *"Why can't you use counting sort for floating-point numbers?"* â€” Floating-point indices are not valid array indices. You could reinterpret the IEEE 754 bit pattern as an integer and use radix sort.
4. *"Is bucket sort always O(n)?"* â€” No, only in expectation under uniform distribution. Worst-case is O(nÂ²).
...
Modern GPU sorting libraries (e.g., CUB, Thrust, C++17 parallel algorithms) implement radix sort as their primary sorting primitive. NVIDIA's CUB library sorts 10â¸ 32-bit integers in ~0.3 seconds on an A100 GPU. The key insight: radix sort is embarrassingly parallel â€” each digit pass can compute histograms in parallel, and the scatter operation (placing elements by digit) maps perfectly to GPU thread blocks. Comparison-based sorts (quick sort, merge sort) do not parallelize nearly as well on GPU architectures.

### Counting Sort in Suffix Arrays

The suffix array â€” a sorted array of all suffixes of a string â€” is a fundamental data structure in stringology and bioinformatics. The SA-IS algorithm (Nong-Zhang-Chan 2009) and induced sorting use counting sort as a subroutine to sort suffixes by their first character (the LMS characters). Since the alphabet size is typically small (4 for DNA, 26 for proteins, 256 for bytes), counting sort at each induction step runs in O(n + Î£) time, contributing to the overall O(n) suffix array construction.

### Bucket Sort in Histograms and Image Processing

Digital image processing uses bucket sort (often called *histogram equalization*) to redistribute pixel intensities. Each pixel's intensity value (0â€“255) maps to a bucket, and cumulative distribution functions derived from bucket counts are used to stretch the intensity range for contrast enhancement. Database query engines also use bucket sort-like partitioning for approximate query processing â€” dividing data into equi-width histograms to estimate selectivity without full sorting.

### More Real-World Applications

| Algorithm | System | Use |
|-----------|--------|-----|
| Counting Sort | Network packet queues (priority 0â€“7) | O(n) per-packet sorting by QoS priority |
...
| Radix Sort | Genomics â€” sorting k-mers by nucleotide order | Alphabet size 4, very long strings |
| Bucket Sort | Approximate sorting in data streams | Bounded error, memory-efficient |
| Bucket Sort | Load balancing â€” partition tasks into priority buckets | O(n) distribution, O(1) per-bucket processing |
...
|---------|-----------|-----------------|----------|
| Counting Sort | Frequency â†’ prefix sum â†’ placement | O(n + k) time, O(n + k) space | Small integer ranges |
| Radix Sort | Digit-by-digit stable sorting | Multiple passes, each a stable sort | Fixed-width integers, strings |
| Bucket Sort | Distribute â†’ sort buckets â†’ concatenate | Expected O(n), worst O(nÂ²) | Uniformly distributed floats |
| LSD vs MSD | Least vs Most Significant Digit first | LSD processes with stable sort | Radix sort choice matters |
| Comparison Bypass | Use key structure, not comparisons | Beats Î©(n log n) bound | Large-scale data with structured keys |

### Quick Reference

| Category | Key Points |
|----------|------------|
| **Key Range Small** | Counting sort â€” O(n + k), stable |
| **Fixed-Width Integers** | Radix sort with base 256 â€” best practical linear sort |
| **Uniform Distribution** | Bucket sort â€” O(n) expected |
| **Stable** | Counting sort, radix sort (per digit) |
| **In-Place** | None of these â€” all require extra space |
...
|-----------|---------------|----------------------|---------------|-------------------|
| Counting Sort | Occasionally â€” sort by age/grade | Rare â€” only when k constraint exists | N/A | Key-indexed counting theory |
| Radix Sort | Sometimes â€” sort strings, sort by multiple keys | Used in suffix array construction | Database sort optimization | Linear-time sorting theory |
| Bucket Sort | Occasionally asked conceptually | Rare â€” needs uniform guarantee | Approximate sorting, histogramming | Average-case analysis |
| Non-Comparison Insight | Frequently â€” "sort in O(n)" questions | Choosing correct algorithm | Large-scale data pipeline design | Lower bound understanding |
...
C) Counting sort â€” O(n + k) where k is the key range.
</details>
...
<details>
<summary>Answer&lt;/summary&gt;
C) O(nÂ²) â€” when all elements land in a single bucket.
</details>
...
<details>
<summary>Answer&lt;/summary&gt;
B) 4 â€” each pass processes 8 bits (256 values), and 32/8 = 4.
</details>

**Q4.** Why does counting sort beat the Î©(n log n) lower bound?
...
<details>
<summary>Answer&lt;/summary&gt;
C) Counting sort â€” the SA-IS algorithm uses counting sort to sort LMS characters by their first character, exploiting the small alphabet size.
</details>
...
6. Explain why counting sort, radix sort, and bucket sort are not subject to the Î©(n log n) lower bound on comparison sorting.

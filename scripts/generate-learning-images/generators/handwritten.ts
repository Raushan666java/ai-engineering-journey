...
export function generateHandwrittenSVG(topic: Topic): string {
  ...
  for (const line of pointLines) {
    pointTexts.push(`    <text x="60" y="${currentY}" font-size="16" fill="#1a1a2e" font-family="'Segoe Print', 'Comic Sans MS', cursive">${line}</text>`);
  ...
  ${titleLines.map((line, i) =>
    `  <text x="60" y="${baseY + i * 36}" font-size="22" font-weight="bold" fill="#1a1a2e" font-family="'Segoe Print', 'Comic Sans MS', cursive" transform="rotate(-0.5 60 ${baseY + i * 36})">${line}</text>`
  ).join('\n')}
  ...
  ${pointTexts.join('\n')}
  <!-- Footer note -->
  <text x="60" y="${Math.min(currentY + 30, SVG_HEIGHT - 30)}" font-size="13" fill="#666" font-family="'Segoe Print', 'Comic Sans MS', cursive" font-style="italic">

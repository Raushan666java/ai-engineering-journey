# Chapter 16: Redis Ã¢â€ â€™ In-Memory Data Store

> **Prev:** [Chapter 15 Ã¢â€ â€™ MongoDB](15-mongodb.md) | **Next:** [Chapter 17 Ã¢â€ â€™ Distributed DB](17-distributed-db.md)
...
|-------|-------------|-------------------|
| **In-Memory Storage** | All data in RAM, disk for persistence only | Monitor memory usage Ã¢â€ â€™ Redis performance drops sharply when data exceeds RAM |
...
**Analogy:** Redis is like a **mechanical keyboard** for data access. A mechanical keyboard registers every keystroke instantly with zero lag because the switch is directly under your finger Ã¢â€ â€™ no membrane layer to press through. Redis keeps every byte in RAM, so reads and writes complete in microseconds without waiting for spinning disks or SSD controllers. Just as a typist relies on that instant key registration for speed, every Redis operation fires directly from RAM with microsecond latency.
...
- **Single-threaded event loop:** All commands execute sequentially on one thread using an event-driven reactor pattern (like Node.js). This means every command is atomic Ã¢â€ â€™ no race conditions within a single instance.
...
- **Client-server protocol:** RESP (REdis Serialization Protocol) Ã¢â€ â€™ simple, human-readable, binary-safe.
...
- Data larger than available RAM (in-memory limitation Ã¢â€ â€™ can lead to OOM or heavy eviction)
- ACID transactions across multiple keys (limited rollback Ã¢â€ â€™ Redis transactions don't roll back on failure)
...
**Key Design Decision Ã¢â€ â€™ Why Single-Threaded?**

| Aspect | Consequence |
|--------|-------------|
| No locks needed | Atomicity is free Ã¢â€ â€™ no mutex, no contention |
...
|-----------|------------|-----|
| SET key value | O(1) | Direct hash table insert Ã¢â€ â€™ key hashed to bucket, value stored |
| GET key | O(1) | Direct hash table lookup Ã¢â€ â€™ no iteration needed |
| DEL key | O(1) | Hash table delete and free value pointer |
| EXISTS key | O(1) | Hash table membership check |
| KEYS pattern | O(N) | Must iterate every key in the keyspace Ã¢â€ â€™ blocks event loop |
| FLUSHALL | O(N) | Deletes every key Ã¢â€ â€™ O(N) in total keys; instantaneous in effect but blocking |
...
**Analogy:** Think of Redis data types as a **Swiss Army knife** Ã¢â€ â€™ each tool is designed for a specific cut. A blade (string) does the common jobs, scissors (list) handle linear sequences, pliers (hash) grip multi-faceted objects, and the awl (sorted set) pierces with precision scoring. Using the wrong tool is like cutting rope with scissors Ã¢â€ â€™ it works, but slowly.
...
**Analogy:** Strings are like **sticky notes on a monitor** Ã¢â€ â€™ you write a short value, stick it somewhere memorable, and read it instantly. The value can be a number you increment (like a tally mark), a JSON blob (like a detailed note), or binary data (like a photo). They're the simplest and most versatile tool.

The most basic Redis type. A value can be a string, number, binary data, or serialized JSON. Maximum value size: 512MB.

**Numbered Steps Ã¢â€ â€™ String Operations:**
...
**Pseudocode Ã¢â€ â€™ Core String Implementation:**
...
| 3 | `INCRBY counter 50` | counter = "151" | 151 | Atomic add of 50 Ã¢â€ â€™ no race possible |
...
DEL user:1:email                           # (integer) 1 Ã¢â€ â€™ returns count of deleted keys
...
INCRBYFLOAT price 15.99                    # 115.99 Ã¢â€ â€™ floating-point increment
...
PERSIST session:abc123                      # Remove TTL Ã¢â€ â€™ persists forever
...
GETSET user:1:name "Bob"                   # "Alice" Ã¢â€ â€™ returns old, sets new

# Range operations (bit-level)
SET mykey "hello"                          # OK
GETRANGE mykey 0 2                         # "hel" Ã¢â€ â€™ substring
STRLEN mykey                                # 5 Ã¢â€ â€™ string length

# Append to string
APPEND mykey " world"                      # 11 Ã¢â€ â€™ new length
...
void stringOperations() {
    ...
    try {
        ...
        long val = redis.incr("counter");        // 101 Ã¢â€ â€™ atomic increment
...
# Numeric operations (atomic Ã¢â€ â€™ no race conditions across processes)
r.set('counter', 100)
...
# TTL Ã¢â€ â€™ automatic expiration
...
| INCR | O(1) | Parse string to int, increment, store Ã¢â€ â€™ all constant time |
...
| Binary-safe Ã¢â€ â€™ can store images, serialized data | String encoding wastes space for numeric types |
...
| Key with TTL already expired | GET returns nil, key auto-deleted | Check return value Ã¢â€ â€™ never assume key exists |
...
| INCR on key with TTL | Preserves TTL Ã¢â€ â€™ never clears it | Use SET with EX to reset TTL if needed |

#### 16.2.2 Lists

**Analogy:** A Redis list is like a **conveyor belt in a warehouse** Ã¢â€ â€™ items arrive on the left (LPUSH), workers pick them off the right (RPOP) for processing. It's a perfect FIFO queue. You can also use it as a stack (LPUSH + LPOP = LIFO), or a timeline (LPUSH + LRANGE to show recent items). The linked-list implementation means millions of items barely slow down head/tail operations.

Ordered sequences of strings implemented as **linked lists** Ã¢â€ â€™ fast head/tail operations (O(1)), slow random access by index (O(N)). Maximum length: 2^32 - 1 (over 4 billion elements).

**Numbered Steps Ã¢â€ â€™ List Queue Pattern (FIFO):**

1. Producer calls `LPUSH queue:task "process_email"` Ã¢â€ â€™ item prepended to list head
2. Redis creates/updates key as list type; links new node at head
3. Consumer calls `RPOP queue:task` Ã¢â€ â€™ item removed from tail
...
6. Multiple consumers can block on the same key Ã¢â€ â€™ each element goes to exactly one consumer

**Pseudocode Ã¢â€ â€™ List Queue Implementation:**
...
function LPUSH(key, value):
    ...
    list = dict[key]
    list.prepend(value)              // O(1) Ã¢â€ â€™ link new head node
...
function RPOP(key):
    ...
    if list is empty:
        return nil
    value = list.removeTail()        // O(1) Ã¢â€ â€™ unlink tail node
    return value
...
function BRPOP(key, timeout_ms):
    ...
    while true:
        ...
        if value is not nil:
            return [key, value]
        if timeout_ms == 0:
            // Block indefinitely Ã¢â€ â€™ subscribe to push notifications
...
**Dry Run Ã¢â€ â€™ List Timeline (Recent Items):**

| Step | Command | List State (left Ã¢â€ â€™ right) | Result | Explanation |
|------|---------|---------------------------|--------|-------------|
...
LPOP queue:tasks                     # "task:1" Ã¢â€ â€™ oldest item first

# Create a stack (LIFO: LPUSH + LPOP)
LPUSH stack:actions "action:1"       # 1
LPUSH stack:actions "action:2"       # 2
LPOP stack:actions                   # "action:2" Ã¢â€ â€™ most recent first
...
# Atomic: RPOP from source Ã¢â€ â€™ LPUSH to destination

# Blocking variant Ã¢â€ â€™ used for reliable queues
...
| Operation | Complexity | Why |
|-----------|------------|-----|
| LPUSH / RPUSH | O(1) | Link new node at head or tail Ã¢â€ â€™ pointer assignment |
| LPOP / RPOP | O(1) | Unlink head or tail node Ã¢â€ â€™ pointer reassignment |
...
| Advantage | Disadvantage |
|-----------|-------------|
| O(1) head/tail operations Ã¢â€ â€™ ideal for queues/stacks | O(N) random access Ã¢â€ â€™ no indexed lookup |
...
| LRANGE start exceeds list length | Returns empty array | No error Ã¢â€ â€™ safe to call |
| LTRIM removes all elements | Key is deleted (empty list removed) | EXISTS returns 0 after LTRIM 0 0 on single element |

#### 16.2.3 Sets

**Analogy:** Redis sets are like a **bouncer's clipboard at an exclusive club** Ã¢â€ â€™ each name is checked once (unique), membership is instant (SISMEMBER), and you can combine two clipboards to find the combined guest list (union), the mutual friends (intersection), or who's on one list but not the other (difference). No duplicates allowed Ã¢â€ â€™ the bouncer crosses out repeated names instantly.
...
**Numbered Steps Ã¢â€ â€™ Set Intersection (Mutual Friends):**
...
7. Result: `"carol"` Ã¢â€ â€™ the mutual friend

**Pseudocode Ã¢â€ â€™ Set Intersection:**
...
**Dry Run Ã¢â€ â€™ Set Operations (Social Graph):**

| Step | Command | Set State (alice) | Set State (bob) | Result | Explanation |
|------|---------|-------------------|------------------|--------|-------------|
| 1 | `SADD friends:alice "bob"` | {bob} | Ã¢â€ â€™ | 1 | Added |
| 2 | `SADD friends:alice "carol"` | {bob, carol} | Ã¢â€ â€™ | 1 | Added |
...
SCARD user:1:interests                                             # 3 Ã¢â€ â€™ cardinality

# Set operations Ã¢â€ â€™ social features
...
SINTERSTORE group:dev_managers group:devs group:managers           # 1 Ã¢â€ â€™ creates new set
...
**C++ Implementation (Set Ã¢â€ â€™ Social Graph):**
...
void setOperations() {
    ...
    // Intersection Ã¢â€ â€™ mutual members
...
**Python Implementation (Set Ã¢â€ â€™ Tags and Filtering):**
...
# Tag intersection Ã¢â€ â€™ find common tags
common = r.sinter('article:42:tags', 'article:99:tags')
print(f"Common tags: {common}")  # {'redis'}

# Tag union Ã¢â€ â€™ all unique tags across articles
...
# Spop Ã¢â€ â€™ remove and return (useful for job assignment)
...
| Operation | Complexity | Why |
|-----------|------------|-----|
| SADD | O(1) | Hash table insert Ã¢â€ â€™ direct bucket insertion |
| SREM | O(1) | Hash table delete Ã¢â€ â€™ direct bucket removal |
| SISMEMBER | O(1) | Hash table lookup Ã¢â€ â€™ constant time |
...
| SUNION | O(N) | Sum of all set sizes Ã¢â€ â€™ iterate and deduplicate |
| SDIFF | O(N) | Size of first set Ã¢â€ â€™ iterate and check exclusion |
...
| Advantage | Disadvantage |
|-----------|-------------|
| O(1) membership check Ã¢â€ â€™ instant existence test | Unordered Ã¢â€ â€™ no ranking or ordering within set |
| Built-in set algebra (union, intersect, diff) | No duplicate tracking Ã¢â€ â€™ duplicates silently ignored |
...
| SREM of non-existing member | Returns 0 | No error Ã¢â€ â€™ idempotent operation |
...
**Numbered Steps Ã¢â€ â€™ Sorted Set Leaderboard Update:**

1. `ZADD leaderboard:week1 1500 "alice"` Ã¢â€ â€™ Redis looks up key in dict
...
4. Insert into hash table: O(1) map member Ã¢â€ â€™ score for direct lookups
...
**Pseudocode Ã¢â€ â€™ Sorted Set Implementation:**
...
**Dry Run Ã¢â€ â€™ Sorted Set Leaderboard:**

| Step | Command | State (member Ã¢â€ â€™ score) | Rank Order (lowÃ¢â€ â€™high) | Result | Explanation |
|------|---------|----------------------|----------------------|--------|-------------|
...
| 7 | `ZINCRBY lb 300 "alice"` | alice=1500Ã¢â€ â€™1800 | [(dave,950),(carol,1800),(alice,1800),(bob,2200)] | "1800" | Alice ties carol at 1800; alice after carol (insertion order) |
...
# Score updates
ZINCRBY leaderboard:week1 300 "alice"                          # "1800" Ã¢â€ â€™ atomic increment
...
ZCOUNT leaderboard:week1 1000 2000                             # 2 Ã¢â€ â€™ count in score range
...
ZLEXCOUNT lex:set "-" "+"                                      # 3 Ã¢â€ â€™ all members
...
# Blocking pop Ã¢â€ â€™ BZPOPMIN / BZPOPMAX (Redis 6.2+)
BZPOPMIN leaderboard:week1 30                                  # Pop lowest with score
```

**C++ Implementation (Sorted Set Ã¢â€ â€™ Leaderboard):**
...
**Python Implementation (Sorted Set Ã¢â€ â€™ Rate Limiter with Sliding Window):**
...
class SlidingWindowRateLimiter:
    ...
    def is_allowed(self, user_id: str) -> bool:
        ...
        # Execute pipeline Ã¢â€ â€™ returns list of results
...
| ZRANK | O(log N) | Skip list rank query Ã¢â€ â€™ traverse levels summing span |
| ZREVRANK | O(log N) | Same as ZRANK, traversing from tail |
| ZSCORE | O(1) | Hash table lookup Ã¢â€ â€™ direct member Ã¢â€ â€™ score map |
...
| Built-in ranking and range queries | Score is a single 64-bit double Ã¢â€ â€™ precision limits for large integers |
| Atomic score increment (ZINCRBY) Ã¢â€ â€™ no race conditions | Members with equal scores ordered lexicographically |
...
| ZREVRANGE start > length | Returns empty array | Safe Ã¢â€ â€™ no error thrown |
| ZINCRBY on non-existing member | Creates member with score = increment amount | Auto-creates Ã¢â€ â€™ may not be desired |
...
**Analogy:** A Redis hash is like a **file folder with labeled tabs** Ã¢â€ â€™ the folder is the key (user:1001), each tab is a field (name, email, age), and behind each tab is a specific value. Unlike a string that stores a whole JSON blob, a hash lets you read or write a single field without touching the others. It's the difference between pulling one document from a filing cabinet (hash) vs photocopying the entire drawer (string).
...
**Numbered Steps Ã¢â€ â€™ Hash Object Operations:**

1. `HSET user:1001 name "Alice Chen"` Ã¢â€ â€™ creates hash key if not exists
2. Redis checks encoding: small hash Ã¢â€ â€™ ziplist (memory efficient), large Ã¢â€ â€™ hash table
3. Inserts field "name" with value "Alice Chen" into internal structure
4. `HGET user:1001 name` Ã¢â€ â€™ direct field lookup, returns value
5. `HGETALL user:1001` Ã¢â€ â€™ iterate all field-value pairs
6. `HINCRBY user:1001 login_count 1` Ã¢â€ â€™ atomic field increment (like INCR for hash fields)

**Pseudocode Ã¢â€ â€™ Hash Operations:**
...
**Dry Run Ã¢â€ â€™ Hash User Profile:**

| Step | Command | Hash State (field Ã¢â€ â€™ value) | Result | Explanation |
|------|---------|---------------------------|--------|-------------|
...
HGET user:1001 age                                        # (nil) Ã¢â€ â€™ not set
...
# Incremental fetch Ã¢â€ â€™ HSCAN (non-blocking)
HSCAN user:1001 0 COUNT 100                               # Cursor-based iteration
```

**C++ Implementation (Hash Ã¢â€ â€™ Session Store):**
...
**Python Implementation (Hash Ã¢â€ â€™ User Profile):**
...
| Operation | Complexity | Why |
|-----------|------------|-----|
| HSET | O(1) | Hash table insert Ã¢â€ â€™ direct bucket placement |
...
| HINCRBY | O(1) | Parse, add, store Ã¢â€ â€™ all constant time |
...
| Advantage | Disadvantage |
|-----------|-------------|
| Field-level access Ã¢â€ â€™ no need to read/write entire object | Cannot set TTL on individual fields (entire key expires) |
...
**Analogy:** A bitmap is like a **parking lot occupancy board** Ã¢â€ â€™ each parking spot is either occupied (1) or empty (0). You can ask "is spot #42 occupied?" (GETBIT), count all occupied spots (BITCOUNT), or find empty spots between rows (BITOP). At 1 bit per entry, a million users' daily activity fits in just 122 KB.
...
GETBIT user:active:202601 42             # 1 Ã¢â€ â€™ user 42 was active
GETBIT user:active:202601 999            # 0 Ã¢â€ â€™ user 999 wasn't
...
# Union Ã¢â€ â€™ users active on either day
BITOP OR dau:week1 dau:2026-01-01 dau:2026-01-02
BITCOUNT dau:week1                        # 3 Ã¢â€ â€™ users 100, 200, 300

# Intersection Ã¢â€ â€™ users active on both days
BITOP AND dau:both dau:2026-01-01 dau:2026-01-02
BITCOUNT dau:both                         # 1 Ã¢â€ â€™ user 100

# Find first set bit
BITPOS user:active:202601 1               # Position of first active user
```

**Complexity:** O(1) for GETBIT/SETBIT (byte-aligned). O(N) for BITCOUNT/BITOP (N = bytes processed). BITCOUNT uses Hamming weight (popcount) hardware instructions on modern CPUs Ã¢â€ â€™ extremely fast.

#### 16.2.7 HyperLogLog

**Analogy:** HyperLogLog is like **estimating the crowd at a concert** without counting every person Ã¢â€ â€™ you look at the number of unique birthday months in the first 100 people (if you see Jan, Feb, Mar... then Dec, you have probably ~365+ people). HLL uses clever probability theory to estimate cardinality using ~12KB of memory, regardless of whether you've seen 1,000 or 1 billion unique items.
...
PFADD daily:visitors:2026-01-01 "ip:192.168.1.1"                    # Duplicate Ã¢â€ â€™ not counted
...
**Analogy:** Geo indices are like a **GPS-enabled friend-finder app** Ã¢â€ â€™ you can register your location (GEOADD), ask "who's within 5km?" (GEORADIUS), or "how far is Alice from Bob?" (GEODIST). Internally, Redis encodes lat/lon pairs as a 52-bit Geohash stored in a sorted set.
...
**Analogy:** Streams are like an **airport black box flight recorder** Ã¢â€ â€™ every event is logged sequentially with a timestamp (the entry ID), you can replay from any point, and multiple analysts (consumer groups) can independently read the same log without interfering. Unlike Pub/Sub (think: radio broadcast Ã¢â€ â€™ if you tune in late, you missed it), streams persist messages until explicitly trimmed.
...
**Numbered Steps Ã¢â€ â€™ Stream Consumer Group Processing:**

1. `XADD sensor:temp * sensor_id "s1" temperature 22.5` Ã¢â€ â€™ server generates ID `1735689600000-0`
2. Entry appended to radix tree at the key `sensor:temp`
3. `XGROUP CREATE sensor:temp group1 $` Ã¢â€ â€™ creates consumer group, $ means "new entries only"
4. `XREADGROUP GROUP group1 consumer1 COUNT 1 STREAMS sensor:temp >` Ã¢â€ â€™ ">" means "give me entries I haven't read"
5. Redis tracks pending entries per consumer (PEL Ã¢â€ â€™ Pending Entry List)
6. Consumer processes message and calls `XACK sensor:temp group1 <entry-id>` to acknowledge
7. Unacknowledged entries remain in PEL Ã¢â€ â€™ other consumers can claim them via `XAUTOCLAIM`

**Pseudocode Ã¢â€ â€™ Stream Consumer Group:**
...
**Dry Run Ã¢â€ â€™ Stream Consumer Group:**

| Step | Command | Stream State | PEL (pending) | Explanation |
|------|---------|-------------|---------------|-------------|
| 1 | `XADD events * type "login"` | ID: 1735689600000-0 | Ã¢â€ â€™ | Entry added |
| 2 | `XADD events * type "purchase"` | ID: 1735689600001-0 | Ã¢â€ â€™ | 2nd entry |
| 3 | `XADD events * type "logout"` | ID: 1735689600002-0 | Ã¢â€ â€™ | 3rd entry |
| 4 | `XGROUP CREATE events grp1 $` | Ã¢â€ â€™ | Ã¢â€ â€™ | Group created, starts after last entry |
| 5 | `XADD events * type "search"` | ID: 1735689600003-0 | Ã¢â€ â€™ | New entry (readable by group) |
| 6 | `XREADGROUP GROUP grp1 c1 COUNT 2 STREAMS events >` | Ã¢â€ â€™ | {0003-0: c1} | c1 gets 2 pending entries |
| 7 | `XACK events grp1 1735689600003-0` | Ã¢â€ â€™ | {0004-0: c1} | First entry acknowledged |
| 8 | `XREADGROUP GROUP grp1 c1 STREAMS events >` | Ã¢â€ â€™ | {0004-0: c1} | No more new entries |
...
# Autoclaim Ã¢â€ â€™ handle crashed consumers (after 60 seconds)
...
**C++ Implementation (Stream Ã¢â€ â€™ Producer/Consumer):**
...
void consumer(Redis& redis, const std::string& name) {
    try {
        ...
        // Group already exists Ã¢â€ â€™ ignore
    }
...
**Python Implementation (Stream Ã¢â€ â€™ Event Processing):**
        ...
        try:
            ...
            for stream_name, entries in results:
                for entry_id, fields in entries:
                    ...
                    # Acknowledge Ã¢â€ â€™ remove from pending list
...
**Analogy:** Redis persistence is like a **journal and a photograph** of your desk. The AOF (journal) records every single action you take Ã¢â€ â€™ "picked up pen," "wrote note," "moved paper" Ã¢â€ â€™ so you can replay everything exactly. The RDB (photograph) takes a picture of the current state Ã¢â€ â€™ fast to load, but any changes after the photo are lost. The hybrid mode is like taking a photo and then keeping a small journal of changes since the photo: best of both.

#### 16.3.1 RDB (Redis Database File)

Point-in-time snapshots. Redis forks a child process that writes the entire dataset to disk.

**Numbered Steps Ã¢â€ â€™ RDB Save:**

1. Client issues `SAVE` (sync) or `BGSAVE` (async), or a `save` config condition triggers
2. On `BGSAVE`, parent process forks via `fork()` Ã¢â€ â€™ creates a child process
...
**Dry Run Ã¢â€ â€™ COW Memory During BGSAVE:**
...
| 4 | T2 | Parent | Modifies a key Ã¢â€ â€™ page fault, OS copies page | 4.0 GB + 4KB per modified page |
| 5 | T3 | Parent | Continue accepting writes | Grows by modified pages ÃƒÆ’Ã¢â‚¬â€ 4KB |
| 6 | T4 | Child | Finishes writing RDB | Child exits, pages freed |
| 7 | T5 | Parent | COW pages reclaimed | Back to ~4.0 GB |

**Python Ã¢â€ â€™ Manual RDB Save with Monitoring:**
...
# Or trigger sync save (blocking Ã¢â€ â€™ do not use in production)
# r.save()
...
**Numbered Steps Ã¢â€ â€™ AOF Rewrite:**
...
**Python Ã¢â€ â€™ AOF Configuration:**
...
#### 16.3.3 Hybrid Persistence (RDB + AOF Ã¢â€ â€™ Redis 6.2+)

**Numbered Steps Ã¢â€ â€™ Hybrid Persistence Load:**

1. On startup, Redis checks for AOF file (if `appendonly yes`)
2. AOF file begins with RDB format header (hybrid mode)
3. Redis loads the RDB portion Ã¢â€ â€™ fast bulk load of all data
...
[INCR user:1:counter ...]          Ã¢â€ Â incremental commands after snapshot
[SET user:2:name "Bob" ...]
[...more AOF entries...]
```
...
| **Load speed** | Fast Ã¢â€ â€™ single file load | Slow Ã¢â€ â€™ replay all commands | Medium Ã¢â€ â€™ RDB load + command replay |
...
|----------|-------------------|
| **Cache only** (no persistence) | None Ã¢â€ â€™ max performance |
| **Session store** with restart tolerance | RDB Ã¢â€ â€™ fast restart |
| **Message queue** (no message loss) | AOF everysec Ã¢â€ â€™ minimal data loss |
| **Database + cache** | Hybrid (RDB+AOF) |
| **Analytics / time series** | RDB Ã¢â€ â€™ periodic snapshots |
| **Financial / critical data** | AOF always Ã¢â€ â€™ zero data loss (at performance cost) |

**Edge Cases:**

| Scenario | Behavior | Mitigation |
|----------|----------|------------|
| BGSAVE fails (disk full) | `stop-writes-on-bgsave-error yes` Ã¢â€ â€™ writes rejected | Monitor disk space; disable in cache-only mode |
| Fork fails (OOM) | BGSAVE returns error; no snapshot taken | Set `vm.overcommit_memory=1` in sysctl |
| AOF file truncated (crash) | `aof-load-truncated yes` Ã¢â€ â€™ loads partially | Enable; set to no for strict environments |
| AOF rewrite fails mid-way | New temp file deleted; old AOF preserved | No data loss Ã¢â€ â€™ retry on next trigger |
| Large key (>10MB) in COW | Every write to that key copies 10MB during BGSAVE | Avoid giant keys; split into chunks |
| Multiple BGSAVE/AOF rewrites | Serialized Ã¢â€ â€™ second call returns error | Check `info persistence` before triggering |
...
**Numbered Steps Ã¢â€ â€™ Replication Handshake:**

1. Replica sends `REPLICAOF master_host master_port` 
2. Master and replica: TCP connection established (non-blocking)
3. Master: `PSYNC ? -1` Ã¢â€ â€™ replica asks for full sync (no partial sync state)
...
**Pseudocode Ã¢â€ â€™ Replication Stream (Steady State):**
...
**Dry Run Ã¢â€ â€™ Replication Synchronization:**

| Time | Master | Replica | Explanation |
|------|--------|---------|-------------|
| T0 | Running, data=4GB | Empty, connects | Ã¢â€ â€™ |
...
**C++ Ã¢â€ â€™ Minimal Replication Check:**
...
**Python Ã¢â€ â€™ Replication Monitoring:**
...
### 16.5 Redis Sentinel Ã¢â€ â€™ High Availability
...
**Numbered Steps Ã¢â€ â€™ Sentinel Failover:**
...
4. If ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ `quorum` Sentinels agree, master marked **objectively down** (O-DOWN)
...
7. Leader sends `SLAVEOF NO ONE` to the chosen replica Ã¢â€ â€™ it becomes master
...
**Pseudocode Ã¢â€ â€™ Sentinel Leader Election (Raft-like):**
...
**Dry Run Ã¢â€ â€™ Sentinel Failover:**
...
**Python Ã¢â€ â€™ Sentinel Client with Auto-Failover:**
...
# Normal operations Ã¢â€ â€™ auto-failover handled by Sentinel
...
| Quorum not reached | No failover Ã¢â€ â€™ all replicas stale | Deploy odd number of Sentinels (3, 5, 7) |
| Old master rejoins after promotion | Becomes replica of new master | Auto-handled by Sentinel |
| All Sentinels lose master simultaneously | No O-DOWN Ã¢â€ â€™ no failover | Impossible with proper quorum |
| Failover during heavy write load | Data loss Ã¢â€ â€™ unsynced writes on old master | Use WAIT command for synchronous replication |
### 16.6 Redis Cluster Ã¢â€ â€™ Automatic Sharding


**Analogy:** A Redis Cluster is like a **warehouse with multiple shelves labeled A-Z** Ã¢â€ â€™ you don't search every shelf for an item; the manifest tells you which shelf holds which items. Each shelf (node) holds a range of letters (hash slots). Adding a shelf means redistributing labels Ã¢â€ â€™ you can expand without rebuilding the warehouse. If one shelf collapses, its backup shelf takes over while you fix it.
...
**Numbered Steps Ã¢â€ â€™ Cluster Key Lookup:**

1. Client computes CRC16(`user:1001`) & 16383 Ã¢â€ â€™ slot 1234
2. Client's cluster client knows slot Ã¢â€ â€™ node mapping
...
**Pseudocode Ã¢â€ â€™ Cluster Key Routing:**
...
function send_command(key, command, args):
    ...
    if response.type == "MOVED":
        // Permanent slot migration Ã¢â€ â€™ update table
...
**Dry Run Ã¢â€ â€™ Cluster Slot Redirection:**

| Step | Client | Node 1 (slots 0-5460) | Node 2 (slots 5461-10922) | Node 3 (slots 10923-16383) |
|------|--------|----------------------|--------------------------|---------------------------|
| 1 | Computes slot for `user:1001` | slot 8964 (node 2) | Ã¢â€ â€™ | Ã¢â€ â€™ |
| 2 | Sends GET `user:1001` | Wrong node! | Correct node | Ã¢â€ â€™ |
| 3 | | Returns `-MOVED 8964 10.0.0.2:6379` | Ã¢â€ â€™ | Ã¢â€ â€™ |
| 4 | Updates slot map, resends | Ã¢â€ â€™ | Returns "Alice" | Ã¢â€ â€™ |
| 5 | Re-sharding: slot 8964 moves to node 3 | Ã¢â€ â€™ | Returns `-ASK 8964 10.0.0.3:6379` | Ready to receive |
| 6 | Sends ASKING, then GET | Ã¢â€ â€™ | Ã¢â€ â€™ | Returns "Alice" |

**Python Ã¢â€ â€™ Cluster Client:**
...
# Normal operations Ã¢â€ â€™ client handles routing
rc.set('user:1001:name', 'Alice')
rc.set('user:2001:name', 'Bob')
name = rc.get('user:1001:name')

# Pipeline in cluster Ã¢â€ â€™ keys must be in same slot (use hash tags)
pipe = rc.pipeline()
...
**A&D Table Ã¢â€ â€™ Sentinel vs Cluster:**

| Aspect | Sentinel | Cluster |
|--------|----------|---------|
| Purpose | High availability | High availability + horizontal scaling |
| Data sharding | No Ã¢â€ â€™ all nodes have full data | Yes Ã¢â€ â€™ 16384 hash slots across nodes |
...
| Large dataset reshard | Slow Ã¢â€ â€™ one key at a time | Use Redis Enterprise for live resharding |
...
7. Message is **fire-and-forget** Ã¢â€ â€™ no persistence, no delivery guarantee
...
**Python Ã¢â€ â€™ Pub/Sub Chat Application:**
...
**Python Ã¢â€ â€™ Pattern Subscribe for Monitoring:**
...
**A&D Table Ã¢â€ â€™ Pub/Sub vs Streams:**

| Aspect | Pub/Sub | Streams |
|--------|---------|---------|
| Message persistence | None Ã¢â€ â€™ fire and forget | Persistent until trimmed |
| Consumer groups | No Ã¢â€ â€™ broadcast to all | Yes Ã¢â€ â€™ load-balanced delivery |
...
**Analogy:** Redis transactions are like a **batch of commands on a shopping list** Ã¢â€ â€™ you write down everything you want to do (MULTI), check the list once, then execute every item in sequence (EXEC). If the store closes mid-list, you keep going and finish the rest (no rollback). Unlike SQL databases where a failed transaction undoes everything, Redis guarantees everything in the batch runs **without interruption** but keeps going past failures.

**Numbered Steps Ã¢â€ â€™ Transaction with WATCH:**

1. Client: `WATCH balance` Ã¢â€ â€™ sets optimistic lock on key
2. Client: `GET balance` Ã¢â€ â€™ 100 (outside transaction, for logic)
3. Client: `MULTI` Ã¢â€ â€™ enters transaction mode
4. Client: `DECRBY balance 50` Ã¢â€ â€™ queued (not executed)
5. Client: `EXEC` Ã¢â€ â€™ Redis checks if `balance` was modified since WATCH
6. If modified Ã¢â€ â€™ return nil (transaction aborted)
7. If not modified Ã¢â€ â€™ execute all queued commands atomically

**Pseudocode Ã¢â€ â€™ WATCH/MULTI/EXEC:**
...
**Dry Run Ã¢â€ â€™ Transaction with WATCH:**

| Step | Client A | Client B | Balance | Explanation |
|------|----------|----------|---------|-------------|
| 1 | WATCH balance | Ã¢â€ â€™ | 100 | A sets watch |
| 2 | GET balance Ã¢â€ â€™ 100 | Ã¢â€ â€™ | 100 | Read current value |
| 3 | MULTI | Ã¢â€ â€™ | 100 | Begin transaction |
| 4 | DECRBY balance 50 | Ã¢â€ â€™ | 100 | Queued |
| 5 | Ã¢â€ â€™ | SET balance 200 | 200 | B modifies! |
| 6 | EXEC | Ã¢â€ â€™ | 200 | **FAILS** Ã¢â€ â€™ balance changed since WATCH |
| 7 | Returns nil | Ã¢â€ â€™ | 200 | A must retry |

**Python Ã¢â€ â€™ Transaction with WATCH and Retry:**
...
def transfer_funds(from_key, to_key, amount, max_retries=5):
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            # Someone modified watched keys Ã¢â€ â€™ retry
...
| WATCH key modified but same value | Still fails Ã¢â€ â€™ version changed | Race condition; retry |
...
**Pseudocode Ã¢â€ â€™ Lua Script in Redis:**
...
**Python Ã¢â€ â€™ Atomic Inventory Check with Lua:**
...
| EVALSHA (cached) | O(N) execution only | No parse overhead Ã¢â€ â€™ just execute |
| Script execution | Blocking Ã¢â€ â€™ all other commands queued | Single-threaded: script holds event loop |
...
| Advantage | Disadvantage |
|-----------|-------------|
| Atomic multi-command logic without network RTT | Blocks the event loop Ã¢â€ â€™ no other commands processed during execution |
| Script caching via EVALSHA (reduces bandwidth) | No debugging support Ã¢â€ â€™ error messages are cryptic |
| Conditional logic (if/else) impossible with plain MULTI/EXEC | Memory limit Ã¢â€ â€™ must not exceed `lua-time-limit` (5s default) |
| Replicated to replicas (deterministic) | Must be deterministic Ã¢â€ â€™ no random/date calls unless using `redis.replicate_commands()` |
| Server-side data processing | Scripts use `redis.sha1hex` for hashing; no external calls |
### 16.10 Caching Patterns


**Analogy:** Caching strategies are like different **meal-prep approaches** for a busy week. Cache-aside: you check the fridge first; if the meal isn't there, you cook it and put leftovers in the fridge. Read-through: a personal chef (cache library) checks the fridge and cooks if needed without you thinking about it. Write-through: you cook and immediately package the leftovers Ã¢â€ â€™ nothing is raw. Refresh-ahead: you guess tomorrow's lunch and pre-cook it while everyone's asleep.
...
5. Next request: cache hit Ã¢â€ â€™ return immediately
...
class UserRepository:
    ...
    def get_user(self, user_id):
        ...
        # 2. Cache miss Ã¢â€ â€™ load from database
        user = self.db.query(
    ...
    def update_user(self, user_id, data):
        ...
        # Invalidate cache Ã¢â€ â€™ next read will repopulate
...
1. Application calls `cache.get("user:42")` Ã¢â€ â€™ the cache library handles the logic
2. Cache misses Ã¢â€ â€™ loader function is called automatically
3. Loader queries DB and returns data
4. Cache library stores data with TTL and returns to app
5. Application is unaware of the database Ã¢â€ â€™ it just asks the cache
...
# App just calls get() Ã¢â€ â€™ cache library handles everything
...
class WriteThroughCache:
    ...
    def set_user(self, user_id, data):
        ...
        # Note: if DB fails, cache has stale data Ã¢â€ â€™ rollback pattern needed
...
class RefreshAheadCache:
    ...
    def get(self, key):
        ...
        if ttl < 0:
            # Key expired or doesn't exist Ã¢â€ â€™ load fresh
            data = self.loader(key)
            if data:
                self.cache.setex(key, self.ttl, json.dumps(data))
            return data
        ...
        if ttl < self.refresh_before:
            # Nearing expiry Ã¢â€ â€™ trigger async refresh in background
...
This is NOT true LRU Ã¢â€ â€™ it's an approximation that works well in practice.
Higher maxmemory-samples (10) Ã¢â€ â€™ more accurate, more CPU.
```
...
**Answer:** Use Redis when your workload is **read-heavy, latency-sensitive, and fits in RAM**. Redis excels at sub-millisecond reads for caching, session storage, real-time analytics, and leaderboards. Use PostgreSQL/MySQL when you need complex queries, joins, ACID transactions across keys, data larger than RAM, or long-term durability. Redis is a force multiplier for your database Ã¢â€ â€™ not a replacement.

#### Q2: How does cache invalidation work in Redis?

**Answer:** Three strategies: (a) **TTL-based**: set `EXPIRE` and let Redis auto-delete Ã¢â€ â€™ simplest, but may serve stale data within TTL window. (b) **Explicit invalidation**: on data update, `DEL` the cache key Ã¢â€ â€™ next read repopulates (cache-aside). (c) **Write-through**: update cache and DB simultaneously Ã¢â€ â€™ strong consistency, higher write latency. Redis keyspace notifications (`__keyspace@0__:*`) can trigger invalidation across services.

#### Q3: Redis vs Memcached Ã¢â€ â€™ which and when?

**Answer:** Memcached for simple caching with maximum throughput (multi-threaded, lower overhead). Redis when you need data structures (lists, sets, sorted sets), persistence (RDB/AOF), replication, Pub/Sub, or value sizes > 1MB. In modern systems, Redis is almost always preferred because the feature advantage outweighs the minor performance gap. Benchmarks show Redis on modern hardware handles 100K-1M ops/sec Ã¢â€ â€™ sufficient for most applications.

#### Q4: Sentinel vs Cluster Ã¢â€ â€™ which deployment should I use?
...
**Answer:** When a cached key expires and N concurrent requests all trigger a cache miss, they all hit the database simultaneously. Solutions: (a) **Mutex lock**: first request acquires a lock on the cache key, others wait. (b) **Stale-while-revalidate**: serve expired data while asynchronously refreshing (Redis doesn't support this natively Ã¢â€ â€™ implement in application). (c) **Early recalculation** (refresh-ahead): proactively refresh before expiry. (d) **Jittered TTL**: add random variance to TTL values so keys don't expire simultaneously.
...
1. **Use hashes instead of plain keys for objects** Ã¢â€ â€™ individual keys for each field (user:1001:name, user:1001:email) cause key explosion. Use `HSET user:1001 name "Alice" email "alice@example.com"` instead.
2. **Always set TTL for cache data** Ã¢â€ â€™ without expiration, unused keys waste memory forever. Redis eviction policies are a safety net, not a strategy.
3. **SCAN is your production friend, not KEYS** Ã¢â€ â€™ `KEYS *` blocks Redis for potentially seconds on large datasets. `SCAN` with cursor-based iteration is non-blocking.
4. **Sorted sets are Redis's superpower** Ã¢â€ â€™ leaderboards, rate limiters, priority queues, and time-series queries all benefit from the O(log N) insertion + O(log N + M) range query.
5. **Be careful with large keys/values** Ã¢â€ â€™ a single key larger than 10MB can block replication and slow down the entire instance. Consider compression or splitting.
6. **Hash tags for cluster multi-key ops** Ã¢â€ â€™ use `{user:1001}:name` and `{user:1001}:email` to force same slot, enabling atomic multi-key operations in Cluster mode.
7. **Pipeline or lose throughput** Ã¢â€ â€™ sending 100 commands individually costs 100 round-trips. Pipelining sends them all in one batch for ~1 round-trip.
8. **Monitor for BGSAVE memory spikes** Ã¢â€ â€™ `fork()` may cause 2x memory usage with COW under heavy writes. Set `vm.overcommit_memory=1` and monitor RSS.
...
- **16.2:** Each data type has specialized commands optimized for its structure Ã¢â€ â€™ LPUSH/RPOP for queues, ZADD/ZRANGE for leaderboards, HSET/HGET for objects.
- **16.3:** Persistence options Ã¢â€ â€™ RDB (point-in-time snapshots), AOF (append-only log), and hybrid Ã¢â€ â€™ offer different durability vs. performance trade-offs.
...
### 16.9 TypeScript Redis Command Executor & Caching Strategy

The TypeScript implementation simulates Redis data structures, commands, TTL, and caching patterns.

```typescript
// ============================================================
// Redis Command Executor Ã¢â‚¬â€ TypeScript
// ============================================================
...
class CacheAside {
  ...
  get(key: string): string | null {
    ...
    // Cache miss Ã¢â‚¬â€ load from DB
    console.log('[Cache] MISS for ' + key + ' Ã¢â‚¬â€ loading from DB');
...
11. Redis is best classified as which type of NoSQL database?
    a) Document store
    b) Key-value store
    c) Column-family store
    d) Graph database

12. The cache-aside pattern requires the application to:
...
13. Redis persistence using AOF (Append-Only File) works by:
...
14. The TTL (Time-To-Live) feature in Redis is used for:

...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/artificial-intelligence/04-csp/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/artificial-intelligence/04-csp/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/artificial-intelligence/04-csp/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/artificial-intelligence/04-csp/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/artificial-intelligence/04-csp/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/artificial-intelligence/04-csp/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
**Real-World Analogy â€” Scheduling University Classes:** Imagine you are the registrar at a university. You have 50 classes to schedule across 10 time slots and 20 rooms. Each professor can only teach during certain hours, no two classes can share the same room at the same time, some courses must be taken in sequence (prerequisite constraints), and some students must be able to take both AI and Machine Learning without a time conflict. This is a classic Constraint Satisfaction Problem (CSP). The variables are (class, time, room) triples, the domains are all possible time-room combinations, and the constraints capture every real-world restriction. Without a CSP framework, you would resort to brute-force enumeration â€” evaluating 20Ã‚Â¹Ã¢ï¿½Â° possible room assignments alone. With CSP techniques (backtracking, forward checking, AC-3), you prune invalid branches early and find a feasible schedule in seconds instead of centuries. CSPs are the mathematical backbone of timetabling, Sudoku, register allocation in compilers, and even NASA's Deep Space Network antenna scheduling.
...
|--------|:--------------:|:----------------------:|
| **State representation** | Opaque â€” states are atomic black boxes | Declarative â€” state = assignment of values to variables |
| **Goal test** | Application-specific function | All constraints satisfied |
| **Path matters?** | Yes â€” the sequence of actions matters | No â€” only the final assignment matters |
| **Action set** | Varies per state | Fixed: assign value to unassigned variable |
| **Commutativity** | Not commutative | Commutative â€” order of assignments irrelevant |
| **Branching factor** | Often small | Large (domain size Ãƒâ€” number of unassigned vars) |
...
| **Global** | k variables (k Ã¢â€°Â¥ 2) | "All seven territories must have distinct colors" | $C_3: Alldifferent(WA, NT, SA, Q, NSW, V, T)$ |
| **Preference (soft)** | arbitrary | "Avoid scheduling Prof. Smith at 8AM" | $C_4: minimize\ Smith\_8AM\_slots$ |

- **Unary constraints** reduce a variable's domain directly (e.g., "X Ã¢â€°Â  red" removes red from D_X).
- **Binary constraints** relate variable pairs â€” the most common type, forming a constraint graph.
- **Global constraints** compactly represent complex interactions â€” $Alldifferent$ alone captures $\binom{k}{2}$ binary inequalities.
...
|-----------|---------|-------------|
| **Empty domain** | $D_{WA} = \{\}$ | No solution exists â€” CSP is unsatisfiable |
| **No constraints** | $m = 0$ | Every complete assignment is a solution; there are $|D|^n$ solutions |
| **Conflicting constraints** | $WA \neq red$ and $WA = red$ | CSP is overconstrained â€” inconsistency detected |
...
| **Redundant constraints** | $WA \neq NT$ and $NT \neq WA$ | Duplicate â€” harmless but wastes memory |

---

## 4.2 Backtracking Search

### Real-World Analogy â€” Solving a Crossword Puzzle
...
### Step-by-Step Dry Run â€” Australia Map Coloring (3 colors)
...
- **Space:** $O(n)$ â€” the depth of the recursion stack is at most $n$. This is linear space, which is a key advantage over BFS.
...
| Advantages | Disadvantages |
|------------|---------------|
| Complete â€” guaranteed to find solution if one exists | Worst-case exponential $O(d^n)$ time |
| Memory efficient â€” only $O(n)$ recursion stack | No guidance â€” tries values in arbitrary order |
...
| **Cyclic constraints** | Still handled â€” cycles increase backtracking | Any graph with cycles |
...
### Real-World Analogy â€” Placing Dominoes
...
4. **Check for empty domains:** If any affected variable now has an empty domain, the current assignment is a dead end â€” backtrack immediately.
...
### Step-by-Step Dry Run â€” Australia with Forward Checking
...
| 7 | T | G | All assigned | â€” | **Solution** |

If at step 4 we had tried Q=G (conflict with NT), Q=B (conflict with SA), then Q=R is the only choice â€” forward checking's domain reduction made the choice obvious.
...
- **Space:** $O(n d)$ â€” we save domain copies for each recursive level.
...
### Forward Checking vs Backtracking â€” Search Tree Nodes
...
| Simple to implement on top of backtracking | Requires domain restoration on backtrack â€” O(d) per undo |
...
| Intuitive â€” "what if" elimination | Fails to detect conflicts between non-neighbor variables |
...
| **All domains singleton** | No branching â€” deterministic | Every variable forced to one value |
...
### Real-World Analogy â€” Friends Planning a Dinner


Alice, Bob, and Charlie want to have dinner. Alice can come Monday or Tuesday. Bob can come Tuesday or Wednesday. Charlie can come Monday, Tuesday, or Wednesday. The constraint: Alice and Bob cannot come the same day. Before anyone commits, they think: "If Alice comes Monday, Bob cannot, so Bob would need Wednesday." Then: "If Bob comes Wednesday, is there still a day for Charlie?" Yes â€” Charlie can take Monday or Tuesday. Now reverse: "If Alice comes Tuesday, Bob cannot, so Bob would need Wednesday." They eliminate any option that leaves someone stranded. This bi-directional reasoning is arc consistency.
...
4. **Check for empty domain:** If $D_i$ becomes empty during revision, the CSP is unsatisfiable â€” return failure.
...
### Step-by-Step Dry Run â€” AC-3 on Australia Map (3 colors)


**Initial Domains:** WA:{R,G,B}, NT:{R,G,B}, SA:{R,G,B}, Q:{R,G,B}, NSW:{R,G,B}, V:{R,G,B}, T:{R,G,B}
**Arcs (all directed):** (WA,NT), (NT,WA), (WA,SA), (SA,WA), (NT,SA), (SA,NT), (NT,Q), (Q,NT), (SA,Q), (Q,SA), (SA,NSW), (NSW,SA), (SA,V), (V,SA), (Q,NSW), (NSW,Q), (NSW,V), (V,NSW), (V,T), (T,V) â€” 20 directed arcs.

| Step | Pop (X_i, X_j) | D_i before | Revise? | D_i after | Requeue (X_k, X_i) |
|:----:|:--------------:|:----------:|:-------:|:---------:|:------------------:|
| 1 | (WA, NT) | {R,G,B} | No | {R,G,B} | â€” |
| 2 | (NT, WA) | {R,G,B} | No | {R,G,B} | â€” |
| 3 | (NT, SA) | {R,G,B} | No | {R,G,B} | â€” |
| ... | (all non-restrictive) | ... | No | ... | â€” |
| 20 | (T, V) | {R,G,B} | No | {R,G,B} | â€” |
...
**Dry Run â€” After WA=R (demonstration):**
...
|:----:|:--------------:|:----------:|:-------:|:---------:|:-------:|
| 1 | (NT, SA) | {G,B} | No â€” each has partner | {G,B} | â€” |
| 2 | (SA, NT) | {G,B} | No | {G,B} | â€” |
| 3 | (NT, Q) | {G,B} | Yes â€” Q has R for G, but check | No removal | â€” |
| 4 | (Q, NT) | {R,G,B} | No â€” all values have partner | {R,G,B} | â€” |
| 5 | (SA, Q) | {G,B} | No | {G,B} | â€” |
| 6 | (Q, SA) | {R,G,B} | Yes â€” if Q=R, SA has partner G or B; Q=G, SA has B; Q=B, SA has G | {R,G,B} | â€” |
...
- **Why AC-3 is not $O(d^n)$:** Unlike search, AC-3 does not enumerate assignments â€” it removes provably impossible values from domains. Each value is removed at most once. The $d^3$ factor comes from iterating over pairs of values in the revise step, not from branching.
...
### Real-World Analogy â€” Emergency Room Triage


The ER doctor has 20 patients (variables) and limited resources (domains). **MRV (Minimum Remaining Values)** says: treat the patient with the fewest treatment options first â€” the one with a rare blood type or unique allergy â€” because if you delay them, they may become unsolvable. **LCV (Least Constraining Value)** says: when deciding which treatment to give a patient, choose the one that leaves the most resources for other patients â€” use the common blood type first, save the rare one.

### Minimum Remaining Values (MRV) Heuristic


Select the variable with the fewest legal values remaining in its domain. Also called "most constrained variable" or "fail-first" heuristic â€” it detects dead ends as early as possible.
...
**Why it works:** A variable with only 1 remaining value is forced â€” assign it immediately and either succeed or fail fast. Variables with large domains can wait.
...
**MRV vs LCV â€” Complementarity:**

| Aspect | MRV | LCV |
|--------|:---:|:---:|
| Applies to | Variable ordering | Value ordering |
| Goal | Fail fast â€” minimize subtree size | Succeed fast â€” find solution early |
...
| Tie-breaker | Degree heuristic | â€” |
...
|:----:|:----------:|:-------------------------------:|:---------:|:------:|
| 1 | {} | Q1:4, Q2:4, Q3:4, Q4:4 â€” all equal | Q1 (tie) | Degree tie, pick first |
| 2 | {Q1=1} | Q2:{3,4}(2), Q3:{2,4}(2), Q4:{2,3,4}(3) | Q2 or Q3 (2 each) | Q2 has min values |
| 3 | {Q1=1, Q2=3} | Q3:{2,4}(2), Q4:{}(0) | Q4 (0 â€” forced fail) | MRV detects dead end |
...
- **LCV:** $O(n d^2)$ per value ordering â€” evaluating each value against all neighbors and their domains. The overhead is worthwhile when the search tree is deep.
...
| **LCV** | Increases chance of early solution; pairs well with forward checking | Expensive to compute â€” must check all values against all neighbors |
...
| **MRV picks a variable with 1 value** | Forced assignment â€” effectively no branching |
| **LCV all values equally constraining** | Falls back to default ordering |
| **No unassigned neighbors** | LCV has no effect â€” all values score 0 |
...
- **Row constraint (binary):** $Q_i \neq Q_j$ for all $i \neq j$ â€” no two queens share a row.
- **Diagonal constraint (binary):** $|Q_i - Q_j| \neq |i - j|$ for all $i \neq j$ â€” no two queens share a diagonal.
...
- **Constraints (3 sets of Alldifferent):** Each row (9), each column (9), each 3x3 box (9) â€” 27 global $Alldifferent$ constraints total.
- **Solving:** AC-3 preprocesses heavily â€” after arc consistency, most cells have domains of 2-5 values. Backtracking with MRV then quickly finds the solution.

### Q4: AC-3 vs Backtracking â€” which is better?
...
| Forward Checking | Propagation | On assignment | Domain filtering | O(nÃ‚Â²dÃ‚Â²) |
| AC-3 | Propagation | Yes/Interleaved | Arc consistency | O(nÃ‚Â²dÃ‚Â³) |
| MAC | Propagation+Search | Interleaved | More pruning than FC | O(nÃ‚Â²dÃ‚Â³) per step |
...
| Tree Decomposition | âœ— | âœ“ | âœ— | âœ“ |
...
<details><summary>Answer&lt;/summary&gt;B) Tree-structured CSPs are solvable in O(n dÃ‚Â²) time â€” linear in the number of variables and quadratic in the domain size.</details>
...
<details><summary>Answer&lt;/summary&gt;C) O(nÃ‚Â² dÃ‚Â³) â€” nÃ‚Â² arcs, each revised at most d times, each revision costing O(dÃ‚Â²).</details>
...
- Backtracking = DFS with incremental constraint checking â€” complete but worst-case exponential
- Forward checking prunes neighbor domains after each assignment â€” detects dead ends early
- AC-3 enforces arc consistency globally in $O(n^2 d^3)$ â€” powerful preprocessing step
- MRV (fail-first) + LCV (succeed-first) drastically reduce search tree size
- Tree-structured CSPs solve in $O(n d^2)$ â€” no backtracking needed

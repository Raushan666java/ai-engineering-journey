...
> **Reading Time:** 4Ã¢â‚¬â€œ5 hours | **Level:** IntermediateÃ¢â‚¬â€œAdvanced
...
| Case Studies | Equifax, Marriott, TikTok, Wells Fargo â†’ root cause analysis | High |
...
**Real-world analogy:** A country's constitution defines the rights, responsibilities, and structure of government. Similarly, information security governance defines how an organization manages security â†’ who decides, who enforces, and how success is measured.
...
- CISO reports to **CIO** â†’ common in tech-focused orgs (conflict: CIO may prioritize uptime over security)
- CISO reports to **CEO** â†’ stronger independence, seen in mature orgs
- CISO reports to **Chief Risk Officer** â†’ risk-aligned, common in financial services
- CISO reports to **Board/Audit Committee** â†’ rare, highest independence
...
| **Procedure** | Step-by-step instructions | "Password reset process: Step 1 â†’ verify identity via ticketing system" | Mandatory |
...
1. **Identify need** â†’ regulatory requirement, risk finding, audit gap
2. **Draft** â†’ policy owner writes (typically GRC team or CISO office)
3. **Review** â†’ legal, HR, business stakeholders review for conflicts
4. **Approve** â†’ executive sign-off (CISO, CEO, or Board depending on tier)
5. **Communicate** â†’ All-hands email, intranet, signed acknowledgment
6. **Train** â†’ Role-based training on policy requirements
7. **Enforce** â†’ Technical controls (e.g., GPO, CASB) and administrative oversight
8. **Monitor** â†’ Compliance metrics, violation tracking
9. **Review** â†’ Annual review cycle (or when regulations change)
10. **Retire** â†’ Archive superseded policies
...
| Avoidance | Don't buy expensive jewelry â†’ keep nothing worth stealing |
| Acceptance | "My lawnmower is old â†’ if someone steals it, I'll buy a new one" |
| Policy | "All family members must lock the door when leaving" |
| Compliance | City ordinance requiring smoke detectors â†’ verified annually by fire inspection |
...
+------------------------------------------------------+
| Step 1:  PREPARE â†’ Establish context, risk appetite  |
| Step 2:  CATEGORIZE â†’ Classify systems by impact     |
| Step 3:  SELECT â†’ Choose baseline controls (NIST SP  |
|            800-53)                                    |
| Step 4:  IMPLEMENT â†’ Deploy selected controls        |
| Step 5:  ASSESS â†’ Evaluate control effectiveness     |
| Step 6:  AUTHORIZE â†’ Risk-based decision to operate  |
| Step 7:  MONITOR â†’ Continuous monitoring & reporting |
+------------------------------------------------------+
```
...
- **1Ã¢â‚¬â€œ3:** Low â†’ Accept, monitor annually
- **4Ã¢â‚¬â€œ6:** Medium â†’ Accept with monitoring, review quarterly
- **8Ã¢â‚¬â€œ12:** High â†’ Mitigation plan required, review monthly
- **15Ã¢â‚¬â€œ25:** Critical â†’ Immediate remediation, escalate to Board

**Qualitative assessment walkthrough â†’ Dry Run:**

Scenario: Acme Corp has an unpatched vulnerability (CVE-2025-1234) in a public-facing web application.

Step 1: Identify the risk â†’ Remote code execution in web server (CVSS 9.8).
Step 2: Determine likelihood â†’ Exploit code publicly available. Internet-facing system. No WAF. -> Likely (4)
Step 3: Determine impact â†’ Breach of customer database (PII). Regulatory fine (GDPR, up to EUR 20M). Average breach cost: $4.5M. -> Severe (5)
Step 4: Calculate risk score â†’ 4 x 5 = 20 (Critical)
Step 5: Determine risk response â†’ Mitigate: Apply vendor patch within 24 hours. If patch unavailable, deploy WAF virtual patch and restrict source IPs.
Step 6: Residual risk â†’ After patching, likelihood drops to Rare (1). Residual score: 1 x 5 = 5 (Low) -> Accept.
...
**Quantitative example â†’ Dry Run:**
...
ROSI of 340% means every $1 spent on controls saves $3.40 â†’ a strong business case.
...
| **Precision** | Low â†’ ordinal only | High â†’ continuous values |
| **Cost** | Low | High |
...
**Hybrid approach:** Most mature programs use both â†’ qualitative for initial triage and emerging risks, quantitative for critical risks that need budget approval.
...
| RISK-003 | 2025-02-01 | Single point of failure â†’ DC fire | Fire | No redundant data center | Primary DC | 2 | 5 | 10 (High) | Cold DR site in different AWS region | 1 | 4 | 4 (Low) | Mitigate | Provision DR failover; test quarterly | IT Ops | 2025-06-01 | In Progress |
...
| RISK-006 | 2025-03-15 | Vendor data breach â†’ payroll processor | Attacker targeting vendor | Insufficient vendor assessment | Employee PII | 3 | 5 | 15 (High) | Vendor assessment complete; contract includes security addendum | 2 | 4 | 8 (Medium) | Transfer | Cyber insurance covers vendor-caused breaches | Procurement | 2025-06-30 | Open |
...
**FAIR analysis steps:**
1. **Scope the scenario** â†’ Define the asset, threat community, and effect
2. **Estimate Loss Event Frequency (LEF)** â†’ How often does it happen?
   - Threat Event Frequency (TEF)
   - Vulnerability (how easily is the threat realized?)
3. **Estimate Loss Magnitude (LM)** â†’ How much does it cost?
   - Primary losses (direct: response, replacement, fines)
   - Secondary losses (indirect: reputational, legal, competitive disadvantage)
4. **Compute risk** â†’ LEF x LM = annualized risk in dollars
5. **Uncertainty analysis** â†’ Monte Carlo simulation for confidence intervals

**FAIR example â†’ Ransomware on File Server:**

| Input | Low | Most Likely | High | Distribution |
|-------|-----|-------------|------|-------------|
| TEF (per year) | 0.1 | 0.3 | 0.8 | Triangular |
| Vulnerability | 0.2 | 0.5 | 0.9 | Triangular |
| LEF (TEF x Vuln) | 0.02 | 0.15 | 0.72 | â†’ |
| Primary loss | $100K | $500K | $1.5M | Triangular |
| Secondary loss | $50K | $200K | $1M | Triangular |
| **Annualized risk** | **$3K** | **$105K** | **$1.8M** | â†’ |

*Monte Carlo simulation would run 10,000+ iterations to produce a probability distribution, giving the 5thÃ¢â‚¬â€œ95th percentile range.*
...
**BIA Example â†’ Acme Corp (3 systems):**
...
1. **Identify business processes** â†’ Interview department heads, review org chart
2. **Determine criticality** â†’ Rank processes by revenue, regulatory, customer impact, reputation
3. **Estimate impact over time** â†’ What happens at 1hr, 4hrs, 24hrs, 72hrs, 7 days
4. **Define recovery requirements** â†’ RTO, RPO, minimum staff, minimum tech
5. **Identify dependencies** â†’ Upstream (data inputs) and downstream (processes that depend on this)
6. **Document resources** â†’ People, systems, data, facilities, third parties
7. **Get approval** â†’ Business process owners sign off on RTO/RPO targets
...
- **Shadow IT:** Critical processes running on unapproved SaaS â†’ invisible to BIA teams.
...
- **Outsourced processes:** Vendor's BIA may not match your BIA â†’ contractual RTO alignment needed.
...
### 3.3 GDPR â†’ General Data Protection Regulation
...
#### Data Subject Rights (GDPR Articles 12Ã¢â‚¬â€œ23)
...
Step 6: DELIVER
  ...
  - No fee (unless manifestly unfounded or excessive â†’ may charge "reasonable fee")
...
**DSAR Edge Cases:**
- **Deceased persons:** GDPR does not apply to deceased data â†’ check local laws
- **Excessive requests:** 50+ copycat requests from the same individual in a month â†’ may refuse or charge
- **Criminal data:** Subject to special processing conditions â†’ review strictly
- **Employee vs customer:** Employee DSARs may include disciplinary records, performance reviews â†’ coordinate with HR
- **Third-party data:** If one person's data reveals another person's data (e.g., CC'd on email) â†’ redact third party
...
| **Record keeping** | All breaches must be documented (even those not notified) â†’ Article 33(5) |

#### GDPR Fines (Tier Structure)

**Tier 1:** Up to EUR 10M or 2% global annual turnover â†’ violations of Articles 8, 11, 25Ã¢â‚¬â€œ39, 41Ã¢â‚¬â€œ43
**Tier 2:** Up to EUR 20M or 4% global annual turnover â†’ violations of Articles 5Ã¢â‚¬â€œ7, 9, 12Ã¢â‚¬â€œ22, 44Ã¢â‚¬â€œ49

### 3.4 PCI DSS 4.0 â†’ Payment Card Industry Data Security Standard
...
| 2 | 1MÃ¢â‚¬â€œ6M transactions/year | Annual QSA assessment + quarterly ASV scans |
| 3 | 20KÃ¢â‚¬â€œ1M e-commerce transactions/year | Annual SAQ + quarterly ASV scans |
| 4 | Less than 20K e-commerce or up to 1M total | Annual SAQ + quarterly ASV scans |

### 3.5 HIPAA â†’ Health Insurance Portability and Accountability Act
...
### 3.6 SOC 2 â†’ Service Organization Control 2
...
- **Type II:** Controls are suitably designed AND operating effectively over a period (typically 6Ã¢â‚¬â€œ12 months)

**Common Criteria (CC) â†’ 9 categories of control criteria:**
1. CC1 â†’ Control Environment
2. CC2 â†’ Communication and Information
3. CC3 â†’ Risk Assessment
4. CC4 â†’ Monitoring Activities
5. CC5 â†’ Control Activities
6. CC6 â†’ Logical and Physical Access
7. CC7 â†’ System Operations
8. CC8 â†’ Change Management
9. CC9 â†’ Risk Mitigation

### 3.7 FedRAMP â†’ Federal Risk and Authorization Management Program
...
### 3.8 SOX â†’ Sarbanes-Oxley Act


**Jurisdiction:** US public companies (and their auditors) | **Year:** 2002

**Relevant Sections for IT:**
- **Section 302:** Corporate responsibility for financial reports â†’ CEO/CFO certify financial statements
- **Section 404:** Management assessment of internal controls â†’ IT controls over financial reporting (ITGC)
- **Section 409:** Real-time issuer disclosures â†’ timely reporting of material changes
- **Section 802:** Criminal penalties for altering documents â†’ record retention requirements
...
### 3.9 Breach Notification Laws â†’ International Comparison
...
1. **Identify data** â†’ Take inventory of all data stores (databases, file shares, SaaS, email, backups)
2. **Define levels** â†’ Establish classification schema (3Ã¢â‚¬â€œ5 levels)
3. **Classify** â†’ Tag data with classification level (manual, automated via DLP, or hybrid)
4. **Apply controls** â†’ Map classification to security controls (encryption, access, retention, disposal)
5. **Train** â†’ Employees must understand classification and handling requirements
6. **Monitor** â†’ DLP alerts on mishandling (e.g., emailing confidential data externally)
7. **Audit** â†’ Verify controls match classification level annually

### 3.11 Data Privacy (Beyond GDPR)


**Privacy by Design (PbD) â†’ 7 Foundational Principles (Ann Cavoukian):**

1. **Proactive not reactive; preventative not remedial** â†’ Build privacy into design before breach
2. **Privacy as default** â†’ Maximum privacy automatically, no action needed by user
3. **Privacy embedded into design** â†’ Privacy is core component, not bolt-on
4. **Full functionality** â†’ Positive-sum, not zero-sum (privacy + security + usability)
5. **End-to-end security** â†’ Full lifecycle protection (collection through deletion)
6. **Visibility and transparency** â†’ Open, documented processes
7. **Respect for user privacy** â†’ User-centric: granular controls, notifications

**DPIA (Data Protection Impact Assessment) â†’ Required when processing is likely to result in high risk:**
...
**Real-world analogy:** A building code defines how to construct a safe building â†’ foundation, frame, electrical, plumbing. NIST CSF defines how to construct a cybersecurity program â†’ regardless of the specific materials (technology) you use.

#### NIST CSF 2.0 â†’ 6 Functions (added "Govern")

```
+----------------------------------------------------------+
|   NIST CSF 2.0 â†’ 6 Functions, 22 Categories, 106 Subcats |
+----------------------------------------------------------+
| GV â†’ Govern         (New in 2.0)                          |
...
#### NIST CSF 2.0 â†’ Function-to-Control Mapping with Implementation Example

**Function: GV (Govern)**

| Category | Subcategory (ID) | Control Example | Implementation |
|----------|-----------------|-----------------|----------------|
| GV.OC â†’ Organizational Context | GV.OC-01: Org mission and stakeholder expectations | Define mission objectives for cybersecurity | CISO presents to Board; document in charter |
| GV.RM â†’ Risk Management Strategy | GV.RM-01: Risk appetite and tolerance | Establish risk appetite statement | Board approves risk appetite: "No residual risk above Medium for systems with PII" |
| GV.RR â†’ Roles and Responsibilities | GV.RR-01: Cybersecurity roles defined | Create RACI matrix for incident response | Document: CISO owns response, SOC Manager owns triage, Legal owns notification |
| GV.PO â†’ Policies and Processes | GV.PO-01: Cybersecurity policy established | Write and approve InfoSec policy | "InfoSec Policy v4.2" approved by CEO, published on intranet |
| GV.SC â†’ Supply Chain | GV.SC-03: Third-party risk integrated into planning | Vendor risk assessment program | All vendors handling PII must complete SIG questionnaire |

**Function: ID (Identify)**

| Category | Subcategory | Control Example | Implementation |
|----------|-------------|-----------------|----------------|
| ID.AM â†’ Asset Management | ID.AM-01: Hardware inventory | Maintain CMDB with automated discovery | Deploy Device42 or ServiceNow CMDB; weekly scans |
| ID.AM â†’ Asset Management | ID.AM-03: Software inventory | Application inventory with versions | Deploy Flexera or Wiz; monthly reports |
| ID.RA â†’ Risk Assessment | ID.RA-01: Vulnerabilities identified | Quarterly vulnerability scanning | Deploy Qualys; scan all IPs; prioritize CVSS 7+ within 30 days |
| ID.RA â†’ Risk Assessment | ID.RA-04: Business impact criteria defined | BIA completed for all critical processes | Interview 20 business owners; document RTO/RPO for 50 processes |
| ID.IM â†’ Improvements | ID.IM-01: Risk management plan updates | Annual risk assessment cycle | Update risk register, review treatment plans |

**Function: PR (Protect)**

| Category | Subcategory | Control Example | Implementation |
|----------|-------------|-----------------|----------------|
| PR.AA â†’ Identity Management & Access Control | PR.AA-01: Identities and credentials for authorized users | MFA for all users | Deploy Azure AD Conditional Access â†’ enforce MFA for all external access |
| PR.AT â†’ Awareness and Training | PR.AT-01: Personnel trained | Annual security awareness training | Deploy KnowBe4; 95% completion target; monthly phishing simulations |
| PR.DS â†’ Data Security | PR.DS-01: Data-at-rest protected | Encrypt all databases | AWS KMS for RDS encryption; enable S3 SSE-S3/S3-KMS |
| PR.DS â†’ Data Security | PR.DS-02: Data-in-transit protected | TLS for all external communications | Enforce TLS 1.2+ via CloudFront; disable SSL 3.0 and TLS 1.0 |
| PR.PS â†’ Platform Security | PR.PS-01: Configuration management | CIS benchmark compliance | Deploy CIS-CAT or Tanium; score >85% on critical systems |
| PR.PS â†’ Platform Security | PR.PS-02: Software integrity verified | Code signing and integrity checks | Sign all internal binaries; deploy SCAP scanning |
| PR.IR â†’ Technology Infrastructure Resilience | PR.IR-01: Backups managed | Immutable backups with tested restore | Veeam + AWS S3 Object Lock; test restore monthly |
| PR.IR â†’ Technology Infrastructure Resilience | PR.IR-04: Network availability | Redundant network paths | BGP multi-homing; load balancers in two AZs |

**Function: DE (Detect)**

| Category | Subcategory | Control Example | Implementation |
|----------|-------------|-----------------|----------------|
| DE.CM â†’ Continuous Monitoring | DE.CM-01: Network monitored | SIEM collecting all logs | Deploy Splunk / Microsoft Sentinel; ingest firewall, EDR, DNS logs |
| DE.CM â†’ Continuous Monitoring | DE.CM-04: Malicious code detected | EDR on all endpoints | Deploy CrowdStrike Falcon or Microsoft Defender for Endpoint on 100% of endpoints |
| DE.CM â†’ Continuous Monitoring | DE.CM-07: Monitoring for unauthorized personnel | Physical access monitoring | Badge reader logs integrated with SIEM; alert on after-hours access |
| DE.AE â†’ Anomalies and Events | DE.AE-02: Event correlation | Correlation rules in SIEM | Alert on: brute force (10 failures in 5 min), impossible travel, new service account |
| DE.AE â†’ Anomalies and Events | DE.AE-04: Impact assessed | Triage SLA defined | Critical: 15 min; High: 1 hour; Medium: 4 hours; Low: 24 hours |

**Function: RS (Respond)**

| Category | Subcategory | Control Example | Implementation |
|----------|-------------|-----------------|----------------|
| RS.MA â†’ Incident Management | RS.MA-01: Response plan executed | IR plan documented and tested | Tabletop exercise every 6 months; update plan annually |
| RS.AN â†’ Incident Analysis | RS.AN-01: Investigation performed | Forensic analysis capability | Deploy FTK or Velociraptor; train 3 forensic analysts |
| RS.MI â†’ Mitigation | RS.MI-01: Incidents contained | Automated containment playbook | SIEM triggers CrowdStrike RTR to isolate host on ransomware detection |
| RS.CO â†’ Communications | RS.CO-02: Internal stakeholders notified | Escalation matrix documented | Notify CISO &lt;15 min, Legal <30 min, CEO <60 min, Board <4 hours |

**Function: RC (Recover)**

| Category | Subcategory | Control Example | Implementation |
|----------|-------------|-----------------|----------------|
| RC.RP â†’ Recovery Plan | RC.RP-01: Recovery plan executed | DR plan tested annually | Full infrastructure failover test to DR region; report outcomes to Board |
| RC.IM â†’ Improvements | RC.IM-01: Lessons learned | Post-incident review within 30 days | Document root cause, corrective actions, update playbooks |
| RC.CO â†’ Communications | RC.CO-02: External stakeholders updated | Breach notification procedures | Legal handles DPA notification within 72h (GDPR); PR handles press release |
...
### 4.2 ISO/IEC 27001 â†’ Information Security Management System (ISMS)
...
#### ISO 27001:2022 â†’ Annex A Control Mapping (93 Controls, 4 Domains)
...
### 4.3 COBIT â†’ Control Objectives for Information and Related Technologies
...
**Governance Domain (Evaluate, Direct, Monitor â†’ EDM):**
- EDM01 â†’ Ensure governance framework setting and maintenance
- EDM02 â†’ Ensure benefits delivery
- EDM03 â†’ Ensure risk optimization
- EDM04 â†’ Ensure resource optimization
- EDM05 â†’ Ensure stakeholder transparency

**Management Domains (4 domains):**

| Domain | Focus | # Objectives | Examples |
|--------|-------|-------------|---------|
| **APO** â†’ Align, Plan and Organize | Strategic planning | 14 | APO01 IT management framework, APO12 Risk management, APO13 Security management |
| **BAI** â†’ Build, Acquire and Implement | Solution delivery | 11 | BAI01 Programs/projects, BAI06 IT change management, BAI09 Asset management |
| **DSS** â†’ Deliver, Service and Support | Service delivery | 6 | DSS01 Operations, DSS02 Service requests/incidents, DSS03 Problem mgmt, DSS05 Security services |
| **MEA** â†’ Monitor, Evaluate and Assess | Performance monitoring | 3 | MEA01 Performance monitoring, MEA02 Internal control, MEA03 Regulatory compliance |
...
|-------|-------------|-------------|---------------------|
| **IG1** | Basic cyber hygiene â†’ minimum standard for all orgs | 56 | Small business, limited security resources |
| **IG2** | Risk-informed â†’ additional protections for sensitive data | 128 | Mid-size enterprise, some security staffing |
| **IG3** | Proactive â†’ advanced defenses for critical infrastructure | 168 | Large enterprise, mature security program |
...
| **Certification** | No certification (self-assessment) | Yes â†’ third-party certification | No certification | QSA assessment for Level 1; SAQ for others |
| **Risk-based?** | Yes â†’ tiered approach | Yes â†’ risk assessment is mandatory | Partially â†’ implementation groups | Partially â†’ compensating controls allowed |
...
| **Flexibility** | Very high â†’ any controls can map | Moderate â†’ SoA documents chosen controls | Moderate â†’ IG1/2/3 tiers | Low â†’ specific technical requirements |
| **Maturity model** | 4 Tiers | Not inherent (but can combine with ISO 330xx) | IG1/2/3 | Not inherent (SAQ A-D) |
| **Supply chain** | New GV.SC category | 5.19Ã¢â‚¬â€œ5.23 supplier controls | Control 15 (Service Provider Management) | Requires TPSP program for service providers |
...
| **Cost to implement** | $50KÃ¢â‚¬â€œ$200K | $100KÃ¢â‚¬â€œ$500K + certification audit | $30KÃ¢â‚¬â€œ$150K | $50KÃ¢â‚¬â€œ$300K + QSA fees |
...
### 4.6 Framework Selection â†’ Decision Tree
...
| **Risk requirement** | Implied across functions (ID.RA, GV.RM) | Explicit clause 6.1 â†’ risk assessment mandatory |
| **Documentation requirements** | Minimal (self-assessment) | Extensive: policy, SoA, risk treatment plan, procedures, records |
| **Leadership involvement** | Implied in Govern function | Explicit Clause 5 â†’ top management must demonstrate commitment |
| **Continuous improvement** | ID.IM, RC.IM subcategories | Clause 10 â†’ nonconformity and corrective action |
...
### 5.1 BCP vs DR â†’ Comparison


Real-world analogy: **BCP** is the escape plan you keep in your nightstand â†’ where to meet, what to grab, who to call if fire breaks out. **DR** is what happens after the fire trucks leave â†’ drying out photos, filing insurance, rebuilding rooms.
...
1. **Plan Overview** â†’ Purpose, scope, assumptions, RTO/RPO summary
2. **Roles and Responsibilities** â†’ DR team org chart, call tree, escalation contacts
3. **Incident Response** â†’ Detection, declaration criteria, activation procedures
4. **Backup and Recovery Procedures** â†’ System-specific recovery steps, backup schedules
5. **Data and System Prioritization** â†’ BIA summary, recovery priority matrix
6. **Communication Plan** â†’ Internal notification, customer notification, regulatory notification, PR
7. **Testing Schedule** â†’ Frequency, scope, success criteria
8. **Plan Maintenance** â†’ Review cycle, version control, distribution
...
| **General** | Does vendor store, process, or transmit your data? Y/N â†’ describe data types | | |
...
|-------------|-----------|--------|
| 0Ã¢â‚¬â€œ10 | Low | Accept, monitor annually |
| 11Ã¢â‚¬â€œ25 | Medium | Remediation plan required within 90 days |
| 26Ã¢â‚¬â€œ40 | High | Approve only with compensating controls; re-assess q90 |
...
Humans are the #1 attack vector â†’ 74% of breaches involve the human element (Verizon DBIR 2024). Security awareness training reduces phishing click rates from 25% to &lt;5% on average.
...
**Month 1 â†’ Phishing & Social Engineering**
...
**Month 2 â†’ Password Security & MFA**
...
**Month 3 â†’ Data Protection & Privacy**
...
**Month 4 â†’ Incident Reporting & Physical Security**
...
**Analysis:**
- Click rate declined 84% from Q1 (11%) to Q4 (1.8%) â†’ training effectiveness demonstrated
- Reporting rate improved 114% â†’ culture of caution established
- Repeat clickers (clicked >=3 campaigns): 12 employees (0.48%) â†’ retrain individually
...
| **Scope** | Wider â†’ all controls, departments | Narrower â†’ defined by standard or regulation |
...
| **Cost** | Lower (internal resources) | Higher ($30KÃ¢â‚¬â€œ$150K per engagement) |
...
|---|-----------------|----------|--------|---------|
| 1 | Is there an access control policy approved by management? | Access Control Policy v3.1 signed by CISO | Pass | â†’ |
| 2 | Are user accounts created through a formal request/approval process? | 20 ticket samples (hit 1/20 â†’ missing manager approval) | Partial Fail | Missing manager approval on 5% of provisioned accounts |
| 3 | Are accounts disabled within 24h of employee termination? | 10 terminated employee samples (all disabled within 12h) | Pass | â†’ |
| 4 | Is MFA enforced for all administrative accounts? | Azure AD Conditional Access policy | Pass | â†’ |
| 5 | Are access rights reviewed quarterly? | Q1/Q2 access review evidence (signed by business owners) | Pass | â†’ |
| 6 | Is privilege access managed with PAM tool? | Deployed CyberArk; check in/check out auditing enabled | Pass | â†’ |
...
| 8 | Are passwords compliant with policy (14+ chars, complexity)? | Password policy in GPO; last audit showed 98% compliance | Pass | â†’ |
| 9 | Is segregation of duties enforced in financial systems? | SAP roles reviewed â†’ conflicting roles identified in 2/50 users | Fail | 2 users have both AP creation and AP approval roles |
| 10 | Are external/contractor accounts reviewed separately? | Contractor review report â†’ 2 expired contractors still active | Critical Fail | 2 terminated contractors still have active network access |
...
**Good metrics are:**
- **S**pecific â†’ clearly defined, unambiguous
- **M**easurable â†’ quantifiable, objective data source
- **A**ctionable â†’ can drive decision or improvement
- **R**elevant â†’ tied to business objectives
- **T**imely â†’ measured at appropriate frequency
...
### Case Study 1: Equifax (2017) â†’ GRC Failure on Every Front
...
- **March 8Ã¢â‚¬â€œMay 13:** Equifax failed to scan for vulnerable Struts instances (no asset inventory)
- **May 13:** Attackers scan internet, find Equifax's vulnerable system â†’ exploit and establish foothold
- **May 13Ã¢â‚¬â€œJuly 29:** Attackers move laterally, exfiltrate 147M records over 76 days â†’ no detection
- **July 29:** Expired TLS certificate causes monitoring failure â†’ security team finally detects the breach
- **September 7:** Public disclosure â†’ stock drops 35%
...
| **Incident Response** | 76 days from initial compromise to containment â†’ no IR playbook for data exfiltration | Inadequate IR plan, no data exfiltration detection, no segmented network to limit lateral movement |
| **Compliance** | GDPR (effective May 2018) would have fined Equifax up to 4% global revenue â†’ not yet enforceable but indicative of gap | No data classification, no breach notification plan, no DPO |
...
Equifax had **policies** but no **enforcement.** The infosec policy required patching within 48 hours for critical vulnerabilities â†’ but no one verified compliance. The board approved security budgets â†’ but no one measured effectiveness. This is the classic GRC failure: paper compliance without operational verification.

**Lessons Learned:**
1. Asset inventory is the foundation of all security â†’ if you don't know you have it, you can't protect it
...
### Case Study 2: Marriott/Starwood (2018) â†’ GDPR Article 32 Failure


**Background:** Marriott International acquired Starwood hotels in 2016. Starwood's reservation database had been compromised since 2014. After acquisition integration, Marriott discovered the breach in 2018 â†’ 339 million guest records exposed.

**Timeline:**
- **2014:** Attackers compromise Starwood reservation system (via RDP brute force)
- **2014Ã¢â‚¬â€œ2016:** Attackers maintain persistence, escalate privileges, install keyloggers on payment systems, exfiltrate records over 2+ years
- **2016:** Marriott acquires Starwood for $13.6B â†’ accelerates IT integration
- **2018 (Sept):** Marriott security detects anomalous database queries in Starwood systems
- **2018 (Nov):** Marriott publicly discloses breach â†’ 339M records (133M with structured PII including passport numbers)
...
| GRC Domain | Failure |
|------------|---------|
| **Data Classification** | Starwood's database was not classified correctly during M&A â†’ 339M records with passport numbers labeled as "Internal" not "Restricted" |
| **M&A Due Diligence** | Insufficient security assessment of Starwood during acquisition â†’ vulnerability scan and pen test should have revealed the compromise |
| **Access Control** | Attackers had admin-level access on Starwood's network for 4 years â†’ no anomaly detection |
| **Detection** | Database queries extracting millions of records went undetected for 4 years â†’ no DLP, no behavioral analytics |
| **Encryption** | Some records were encrypted, but not all â†’ inconsistent encryption across merged entities |
| **Incident Response** | Took 2 months from detection to containment â†’ slow investigation, no automated response |
| **GDPR Compliance** | Article 32 (security of processing) violation â†’ insufficient technical and organizational measures |

**GDPR Fine Breakdown:**
- Original proposed: 99.2 million pounds (1.5% of 2018 global revenue)
- Final: 18.4 million pounds (0.28% of revenue) â†’ reduced on appeal considering:
...
5. GDPR requires "appropriate technical and organizational measures" â†’ not just having measures, but having EFFECTIVE ones

---

### Case Study 3: TikTok (2023) â†’ GDPR Children's Data Violations
...
|-----------|-------------|---------|----------------|
| Children's privacy settings | Art 5(1)(c) â†’ Data minimization | TikTok's platform settings for 13Ã¢â‚¬â€œ17 year olds were not private by default. Public profiles, public videos, public comments were default | EUR 120M |
| Age verification | Art 5(1)(f) â†’ Integrity and confidentiality | "Family Pairing" mode allowed adults to bypass teen privacy protections | EUR 80M |
| Dark patterns | Art 5(1)(a) â†’ Lawfulness, fairness, transparency | Push notifications during "bedtime hours" (10PMÃ¢â‚¬â€œ8AM) for 13Ã¢â‚¬â€œ17 year olds incentivized addictive use | EUR 100M |
| Transparency of processing | Art 12 â†’ Transparent information | Privacy information presented to children was not designed in a child-friendly manner | EUR 45M |

**Total Fine:** EUR 345M (2nd largest GDPR fine at the time, after Meta's EUR 1.2B)

**Root Cause (GRC Perspective):**
TikTok's **Data Protection Impact Assessment (DPIA)** was inadequate â†’ it identified risks to children but failed to implement corresponding controls. The governance structure prioritized engagement metrics (time on platform, daily active users) over data protection. The DPO was not sufficiently empowered to enforce changes.
...
3. Age verification is not optional â†’ the burden is on the controller to verify age
...
### Case Study 4: Wells Fargo (2016Ã¢â‚¬â€œ2020) â†’ Governance Failure
...
| GRC Domain | Failure |
|------------|---------|
| **Governance â†’ Tone at the Top** | CEO and Board set "Eight is Great" cross-selling strategy (8 products per customer) without operational risk oversight. Bonuses tied to account creation, not ethical behavior |
...
| **Internal Audit** | Internal audit identified fraudulent account creation in 2013 but did not escalate to Board or Audit Committee â†’ findings were buried |
| **Whistleblower Protection** | Over 5,000 employees fired for reporting fraudulent account creation â†’ no whistleblower protection mechanism worked |
...
- Federal Reserve imposed unprecedented asset cap ($1.95T) â†’ restricted growth until 2021
- Wells Fargo lost its "too big to fail" exemption

**Root Cause (GRC Perspective):**
The governance structure at Wells Fargo allowed **sales incentives to override all risk and compliance controls.** The Board-approved risk appetite statement was aspirational, not operational â†’ risk limits were never enforced because business leaders controlled risk decisions.

**Lessons Learned:**
1. **Tone at the top is not enough** â†’ must be reinforced by incentives, controls, and enforcement
2. Risk and compliance functions must be **independent** â†’ cannot report to the business they oversee
3. Whistleblower protection must be **structural** â†’ independent hotline, guaranteed anonymity, anti-retaliation enforcement
4. Board composition must include **relevant expertise** â†’ Audit Committee members should understand financial controls
5. Risk appetite is meaningless without **enforcement mechanisms** â†’ if you say "no cross-selling violations" but never enforce it, it's not a risk appetite, it's a slogan
6. **Internal audit escalation** must go to the Board, not management â†’ findings buried by management are audit failures
...
| **Right to deletion** | Art 17 â†’ "Right to be forgotten" | Yes â†’ businesses must delete on request | Art 18 â†’ similar to GDPR | Limited |
| **Data portability** | Art 20 â†’ yes | Yes (CPRA) | Art 18 â†’ yes | No |
...
**A:** NIST CSF 2.0 has six core functions: **Govern** (new in 2.0 â†’ org context, risk management strategy, roles, policies), **Identify** (asset management, risk assessment, improvement), **Protect** (identity management, awareness, data security, platform security, resilience), **Detect** (continuous monitoring, anomaly detection, event analysis), **Respond** (incident management, analysis, mitigation, communications), and **Recover** (recovery planning, improvements, communications). These functions are performed in parallel, not sequentially â†’ an organization should always be identifying, protecting, and detecting simultaneously.

### Q4: What is the difference between qualitative and quantitative risk assessment?


**A:** **Qualitative** uses descriptive scales (Low, Medium, High) for likelihood and impact â†’ fast, subjective, good for initial triage. **Quantitative** uses monetary values (ALE = SLE x ARO) and statistical models â†’ data-driven, precise, but slow and resource-intensive. Most mature organizations use a hybrid: qualitative for most risks, quantitative for critical risks that require budget decisions or insurance. Example: Qualitative says "High risk," but the CFO asks "How much?" â†’ that's when you need quantitative.

### Q5: What is a risk register and what should it contain?


**A:** A risk register is the central repository of all identified risks. Minimum fields: Risk ID, date identified, risk description (threat + vulnerability + asset), likelihood, impact, inherent risk score, controls, residual likelihood/impact/score, risk response (mitigate/accept/transfer/avoid), treatment plan, owner, review date, status. The risk register should be a living document â†’ reviewed quarterly and updated when new risks emerge or controls change.
...
**A:** An effective program has five pillars: (1) **Ongoing engagement** â†’ monthly (not annual) training modules, quarterly phishing simulations, regular security newsletters. (2) **Role-based content** â†’ generic training for all, specialized training for developers (secure coding), executives (whaling), HR (data handling). (3) **Measurable outcomes** â†’ phishing click rate (<5%), training completion (100%), reporting rate (>50%), time to report (<5 min). (4) **Positive reinforcement** â†’ reward reporting real phishing emails, gamify completion rates, recognize "security champions" in each department. (5) **Accountability** â†’ repeat clickers get retraining, 3+ clicks escalate to manager, persistent non-compliance has consequences.
...
|--------|----------------|-------------------|
| Network Security | PCI DSS requirement 1 â†’ firewall segmentation | Segment CDE from corporate network; quarterly firewall rule review |
...
| IAM | SOX ITGC â†’ access recertification | Quarterly user access review of all finance-related systems |
...
| Pentesting | PCI DSS requirement 11.4 â†’ penetration testing | Annual application and network pen test by QSA-approved firm |
| Mobile Security | HIPAA mobile device policy | Enroll all mobile devices in MDM; remote wipe capability |
| IoT/OT | NIST CSF â†’ ICS-specific risk assessment | Purdue model segmentation; OT-specific vulnerability scanning |
| AI/ML | EU AI Act (2025) / GDPR Article 22 â†’ automated decision-making | Algorithmic impact assessment; human-in-the-loop for high-risk AI decisions |

---

## Summary

- **Governance** defines the rules, roles, and oversight structure for information security â†’ starting from the Board and flowing through policies, standards, procedures, and baselines.
...
- **Compliance** ensures legal and regulatory obligations are met â†’ GDPR, PCI DSS, HIPAA, SOC 2, FedRAMP, SOX â†’ each with specific requirements, timelines, and penalties.
...
- **Security awareness** addresses the human element â†’ the most common attack vector â†’ through training, simulations, and behavioral measurement.
- **Audits** validate that controls are designed and operating effectively â†’ internal audits for readiness, external audits for certification.
- **Metrics** connect GRC to business outcomes â†’ KPIs measure performance, KRIs predict risk, and board dashboards communicate status.
- **Case studies** demonstrate that GRC failure has real consequences â†’ Equifax ($1.4B), Marriott (EUR 18.4M), TikTok (EUR 345M), Wells Fargo ($3B) â†’ all caused by governance and risk management breakdowns, not technology failures.

---

## References

1. NIST Cybersecurity Framework 2.0 (February 2024) â†’ https://www.nist.gov/cyberframework
2. ISO/IEC 27001:2022 Information Security Management Systems
3. PCI DSS v4.0 (March 2022) â†’ https://www.pcisecuritystandards.org/
4. GDPR (Regulation (EU) 2016/679) â†’ https://gdpr-info.eu/
5. HIPAA Security Rule (45 CFR Parts 160, 162, and 164)
6. NIST SP 800-53 Rev. 5 â†’ Security and Privacy Controls
7. NIST SP 800-34 Rev. 1 â†’ Contingency Planning Guide
8. FAIR Institute â†’ FAIR-TM Quantitative Risk Analysis
9. CIS Controls v8 â†’ https://www.cisecurity.org/controls
10. COBIT 2019 â†’ ISACA
11. AICPA SOC 2 Trust Services Criteria
12. FedRAMP Rev. 5 Baseline â†’ https://www.fedramp.gov/
13. Sarbanes-Oxley Act of 2002 (Public Law 107-204)
14. Verizon 2024 Data Breach Investigations Report
15. Ponemon Institute â†’ Cost of Compliance vs Cost of Non-Compliance (2024)
16. UK ICO â†’ Marriott International Inc. Monetary Penalty Notice (2020)
17. Irish DPC â†’ TikTok Decision (DPC Inquiry IN-22-2-2, September 2023)
18. US DOJ â†’ Wells Fargo Enforcement Actions (2020)
## Section 15: Supplementary Content

### 15.1 Expanded Interview Corner â†’ Additional 15 Q&As


### Q16: What is the principle of "Privacy by Design" and how do you implement it?


**A:** Privacy by Design (PbD) is a framework developed by Ann Cavoukian that embeds privacy into the design specifications of technology, business practices, and infrastructure â†’ rather than adding it as an afterthought. The seven principles: (1) Proactive not reactive â†’ anticipate privacy issues before they occur, (2) Privacy as default â†’ no action required from the user, (3) Privacy embedded into design â†’ system architecture includes privacy controls, (4) Full functionality â†’ privacy and security are not trade-offs, (5) End-to-end security â†’ data protected from collection to deletion, (6) Visibility and transparency â†’ processes are open and documented, (7) Respect for user privacy â†’ user-centric controls and granular notifications. Implementation: Conduct a DPIA before building any new system, minimize data collection by default, encrypt data at rest and in transit by default, provide users with granular consent controls, and log all access to personal data.
...
**A:** The SoA is a mandatory document in ISO 27001 (Clause 6.1.3) that lists all 93 Annex A controls and for each one states: (a) whether it is applicable or not, and (b) justification for inclusion or exclusion. For applicable controls, the SoA cross-references to the specific policies, procedures, and controls that address each requirement. The SoA bridges the risk assessment and the control implementation â†’ it proves that the organization has systematically considered every Annex A control and made a risk-based decision about each one. Excluding controls requires specific justification (e.g., "Control 8.19 (installation of software) is not applicable because the organization uses only SaaS applications and no software installation is permitted on endpoints").

### Q19: What is the "shared responsibility model" in cloud security?


**A:** The shared responsibility model defines which security controls are managed by the cloud provider vs the customer. The provider is responsible for **security OF the cloud** â†’ physical security, hardware, hypervisor, network infrastructure, and (for SaaS) the application itself. The customer is responsible for **security IN the cloud** â†’ data classification, encryption, IAM configuration, network ACLs, OS patching (in IaaS), application security (in PaaS), and compliance. The exact split depends on the service model:
...
**A:** SOX Section 404 requires management to assess and report on the effectiveness of internal controls over financial reporting (ICFR). IT controls are relevant because financial systems (ERP, accounting software, payment systems) are IT systems. The IT General Controls (ITGC) that support financial reporting integrity must be documented, tested, and proven effective. Key ITGC domains: (1) Access Management â†’ who can create, modify, and delete financial records; (2) Change Management â†’ changes to financial systems are authorized, tested, and approved before deployment; (3) Computer Operations â†’ batch jobs run correctly, error handling, backup/restore; (4) Program Development â†’ new financial systems follow secure SDLC. The external auditor tests ITGCs as part of the financial statement audit.
...
**A:** Defense in depth is a layered security strategy where multiple independent controls protect the same asset, so if one fails, another still provides protection. In GRC terms, each layer is a control that corresponds to a policy requirement, risk mitigation, or compliance mandate. Layers typically include: (1) Physical security (fences, locks, guards) â†’ policy: Physical Security Policy, (2) Network security (firewalls, IDS/IPS, segmentation) â†’ standard: Network Security Standard, (3) Endpoint security (EDR, antivirus, patch management) â†’ baseline: Endpoint Security Baseline, (4) Application security (WAF, input validation, secure coding) â†’ procedure: Secure SDLC Procedure, (5) Data security (encryption, DLP, access controls) â†’ standard: Data Protection Standard, (6) Identity security (MFA, RBAC, PAM) â†’ policy: Access Control Policy, (7) Administrative controls (awareness training, background checks) â†’ procedure: Onboarding Procedure. GRC ensures each layer has a documented owner, is tested periodically, and has compensating controls for known gaps.
...
**A:** Materiality threshold is the level at which a risk becomes significant enough to require disclosure to the board, regulators, shareholders, or the public. It's borrowed from financial auditing (SEC defines materiality as information that would influence a reasonable investor's decision). In cybersecurity: (1) **Financial materiality** â†’ breach costs exceeding X% of revenue (e.g., 3% for SEC proposed rules), (2) **Operational materiality** â†’ downtime exceeding MTD, (3) **Regulatory materiality** â†’ breach volume exceeding GDPR notification thresholds, (4) **Reputational materiality** â†’ breach affecting VIP customers or causing national news coverage. SEC's 2023 cybersecurity disclosure rules require public companies to disclose material cybersecurity incidents within 4 business days. The CISO and CFO must agree on materiality assessment criteria before an incident occurs.

### Q28: How do you build a business case for security investment?


**A:** A security business case must speak the language of the CFO â†’ ROI and risk reduction. Framework: (1) **Identify the risk** â†’ what could happen (ransomware encrypting 500 servers), probability (20% annual), impact ($5M). (2) **Quantify the current ALE** â†’ $5M x 0.2 = $1M/year. (3) **Propose controls** â†’ EDR + immutable backups + IR retainer = $200K/year. (4) **Quantify residual ALE** â†’ after controls, likelihood drops to 5%, impact drops to $500K. New ALE = $25K/year. (5) **Calculate ROSI** â†’ ($1M - $25K - $200K) / $200K = 387%. (6) **Include qualitative factors** â†’ regulatory fines, reputational damage, customer trust. (7) **Compare to alternatives** â†’ cyber insurance ($150K/year with $500K deductible) vs self-insuring vs accepting risk. (8) **Request specific budget** â†’ with clear deliverables, timeline, and success metrics.

### Q29: What is the difference between an Information Security Policy and an ISMS Scope?


**A:** The **Information Security Policy** (ISO 27001 Clause 5.2) is a high-level document that states management's commitment to information security, defines high-level objectives, and assigns responsibilities. It's typically 2-5 pages and applies to the entire organization. The **ISMS Scope** (Clause 4.3) defines the boundaries of the Information Security Management System â†’ which systems, departments, locations, and data are covered by ISO 27001 certification. The scope might be narrower than the entire organization (e.g., "The ISMS covers the production data center in Frankfurt and all customer data processing systems. Corporate HR systems are out of scope."). The scope is determined by: organizational context, stakeholder requirements, interfaces and dependencies with external organizations, and excluded systems with justification.

### Q30: What are the key considerations for international data transfers under GDPR?


**A:** GDPR Chapter V restricts transfers of personal data outside the EU/EEA unless one of the following "transfer mechanisms" is in place: (1) **Adequacy decision** â†’ European Commission determines the destination country has "adequate" data protection (currently: Andorra, Argentina, Canada (commercial), Faroe Islands, Guernsey, Israel, Isle of Man, Japan, Jersey, New Zealand, Republic of Korea, Switzerland, UK, United States (Data Privacy Framework), Uruguay). (2) **Standard Contractual Clauses (SCCs)** â†’ pre-approved contractual clauses between data exporter and importer. Updated in 2021 (Module 1-4). Must conduct Transfer Impact Assessment (TIA). (3) **Binding Corporate Rules (BCRs)** â†’ approved internal data protection policies for multinational groups. (4) **Derogations** â†’ explicit consent, contract necessity, vital interests, public interest, legal claims, or legitimate interests (must notify and document). Key consideration post-Schrems II (CJEU 2020): even with SCCs, you must assess whether the destination country's laws provide "essentially equivalent" protection â†’ if not, you need supplementary measures (technical encryption, contractual enforcement, data minimization).
...
| R-005 | Insider threat â†’ DBA exfiltrates patient records | Disgruntled employee | No DLP, no separation of duties | A-001 |
...
| R-009 | Vendor breach â†’ AWS itself | Sophisticated attacker | Over-reliance on single provider | All |
...
1. Categories of personal data we process:
   [List categories â†’ e.g., Identity data, Contact data, Financial data, Transaction data]
...
3. Recipients who have received your personal data:
   [List recipients or categories â†’ e.g., Payment processor (Stripe), Email provider (SendGrid)]
...
```
SECURITY SCORECARD â†’ [MONTH] [YEAR]
...
RISK MANAGEMENT (18/25)
  [+] Open critical risks: 1 (target: 0) â†’ remediation due 04/15
  [=] Open high risks: 5 (target: <5) â†’ trending down from 8 last quarter
  [+] Risk assessments completed: 3/3 for Q1 (vendor, new app, annual)
  [-] Cyber insurance renewal: Pending â†’ new requirements not yet met
...
**AI/ML Governance:**
- EU AI Act (2025) â†’ risk-based classification of AI systems (minimal, limited, high, unacceptable risk)
- NIST AI Risk Management Framework (AI RMF 1.0)
- Algorithmic impact assessments â†’ similar to DPIA but for AI bias, safety, transparency
- Model card documentation â†’ standard format for reporting model capabilities, limitations, and fairness metrics
- AI Bill of Materials (AIBOM) â†’ similar to SBOM for machine learning models (training data sources, model architecture, performance metrics)

**Blockchain / DLT Governance:**
- Decentralized identity â†’ self-sovereign identity (SSI) for access management
- Smart contract audits â†’ third-party code review (similar to application security assessment)
- Consensus mechanism risk â†’ 51% attack, governance token manipulation
- Regulatory compliance â†’ FATF Travel Rule for crypto transactions, MiCA (EU Markets in Crypto Assets)

**IoT/OT Governance:**
- Purdue model segmentation for ICS/SCADA
- NIST IR 8259 â†’ IoT device cybersecurity core baseline
- OT-specific risk assessments (IEC 62443 series â†’ security for industrial automation)
- Physical security integration â†’ badge access, CCTV, environmental monitoring as security controls

**Quantum Computing Risk:**
- Cryptographic agility â†’ ability to migrate from current asymmetric crypto (RSA, ECC) to post-quantum cryptography (CRYSTALS-Kyber, Dilithium)
- Harvest-now-decrypt-later risk â†’ adversaries collecting encrypted data today to decrypt with future quantum computers
...
| Term | Definition |
|------|-----------|
| **ALE** | Annualized Loss Expectancy â†’ expected annual financial loss from a risk (SLE x ARO) |
| **ARO** | Annualized Rate of Occurrence â†’ expected frequency of a risk event per year |
| **BIA** | Business Impact Analysis â†’ process of identifying critical functions and recovery requirements |
| **BCP** | Business Continuity Plan â†’ plan to maintain operations during disruption |
| **CISO** | Chief Information Security Officer â†’ executive responsible for information security |
| **CIS Controls** | Prioritized set of 18 cybersecurity best practices |
| **COBIT** | Control Objectives for Information and Related Technologies â†’ IT governance framework |
| **CVSS** | Common Vulnerability Scoring System â†’ standard for rating vulnerability severity (0-10) |
| **DAST** | Dynamic Application Security Testing â†’ testing running applications for vulnerabilities |
| **DPA** | Data Protection Authority â†’ national GDPR enforcement body |
| **DPIA** | Data Protection Impact Assessment â†’ GDPR risk assessment for high-risk processing |
| **DPO** | Data Protection Officer â†’ mandatory GDPR role for certain organizations |
| **DR** | Disaster Recovery â†’ restoring IT systems after disruption |
| **DSAR** | Data Subject Access Request â†’ GDPR right to access personal data |
| **EDR** | Endpoint Detection and Response â†’ advanced anti-malware with behavioral detection |
| **FAIR** | Factor Analysis of Information Risk â†’ quantitative risk analysis model |
| **FedRAMP** | Federal Risk and Authorization Management Program â†’ US federal cloud security program |
| **GDPR** | General Data Protection Regulation â†’ EU data protection law |
| **HIPAA** | Health Insurance Portability and Accountability Act â†’ US healthcare privacy law |
| **IG1/2/3** | CIS Implementation Groups â†’ tiered approach to implementing CIS Controls |
| **IR** | Incident Response â†’ process of responding to security incidents |
| **ISMS** | Information Security Management System â†’ ISO 27001 framework |
| **ITGC** | IT General Controls â†’ SOX controls over IT systems supporting financial reporting |
| **KPI** | Key Performance Indicator â†’ metric measuring performance against target |
| **KRI** | Key Risk Indicator â†’ leading indicator predicting increased risk |
| **MFA** | Multi-Factor Authentication â†’ authentication using 2+ factors (password + token) |
| **MTD** | Maximum Tolerable Downtime â†’ longest acceptable outage before irreparable harm |
| **NIST CSF** | NIST Cybersecurity Framework â†’ 6-function framework for cybersecurity programs |
| **PDCA** | Plan-Do-Check-Act â†’ continuous improvement cycle in ISO 27001 and ISO 22301 |
| **PHI** | Protected Health Information â†’ HIPAA-regulated health data |
| **PII** | Personally Identifiable Information â†’ any data identifying an individual |
| **RMF** | Risk Management Framework â†’ NIST 7-step risk management process |
| **ROSI** | Return on Security Investment â†’ ROI calculation for security controls |
| **RPO** | Recovery Point Objective â†’ maximum acceptable data loss (time) |
| **RTO** | Recovery Time Objective â†’ target restoration time after disruption |
| **SAST** | Static Application Security Testing â†’ analyzing source code for vulnerabilities |
| **SCC** | Standard Contractual Clauses â†’ GDPR-approved contract for international data transfers |
| **SLE** | Single Loss Expectancy â†’ cost of a single occurrence of a risk (AV x EF) |
| **SoA** | Statement of Applicability â†’ ISO 27001 document listing applicable Annex A controls |
| **SOC 2** | Service Organization Control 2 â†’ AICPA standard for service org controls |
| **SOX** | Sarbanes-Oxley Act â†’ US law for financial controls in public companies |
| **TPRM** | Third-Party Risk Management â†’ vendor security assessment program |
| **UEBA** | User and Entity Behavior Analytics â†’ ML-based anomaly detection |

### 15.12 Exam Tips & Tricks


**For CISSP (Domain 1 â†’ Security and Risk Management):**
...
**For CISA (Domain 1 â†’ Information System Auditing Process):**
...
**For CISM (Domain 1 â†’ Information Security Governance):**
...
**Common Exam Traps:**
- "Risk = Threat x Vulnerability" is WRONG â†’ risk = likelihood x impact (threat x vulnerability x impact is incomplete)
...
|------|--------|---------------|
| 1 | Preserve evidence (forensic images, logs, memory) | Compliance â†’ chain of custody |
| 2 | Identify affected data types (PII, PHI, PCI) | Compliance â†’ breach notification trigger |
| 3 | Determine data subjects affected | Compliance â†’ GDPR Art 33/34, HIPAA Breach Rule |
| 4 | Count affected records | Compliance â†’ notification thresholds |
| 5 | Start 72-hour timer (GDPR) | Compliance â†’ countdown from discovery |
| 6 | Notify DPO / Privacy Officer | Governance â†’ incident escalation procedure |
| 7 | Activate IR plan | Governance â†’ plan execution |
| 8 | Notify cyber insurance broker | Risk Transfer â†’ policy requirements |
| 9 | Contain breach (isolate systems, revoke access) | Risk Mitigation â†’ reduce further impact |
| 10 | Notify Legal Counsel | Governance â†’ legal privilege, regulatory notification |
| 11 | Engage forensics firm (if needed) | Risk Transfer / Mitigation |
| 12 | Begin internal investigation (root cause) | Risk Management â†’ prevent recurrence |
| 13 | Prepare regulatory notifications | Compliance â†’ DPA, HHS, PCI acquirer |
| 14 | Prepare customer notifications | Compliance + Reputation |
| 15 | Prepare press release / public statement | Governance â†’ board communication, PR |
| 16 | Document lessons learned | Risk Management â†’ update risk register |
| 17 | Update IR plan based on lessons | Governance â†’ continuous improvement |
| 18 | Report to Board | Governance â†’ fiduciary duty |
| 19 | File regulatory reports (e.g., SEC 8-K) | Compliance â†’ public company disclosure |
| 20 | Close incident and update risk register | Risk Management â†’ update residual risk scores |
...
- **Governance** establishes the rules through policies, standards, procedures, and an organizational structure with clear ownership â†’ starting at the Board and flowing through CISO, risk committees, and operational teams.
...
- **Compliance** ensures adherence to a complex web of regulations â†’ GDPR (privacy, DSARs, breach notification), PCI DSS 4.0 (12 requirements for card data), HIPAA (administrative/physical/technical safeguards for PHI), SOC 2 (5 trust service criteria), FedRAMP (cloud security for federal data), and SOX (financial controls and ITGC).

- **Frameworks** provide structured approaches â†’ NIST CSF 2.0 for strategic program design, ISO 27001 for certifiable ISMS with 93 Annex A controls, COBIT for IT governance, and CIS Controls for prioritized, actionable safeguards.
...
- **Audits** validate control effectiveness â†’ internal for readiness, external for certification and compliance assurance.

- **Metrics and KPIs** connect GRC to business outcomes, enabling data-driven decisions and board-level visibility.

The four case studies (Equifax, Marriott, TikTok, Wells Fargo) demonstrate that GRC failures have consequences measured in billions of dollars and millions of affected individuals â†’ not because of sophisticated technology attacks, but because of governance, risk management, and compliance breakdowns.
...
5. HIPAA Security Rule (45 CFR Parts 160, 162, and 164)
6. NIST SP 800-53 Rev. 5 â†’ Security and Privacy Controls
7. NIST SP 800-34 Rev. 1 â†’ Contingency Planning Guide for Federal Information Systems
8. FAIR Institute â†’ FAIR-TM Quantitative Risk Analysis
9. CIS Controls v8 â†’ https://www.cisecurity.org/controls
10. COBIT 2019 â†’ ISACA
11. AICPA SOC 2 Trust Services Criteria
...
```typescript
/**
 * Compliance Finding â€” represents a single control evaluation result
 */
...
/**
 * Compliance Rule â€” defines a checkable control with a test function
 */
...
/**
 * Compliance Report â€” aggregated result of checking all rules in a framework
 */
...
/**
 * ComplianceChecker â€” evaluates system configurations against
...
class ComplianceChecker {
  ...
  private registerDefaultRules(): void {
    // â”€â”€ GDPR Rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    this.rules.push(
      {
        id: 'GDPR-01',
        framework: 'GDPR',
    ...
    // â”€â”€ ISO 27001 Rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    this.rules.push(
      {
        id: 'ISO-01',
        framework: 'ISO 27001',
    ...
    // â”€â”€ PCI DSS Rules â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    this.rules.push(
      {
        id: 'PCI-01',
        framework: 'PCI DSS',
  ...
  check(config: Record<string, any>, framework: string): ComplianceReport {
    ...
    for (const rule of applicableRules) {
      ...
      findings.push({
        ...
        details: compliant
          ? `Compliant â€” ${rule.control} passed`
          : `Non-compliant â€” ${rule.description}`,
      });
      if (compliant) passed++;
      else failed++;
    }
    ...
    return findings
      ...
      .map(
        f =>
          ...
          `â†’ Implement control "${f.controlName}"`,
      );
  }
}

// â”€â”€ Example Usage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * PolicyAnalyzer â€” evaluates policy documents for completeness
...
if (gaps.length === 0) {
  console.log('âœ“ No gaps found â€” policy is structurally complete');
} else {
  console.log(`Found ${gaps.length} gap(s):`);
  gaps.forEach(g =>
    console.log(`  âš  ${g.missingElement}: ${g.recommendation}`),
  );
}
...
isoChecks.forEach(c =>
  console.log(`  ${c.present ? 'âœ“' : 'âœ—'} ${c.element}`),
);
...
gdprChecks.forEach(c =>
  console.log(`  ${c.present ? 'âœ“' : 'âœ—'} ${c.requirement}`),
);
```
...
```mermaid
flowchart TD
    L["ðŸ“œ Laws<br/>(e.g., GDPR, HIPAA, SOX, FISMA)"]
    R["ðŸ“‹ Regulations<br/>(e.g., Data Protection Act, HITECH, SEC Rules)"]
    P["ðŸ“„ Policies<br/>(e.g., Security Policy, Privacy Policy, Acceptable Use)"]
    S["ðŸ“ Standards<br/>(e.g., ISO 27001, NIST SP 800-53, CIS Controls)"]
    PR["âš™ï¸ Procedures<br/>(e.g., Incident Response Playbook, Access Request Process)"]
    G["ðŸ“˜ Guidelines<br/>(e.g., Secure Coding Guide, Password Best Practices)"]
...
```mermaid
flowchart LR
    A["ðŸ” Identify<br/>Asset & Threat<br/>Identification"]
    B["ðŸ“Š Analyze<br/>Likelihood &<br/>Impact Assessment"]
    C["ðŸ“ˆ Evaluate<br/>Risk Level vs<br/>Risk Appetite"]
    D{"ðŸ¤” Treat<br/>Select Treatment<br/>Strategy"}
    E1["âœ… Mitigate<br/>Implement Controls<br/>Reduce Risk"]
    E2["ðŸ”„ Accept<br/>Formal Acceptance<br/>Residual Risk"]
    E3["ðŸ“¤ Transfer<br/>Insurance /<br/>Outsource"]
    E4["âŒ Avoid<br/>Discontinue<br/>Activity"]
    F["ðŸ“‹ Monitor<br/>Continuous<br/>Monitoring & Review"]
...
**C) ISO/IEC 27001:2022** â€” Organizations can achieve ISO 27001 certification through accredited registrars. NIST CSF, CIS Controls, and COBIT are frameworks intended for self-assessment and guidance, not certification.
</details>
...
<details>
<summary>Answer</summary>
**C) 72 hours** â€” Article 33(1) requires notification "without undue delay and, where feasible, not later than 72 hours after having become aware of the breach."
</details>
...
A) Threat Ã— Vulnerability
B) Likelihood Ã— Impact
C) Asset Value Ã— Threat Score Ã— Vulnerability Score
D) Probability Ã— Consequence

<details>
<summary>Answer</summary>
**B) Likelihood Ã— Impact** â€” Qualitative assessments use ordinal scales (e.g., Low/Medium/High) for likelihood and impact. The product determines the risk level.
</details>
...
<details>
<summary>Answer</summary>
**C) Tracking and monitoring** â€” Requirement 10 is "Track and monitor all access to network resources and cardholder data" via audit logging, log reviews, and log retention.
</details>
...
<details>
<summary>Answer</summary>
**B) Policies are mandatory; guidelines are advisory** â€” Policies use "must/shall" language and carry enforcement mechanisms. Guidelines use "should/consider" language and suggest best practices.
</details>
...
A) Administrative Safeguards â€” Security Management Process
B) Physical Safeguards â€” Facility Access Controls
C) Technical Safeguards â€” Access Control
D) Organizational Requirements â€” Business Associate Contracts

<details>
<summary>Answer</summary>
**C) Technical Safeguards â€” Access Control (45 CFR Â§ 164.312(a)(1))** â€” Unique User Identification (a)(2)(i) is an implementation specification of the Access Control standard.
</details>
...
<details>
<summary>Answer</summary>
**C) 7 steps** â€” The NIST RMF 7-step process: Prepare â†’ Categorize â†’ Select â†’ Implement â†’ Assess â†’ Authorize â†’ Monitor (NIST SP 800-37 Rev. 2).
</details>
...
<details>
<summary>Answer</summary>
**B) Business Impact Analysis (BIA)** â€” The BIA identifies critical business functions and determines maximum tolerable downtime, from which RTO, RPO, and MTD are derived.
</details>
...
B) The operating effectiveness of controls over a specified period (typically 6â€“12 months)
...
<details>
<summary>Answer</summary>
**B) Operating effectiveness over a period** â€” SOC 2 Type II tests whether controls are not only designed properly (Type I) but also operating effectively over time (Type II).
</details>
...
<details>
<summary>Answer</summary>
**C) Quantitative risk analysis** â€” FAIR decomposes risk into Loss Event Frequency (LEF) and Loss Magnitude (LM). LEF Ã— LM = Annualized Loss Expectancy (ALE).
</details>
...
1. **Map every regulation to specific controls** â€” Create a traceability matrix linking regulatory requirements (e.g., GDPR Art 32) to specific technical and procedural controls.
2. **Automate compliance checks** â€” Use the ComplianceChecker TypeScript code above to integrate compliance validation into CI/CD pipelines and infrastructure-as-code workflows.
3. **Write policies with structure** â€” Every security policy should contain Purpose, Scope, Roles, Enforcement, Exceptions, and Review Cycle. Use the PolicyAnalyzer to validate completeness.
4. **Adopt a risk treatment strategy** â€” Document whether each risk is mitigated, accepted, transferred, or avoided, with clear rationale and approval for acceptance decisions.
5. **Know your reporting obligations** â€” GDPR 72-hour breach notification, PCI DSS annual penetration testing, HIPAA BA agreements, and SOC 2 control testing all have specific timelines and formats.
6. **Use frameworks as a starting point** â€” NIST CSF, ISO 27001, and CIS Controls are complementary, not mutually exclusive. Map them together for comprehensive coverage.
7. **Audit readiness is continuous** â€” Don't prepare for audits reactively. Maintain evidence of control operation (logs, review records, attestations) as part of daily operations.

...
- Recognize real-world applications â€” social networks, image segmentation, percolation.
...
Imagine you walk into a party with 100 people you've never met. You want to know: *are these two people in the same social circle?* Each time someone introduces themselves â€” "Hi, I'm Alice, and this is my friend Bob" â€” you mentally merge their groups. After enough introductions, you can answer instantly whether any two people belong to the same extended friend group.

This is exactly what **Union-Find** does. It starts with every element in its own set (every person is their own island). As relationships arrive â€” (Alice, Bob), (Bob, Charlie) â€” it **unions** their sets. When you need to check connectivity â€” "Is Dave in Alice's circle?" â€” it **finds** the representative of each person's set and compares.

Union-Find is the most efficient data structure for **dynamic connectivity** â€” handling union and find queries interleaved in real time. With path compression and union by rank, both operations run in amortized **O(Î±(n))**, the inverse Ackermann function â€” for any practical input (n â‰¤ 10â¶â°â°), Î±(n) â‰¤ 5. That's effectively **constant time per operation**.

From Kruskal's MST (networks, circuit design) to social networks ("People You May Know"), image segmentation, and percolation theory â€” Union-Find is the hidden engine behind connectivity at scale.
...
| Path compression | Flatten tree during find | O(log n) amortized alone, O(Î±(n)) with union by rank |
| Union by rank | Attach smaller tree under larger | Limits tree depth to O(log n) |
| Combined optimizations | Path compression + union by rank | O(Î±(n)) amortized â€” effectively constant |
| Kruskal's MST | Sort edges + DSU for cycle detection | O(E log V) total |
| Connected components | Union adjacent elements in any graph | O(n Î±(n)) for n elements |
...
> **One-Sentence Takeaway:** DSU, with path compression and union by rank, supports find and union in amortized near-constant O(Î±(n)) â€” the most efficient dynamic connectivity structure ever devised.
...
Think of a corporate org chart. Every employee reports to a manager, who reports to a director, who reports to the CEO. **Find** is the question: *Who is the ultimate boss of this person?* You follow the chain of command until you reach the top â€” the person who reports to nobody (self-loop). That top person is the **representative** of the entire set (the whole org tree).
...
1. Check if `parent[x] == x`. If yes, `x` is the root â€” return `x`.
...
|------|---|-----------|-----------------|--------|--------|
| 1 | 3 | 2 | No | Recurse find(2) | â€” |
| 2 | 2 | 1 | No | Recurse find(1) | â€” |
| 3 | 1 | 0 | No | Recurse find(0) | â€” |
| 4 | 0 | 0 | **Yes** | Root found | 0 |
| Back | 1 | â€” | â€” | parent[1] = 0 | 0 |
| Back | 2 | â€” | â€” | parent[2] = 0 | 0 |
| Back | 3 | â€” | â€” | parent[3] = 0 | 0 |

**After find(3)**: parent = [0, 0, 0, 0, 4]

Now the tree is flat â€” 1, 2, 3 all point directly to root 0. That's **path compression** in action.
...
| With both optimizations | **O(Î±(n))** amortized | Inverse Ackermann function â€” see combined analysis |

**Why O(Î±(n))?** The inverse Ackermann function Î±(n) grows so slowly that for any n â‰¤ 10â¶â°â°, Î±(n) â‰¤ 5. It is the inverse of the Ackermann function A(n), which grows faster than any primitive recursive function. The proof (by Tarjan 1975) shows that the number of times path compression can improve any node's parent is bounded by the iterated logarithm, which collapses under the inverse Ackermann bound.
...
| Advantages | Disadvantages |
|------------|--------------|
| Extremely fast â€” effectively constant per operation | Read-only operation; no built-in support for splits |
...
| Edge Case | Behavior |
|-----------|----------|
| Single element | find(0) â†’ 0 immediately; parent[0] == 0 |
...
3. If `rootX == rootY`, they are already in the same set â€” return.
4. Otherwise, attach `rootY` under `rootX` (or vice versa, depending on rank/size).

### Pseudocode (without rank optimization â€” basic version)
...
| 2 | rootX(2) â‰  rootY(4) â†’ attach | parent[4] = 2 | â€” |
...
| 2 | rootX(1) â‰  rootY(3) â†’ attach | parent[3] = 1 | â€” |
...
| 2 | rootX(1) â‰  rootY(2) â†’ attach | parent[2] = 1 | â€” |
...
| With union by rank | **O(Î±(n))** amortized | Both optimizations combined |
...
| Edge Case | Behavior |
|-----------|----------|
| Union already-connected elements | find(u) == find(v) â†’ returns immediately, no change |
| Self-union (x == y) | rootX == rootY â€” no-op |
| Union with a root element | Works identically â€” find returns the root immediately |
| Union across large, already-compressed trees | Both finds are O(Î±(n)), union is O(1) |
...
FUNCTION find(x):
    IF parent[x] != x THEN
        parent[x] = find(parent[x])    // â† path compression: set parent to root
    END IF
    RETURN parent[x]
END FUNCTION
```
...
**Before**: parent = [0, 0, 1, 2, 3, 5] â€” a deep chain for elements 1-4.
...
```
    0 â† 1 â† 2 â† 3 â† 4     5 (root of itself)
```
...
|-----------------|---|-----------|--------|-----------------|
| 1 | 4 | 3 | Recurse find(3) | â€” |
| 2 | 3 | 2 | Recurse find(2) | â€” |
| 3 | 2 | 1 | Recurse find(1) | â€” |
| 4 | 1 | 0 | Recurse find(0) | â€” |
| 5 | 0 | 0 | **Root**, return 0 | â€” |
| Back 4 | 1 | â€” | parent[1] = 0 | 0 |
| Back 3 | 2 | â€” | parent[2] = 0 | 0 |
| Back 2 | 3 | â€” | parent[3] = 0 | 0 |
| Back 1 | 4 | â€” | parent[4] = 0 | 0 |
...
| With rank + compression | O(Î±(n)) amortized | Optimal â€” Tarjan proved the inverse Ackermann bound |
| Extra memory | None | No additional arrays needed â€” uses parent array in-place |

The key insight: path compression alone is already very good (O(log n) amortized). Adding union by rank only improves it from O(log n) to O(Î±(n)) â€” a theoretical improvement that matters more for proofs than for practice.
...
| Very deep tree (n = 10â¶) | Recursion depth = n; use iterative find to avoid stack overflow |
...
Both achieve O(log n) tree height without path compression, and O(Î±(n)) with path compression.
...
**union(0, 1)**: roots 0 and 1, both rank 0 â†’ equal â†’ attach 1â†’0, rank[0] = 1.
...
**union(2, 3)**: roots 2 and 3, both rank 0 â†’ attach 3â†’2, rank[2] = 1.
...
**union(4, 5)**: roots 4 and 5, both rank 0 â†’ attach 5â†’4, rank[4] = 1.
...
**union(0, 4)**: rootX = find(0) = 0 (rank 1), rootY = find(4) = 4 (rank 1). Equal ranks â†’ attach 4â†’0, rank[0] = 2.
...
**union(2, 0)**: rootX = find(2) = 2 (rank 1), rootY = find(0) = 0 (rank 2). rank[2] &lt; rank[0] â†’ attach 2â†’0.
...
| Why O(Î±(n)) for Combined Optimizations |
|----------------------------------------|
| **Union by rank alone** guarantees tree height â‰¤ logâ‚‚(n). Each find traverses at most O(log n) nodes. |
| **Path compression alone** makes subsequent finds cheaper. Tarjan (1975) proved amortized O(Î±(n)). |
| **Together**, union keeps height low and find flattens aggressively. The inverse Ackermann bound emerges because the number of times a node's parent can change is bounded by the iterated logarithm, which collapses to â‰¤ 5 for all practical n. |

**Intuition**: Î±(n) grows slower than log(log(...(n)...)) for any fixed number of logs. For n = 10â¶â°â°, Î±(n) â‰ˆ 5. Each operation is effectively O(1).
...
| Combined with path compression: O(Î±(n)) â€” theoretical best | Union by size is slightly more intuitive but equivalent |
| No complex logic â€” simple if-else | Does not help with set enumeration |
...
| Root rank never decreases | Rank is monotonic â€” only increases, never decreases |
...
In a growing friend group, you introduce two friends to each other. If they turn out to be *already* in the same circle through other connections, that introduction creates **redundancy** â€” a "cycle" in the social graph. In a computer network, adding a cable between two computers that already have a path between them creates a loop that disrupts routing.
...
2. If they are the same root, u and v are already connected â€” adding this edge would create a **cycle**.
...
|------|--------|----------|--------|
| find(0) = 0, find(1) = 1 | Different roots â†’ union | [0, 0, 2, 3, 4] | No |

**Process edge (1, 2):**

| Step | Action | parent[] | Cycle? |
|------|--------|----------|--------|
| find(1) = 0, find(2) = 2 | Different roots â†’ union | [0, 0, 0, 3, 4] | No |

**Process edge (2, 3):**

| Step | Action | parent[] | Cycle? |
|------|--------|----------|--------|
| find(2) = 0, find(3) = 3 | Different roots â†’ union | [0, 0, 0, 0, 4] | No |
...
The cycle is detected because when processing (3,1), both 3 and 1 already have root 0 â€” they were connected via 3â†’0 and 1â†’0 through the path 1-0-2-3 (or 1-2-3). Adding edge (3,1) closes the triangle.
...
| Each edge processed | O(Î±(n)) | Two finds + one union, each O(Î±(n)) |
| **Total** | **O(E Î±(n))** | Linear in edges, effectively O(E) for practical inputs |
| Without DSU (DFS) | O(V + E) | DFS per component; comparable but requires recursion/stack |

**Why DSU is better for dynamic graphs**: If edges arrive one at a time (online), DSU handles each edge in O(Î±(n)) without revisiting previous edges. DFS would need to re-traverse the entire graph for each new edge.
...
| Advantages | Disadvantages |
|------------|--------------|
| Simple â€” no recursion, no visited arrays | Only works for undirected graphs |
| Handles online edge additions efficiently | Directed cycle detection requires DFS (topological order) |
| O(E Î±(n)) time â€” effectively linear | Cannot detect *which* edges form the cycle (just *whether* one exists) |
...
| Edge Case | Behavior |
|-----------|----------|
| Empty graph (no edges) | No cycle â€” returns false |
| Single vertex with self-loop | u == v in an edge â†’ find(u) == find(v) â†’ **cycle detected** |
| Disconnected cycle | Works per component â€” each component checked independently |
...
A city wants to lay fiber-optic cable connecting all neighborhoods. Running cable is expensive â€” you pay per meter. You want to connect every neighborhood (vertices) using the *least total cable length* (minimum total weight) without redundant loops (no cycles). You start with the cheapest cable segments and add them one by one, skipping any segment whose endpoints are already connected. This is Kruskal's algorithm.
...
| 2 | (0,2,2) | 0 | 1 | No | Union | [0, 1, 1, 3, 4] â†’ compress find(0)=0, find(2)=1 â‰  | parent[0]=1? No â€” use union by rank. Let's do basic: parent[1]=0 | [0, 0, 1, 3, 4] | [(1,2,1), (0,2,2)] | 3 |
...
| 6 | (2,4,8) | find(2)=find(1)=0, find(4)=find(3)=0 | Same | Skip | â€” | â€” | 10 |
...
| Operation | Complexity | Why? |
|-----------|------------|------|
| Sorting edges | O(E log E) = O(E log V) | E â‰¤ VÂ² for simple graphs, so log E â‰ˆ 2 log V |
| DSU operations per edge | O(Î±(V)) | Two finds + one union per edge processed |
| Total DSU cost | O(E Î±(V)) | All edges processed at most once |
| **Total** | **O(E log V)** | Sorting dominates â€” DSU is effectively free |
| Prim's algorithm | O(E log V) | Same, but with heap; better for dense graphs |

**Why sorting dominates**: For any realistic graph, sorting O(E log V) >> DSU O(E Î±(V)). If E = 10â¶, log V â‰ˆ 20, but Î±(V) â‰ˆ 4. Sorting is 5Ã— more expensive than the DSU part.
...
| DSU simplifies cycle detection | Not suitable for directed graphs (directed MST = Chuâ€“Liu/Edmonds) |
...
|-------------|----------|----------|------------------------|
| Kruskal's MST | Union endpoints of sorted edges; skip if already connected | Detects cycles in O(Î±(V)) per edge | DFS per edge â†’ O(EÂ²) |
| Connected components labeling (image) | Union adjacent pixels with same color | Processes entire image in O(pixels) | BFS/DFS flood fill â€” O(pixels) but uses recursion stack |
| Social network friend circles | Union pairs of friends | Dynamic â€” handles new friendships in real time | BFS after each new connection â†’ O(nÂ²) |
| Cycle detection in undirected graph | For each edge, if find(u) == find(v), cycle exists | Online â€” works as edges arrive | DFS from scratch each time â†’ O(V(V+E)) |
| Percolation (physics) | Union open sites with neighbors | Answers "does system percolate?" in O(Î±(n)) | Union-Find is the standard approach |
| Number of Islands II (dynamic) | Union newly added land with adjacent land | Handles incremental island addition | Flood fill after each add â†’ O(k * grid) |
| Longest Consecutive Sequence | Union consecutive numbers | O(n) with hash map to track values | Sorting â†’ O(n log n) |
| Accounts Merge | Union emails belonging to same person | Groups accounts without building full graph | DFS on email graph â€” comparable but more code |
| Redundant Connection | Union edge endpoints; if already connected, edge is redundant | Finds the exact redundant edge in O(E Î±(V)) | DFS each edge â†’ O(EÂ²) |
| Maze generation (randomized Kruskal) | Union cells connected by removed wall | Generates uniform spanning tree | Randomized DFS â€” less uniform |

## Interview Corner

> **One-Sentence Takeaway:** Union-Find is the secret weapon for connectivity problems in coding interviews â€” when you hear "connected components" or "union" in a problem, reach for DSU.
...
**Problem**: Given an unsorted array of integers, find the length of the longest consecutive elements sequence (e.g., [100, 4, 200, 1, 3, 2] â†’ 4 for [1, 2, 3, 4]).
...
**Time**: O(N Î±(K)) where N = total emails, K = unique emails, Space: O(N).
...
**Why DSU?**: Process edges in order. For each edge (u, v), if find(u) == find(v), this edge creates a cycle â€” it's the redundant connection.
...
See the Cycle Detection section above for a complete treatment. The interview pattern is always the same: DSU processes edges in O(E Î±(V)), returning true the moment a cycle is found.

## Applications in Real Systems

### 1. Kruskal's Minimum Spanning Tree â€” Network Design
...
DSU makes the cycle-detection step nearly free â€” O(Î±(V)) per edge â€” allowing Kruskal to handle graphs with millions of vertices.

### 2. Image Segmentation â€” Connected Component Labeling
...
### 3. Social Networks â€” Friend Circles and Recommendations
...
Facebook's 2021 friend graph had ~3 billion nodes â€” DSU operations on this scale run in milliseconds per query with path compression.

### 4. Percolation Theory â€” Physics and Materials Science
...
### 5. Dynamic Connectivity â€” Database and Distributed Systems
...
> **One-Sentence Takeaway:** Union by rank limits tree depth; path compression alone already gives O(log n) amortized â€” together they reach the theoretical best.
...
- **Path compression alone is almost enough**: Even without union by rank, path compression gives amortized O(log n). Adding union by rank yields the inverse Ackermann bound O(Î±(n)).
...
- **Kruskal's algorithm + DSU is the canonical MST algorithm**: Sort edges by weight (O(E log V)), process edges in order, union endpoints if they belong to different components. DSU detects cycles in O(Î±(V)) per edge.
- **Iterative find for safety**: In languages with limited recursion depth (Python, Java default stack), consider an iterative find implementation to avoid stack overflow on very deep trees (n > 10âµ). The recursive version is cleaner and preferred for interviews.
...
- Combined optimizations yield O(Î±(n)) per operation, where Î± is the inverse Ackermann function â€” â‰¤ 5 for any practical input.
- Kruskal's algorithm uses DSU to find MSTs in O(E log V) by processing edges in sorted order.
- DSU detects cycles in undirected graphs in O(E Î±(V)) â€” simpler than DFS for online edge additions.
...
|-------------|---------|---------------------|-------------------|------|
| find() complexity | O(n) | O(log n) amortized | O(log n) | O(Î±(n)) amortized |
| union() complexity | O(n) | O(log n) amortized | O(log n) | O(Î±(n)) amortized |
...
| Worst-case tree depth | n | O(log n) | O(log n) | O(Î±(n)) |
| Practical Î±(n) for n â‰¤ 10â¶â°â° | â€” | â€” | â€” | â‰¤ 5 |
...
| find(x) | if parent[x] != x: parent[x] = find(parent[x]); return parent[x] | O(Î±(n)) |
| union(x, y) | rx = find(x); ry = find(y); if rx == ry: return; if rank[rx] &lt; rank[ry]: parent[rx] = ry; else if rank[rx] &gt; rank[ry]: parent[ry] = rx; else: parent[ry] = rx; rank[rx]++ | O(Î±(n)) |
| connected(x, y) | return find(x) == find(y) | O(Î±(n)) |
...
|---------|-----------------|------------|
| Kruskal's MST | Union endpoints of each edge; process sorted edges | O(E log V + E Î±(V)) |
| Connected components | Union adjacent nodes; count roots | O(n Î±(n)) |
| Number of islands (static) | DSU on grid cells (4-dir connectivity) | O(mn Î±(mn)) |
| Number of Islands II (dynamic) | Union newly added land with 4-dir adjacent land | O(k Î±(mn)) |
| Cycle detection (undirected) | For each edge: if find(u) == find(v), cycle exists | O(E Î±(V)) |
| Friend circles (social network) | Union connected users; count sets | O(m Î±(n)) for m relationships |
| Longest consecutive sequence | Union value-1 and value+1 via hash map | O(n Î±(n)) |
| Accounts merge | Union emails in same account; group by root | O(N Î±(K)) |
| Redundant connection | Union edge endpoints; find(u) == find(v) â†’ redundant | O(E Î±(V)) |
| Maze generation | Randomized Kruskal: union cells connected by removed wall | O(nÂ² Î±(nÂ²)) |
| Image segmentation | Union adjacent same-color pixels | O(pixels Î±(pixels)) |
| Percolation | Union open sites; check top-bottom connectivity | O(nÂ² Î±(nÂ²)) |
...
| Using only path compression without union by rank | Still better than naive, but union-by-rank gives optimal amortized O(Î±(n)) | Use both path compression AND union by rank for inverse-Ackermann complexity |
...
8. **The inverse Ackermann function Î±(n) for practical values of n (â‰¤ 10â¶) is:**
   - a) â‰¤ 5 âœ“
   - b) â‰¤ 10
   - c) â‰¤ logâ‚‚n
   - d) â‰¤ n
...
12. **In Kruskal's algorithm, DSU is used to:**
    ...
    - b) Detect cycles in O(Î±(n)) time âœ“
...
13. **Number of Islands II (LC 305): Initially all cells are water. When land appears, DSU can track islands in:**
    - a) O(1) per operation
    - b) O(Î±(n)) per operation âœ“
    - c) O(log n) per operation
    - d) O(nÂ²) per operation
...
14. **Redundant connection II (LC 685)**: Find the redundant edge in a rooted directed graph. Same concept but with directed edges â€” handle cases of two parents and cycles.
...
18. **Regions cut by slashes (LC 959)**: nÃ—n grid with slashes '/' and '\\'. Find the number of regions created. Use DSU on subdivided cells.
...
20. **Swim in rising water (LC 778)**: An nÃ—n grid with elevation values. Rain falls, water rises. Find the minimum time when there's a path from (0,0) to (n-1,n-1). Use DSU with increasing time.
...
| Union by rank + path compression | O(Î±(n)) amortized | O(Î±(n)) | O(n) | Both arrays | Production standard |
| Union by size | O(Î±(n)) amortized | O(Î±(n)) | O(n) | Component sizes | Size queries needed |
| Weighted DSU | O(Î±(n)) amortized | O(Î±(n)) | O(n) | Ratio/difference array | Equations with ratios |
...
| 2D DSU | O(Î±(n)) amortized | O(Î±(n)) | O(nÂ²) | Grid neighbor mapping | Grid/pixel connectivity |
...
4. **What is the inverse Ackermann function Î±(n) for practical n â‰¤ 10â¶â°â°?**
   - a) ~4 âœ“
   - b) ~log n
   - c) ~1
   - d) ~n
...
6. **What happens when find(u) == find(v) during Kruskal's algorithm?**
   ...
   - b) Edge is skipped (would create a cycle) âœ“
...
7. **Which is NOT a valid application of DSU?**
   ...
   - b) Finding shortest path between two nodes âœ“ (use Dijkstra's)
...
8. **How does union by rank (or size) improve DSU?**
   ...
   - b) It ensures the tree height stays O(log n) without compression âœ“
...
7. Use DSU to solve the **number of islands** problem in a binary grid (static version â€” all land given at once).
8. Implement **DSU with rollback** â€” support undoing the last union operation (useful for offline dynamic connectivity). Use a stack to record changes.

### Interview-Style Problems


9. **Number of Islands II**: Given an m Ã— n grid of water, and a list of positions to add land one-by-one, return the number of islands after each addition. (See Interview Corner above.)
...
11. **Redundant Connection II** (hard): This time the graph is a directed graph. Find the extra edge that makes it not a tree. Hint: consider three cases â€” a node with two parents, a cycle, or both.
...
- DSU maintains disjoint sets with find and union operations, both running in near-constant amortized O(Î±(n)) time with path compression and union by rank.
...
- The inverse Ackermann function Î±(n) grows so slowly that it is â‰¤ 5 for any practical input size.
- **Kruskal's algorithm** sorts edges by weight and uses DSU to build a minimum spanning tree in O(E log V).
- **Cycle detection** in undirected graphs is trivially O(E Î±(V)) with DSU â€” simply check if the endpoints of each edge are already in the same set.

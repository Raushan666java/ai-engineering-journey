...
```mermaid
flowchart TB
    A[Sadharan roop mein Krishna] --> B[Arjuna ka anurodh: vishvarupa dikhao]
    B --> C[Divya chakshu: jagrukta ki aankh]
    C --> D[Vishvarupa ka anubhav: sab ek]
    D --> E[Bhaya: man ko bada pan bhaari lagta hai]
    E --> F[Kala-asmi: samay sab kuch bahata hai]
    F --> G[Wapas aana: sadharan roop]
    G --> H[Ordinary hi param rahasy hai]
    H --> I[Roz ka jeevan hi sadhna hai]
```

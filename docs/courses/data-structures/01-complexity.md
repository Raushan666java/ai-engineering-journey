...
**Real-World Analogy:** Imagine you are building a contact-list app for a phone with 1 million users. When a user searches for a contact, using an **ArrayList** (O(n) search) would scan through all million entries â€” taking ~10 milliseconds. But using a **HashMap** (O(1) average search) retrieves the contact in under 1 microsecond. At scale, the difference between O(n) and O(1) is the difference between a snappy app and one that freezes. Data structure choice IS complexity analysis in action.

Every major system â€” Google Search, Redis, PostgreSQL, Netflix â€” lives or dies by choosing the right data structure for each operation. Complexity analysis is the tool that makes that choice rigorous.
...
### Big-O Notation (O) â€” Upper Bound


**Real-World Analogy:** An elevator has a maximum capacity of 1,000 kg. Whether you load 500 kg, 800 kg, or 1,000 kg, the elevator *never exceeds* 1,000 kg. Big-O gives the worst-case ceiling â€” the algorithm will never perform worse than this bound.
...
2. **Drop all constant factors** â€” \( 3n \) becomes \( n \); \( 1000 \) becomes \( 1 \).
3. **Keep only the highest-order term** â€” \( n^2 + n \) becomes \( n^2 \).
4. **Ignore lower-order terms** â€” they become negligible as \( n \) grows.

**Pseudocode â€” Determining Big-O of Linear Search:**
...
**Dry Run â€” Growth of O(n) vs O(nÂ²) for n = 1 to 10:**

| n | O(1) ops | O(log n) ops | O(n) ops | O(n log n) ops | O(nÂ²) ops |
|---|----------|--------------|----------|----------------|-----------|
| 1 | 1        | 0            | 1        | 0              | 1         |
| 2 | 1        | 1            | 2        | 2              | 4         |
| 3 | 1        | 2            | 3        | 5              | 9         |
...
Notice: at n=10, O(1) and O(nÂ²) differ by **100Ã—**. At n=1000, O(1) is still 1 operation but O(nÂ²) is 1,000,000 operations â€” the gap widens without bound.

**Demonstration Code â€” C++, Python, Java:**

```cpp
// C++ â€” Linear Search O(n)
#include <iostream>
#include <vector>
using namespace std;
...
- **Empty input (n = 0):** Loop body never executes. Big-O still O(n) â€” zero iterations is still linear in zero.
- **n = 1:** No loop iterations for the trivial case â€” O(1) effectively, but we still classify as O(n).
- **Target not present:** Every element is examined â€” true worst case, exactly n iterations.
- **Target at first position:** Best case O(1) â€” but Big-O describes worst case, so we still say O(n).
- **Duplicate elements:** Search stops at first match. Still O(n) worst-case since the match might be last.
- **Very small n (n &lt; nâ‚€):** Big-O guarantees only apply for n â‰¥ nâ‚€. For tiny inputs, a quadratic algorithm may outperform linear â€” this is why hybrid sorts (Timsort) exist.

---

### Omega Notation (Î©) â€” Lower Bound


**Real-World Analogy:** No matter how fast you drive, you cannot reach a destination in less time than the speed limit permits. If the speed limit is 60 km/h on a 100 km road, the *best possible* time is 100/60 â‰ˆ 1.67 hours. Omega provides this "best case" floor.
...
**Pseudocode â€” Best Case of Linear Search:**
...
**Dry Run â€” Î©(n) vs Î©(1) for Linear Search:**

| Input state | Operations | Omega class |
|-------------|-----------|-------------|
| Target at index 0 | 1 comparison | Î©(1) |
| Target at index 1 | 2 comparisons | Î©(1) â€” still constant |
| Target at index k | k + 1 comparisons | Î©(1) â€” constant for fixed k |
| Empty array | 0 comparisons | Î©(1) |
| Target absent (full scan) | n comparisons | Î©(n) â€” but this is the lower bound for all inputs |

For linear search, the best case is always Î©(1) because the target *could* be at position 0. But the worst case is O(n). The algorithm has different upper and lower bounds.

**Demonstration Code:**

```cpp
// C++ â€” Best-case demonstration
#include <iostream>
#include <vector>
using namespace std;

int findFirst(const vector<int>& arr) {
    return arr[0];  // Î©(1) â€” always one operation
}
...
```python
# Python â€” Best-case demonstration
def find_first(arr):
    return arr[0]  # Î©(1) â€” always one operation
...
```java
// Java â€” Best-case demonstration
...
**Complexity Analysis â€” WHY Î©(1)?**

The function `arr[0]` performs exactly one array access regardless of array size. No loop, no conditionals. The number of operations is constant (1) no matter whether n = 1 or n = 10â¹. Therefore the lower bound is Î©(1).
...
| Foundation for lower-bound proofs | Many algorithms have Î©(1) best case |

**Edge Cases:**

- **Empty array:** Î©(1) â€” still constant time to return the result (or throw).
- **Array with one element:** Î©(1) â€” one access.
- **Î©(n) algorithms:** Any algorithm that must examine all input (e.g., sum all elements) has Î©(n) â€” you can't sum n numbers without looking at each one.

---

### Theta Notation (Î˜) â€” Tight Bound


**Real-World Analogy:** A train that takes exactly 2 hours between two cities every trip â€” not faster, not slower. Î˜(g(n)) means the algorithm's running time is *both* O(g(n)) and Î©(g(n)) â€” it grows *exactly* like g(n) for large inputs.
...
**Dry Run â€” Trace Table for sumArray([3, 7, 1]):**
...
Total = 3n + 2 operations = Î˜(n). The upper bound is O(n) and the lower bound is Î©(n) â€” they match, so we get Î˜(n).
...
**Complexity Analysis â€” WHY Î˜(n)?**

The loop always runs exactly n times regardless of input values. Every input of size n produces exactly n iterations. The upper bound is O(n) (it never exceeds cn) and the lower bound is Î©(n) (it never does fewer than c'n operations). Since upper = lower = n, the bound is tight: Î˜(n).
...
| Eliminates ambiguity of "O vs Î©" | Harder to prove than Big-O alone |
...
- **No tight bound:** Linear search has O(n) but Î©(1) â€” they don't match, so there is no Î˜ bound.
- **Constant operation:** `arr[0]` is Î˜(1) â€” both upper and lower bounds are constant.
- **Nested loops that always run:** `for i in 0..n: for j in 0..n:` is Î˜(nÂ²) â€” always runs exactly nÂ² iterations.
...
**Dry Run â€” Cost per push_back Operation:**

| Operation | size before | capacity before | Resize? | Actual cost | Running total | Amortized cost |
|-----------|-------------|-----------------|---------|-------------|---------------|----------------|
| push(1)   | 0           | 1               | No      | 1           | 1             | 1              |
| push(2)   | 1           | 1               | Yes (â†’2)| 3 (alloc+copy+insert)| 4      | 2              |
| push(3)   | 2           | 2               | Yes (â†’4)| 5           | 9             | 3              |
| push(4)   | 3           | 4               | No      | 1           | 10            | 2.5            |
| push(5)   | 4           | 4               | Yes (â†’8)| 9           | 19            | 3.8            |
| push(6)   | 5           | 8               | No      | 1           | 20            | 3.33           |
| push(7)   | 6           | 8               | No      | 1           | 21            | 3              |
| push(8)   | 7           | 8               | No      | 1           | 22            | 2.75           |
| push(9)   | 8           | 8               | Yes (â†’16)| 17          | 39            | 4.33           |

After k pushes, the total cost â‰¤ 3k. Each push contributes at most 3 units on average â†’ **O(1) amortized**.

**Demonstration Code:**

```cpp
// C++ â€” Dynamic array amortized analysis
#include <iostream>
#include <vector>
using namespace std;
...
```python
# Python â€” List amortized analysis
import sys
...
```java
// Java â€” ArrayList amortized analysis
...
**Complexity Analysis â€” WHY O(1) Amortized?**

The key insight: expensive resize operations become rarer as the array grows. After a resize to capacity k, the next k/2 insertions all cost O(1). The total cost of k insertions is: k (cheap inserts) + k (copy during resize) + 1 (last insert) = 2k + 1. Average = (2k + 1) / k â‰ˆ 2 = **O(1) amortized**.
...
- **Single insertion:** Amortized analysis is meaningless for one operation â€” analyze as O(n) worst-case.
- **Repeated resizing:** If the growth factor is too small (e.g., 1.1Ã— instead of 2Ã—), amortized cost increases.
- **Memory fragmentation:** Theory assumes contiguous allocation; real systems may slow due to cache misses.
- **Shrinking:** Removing elements doesn't automatically release memory â€” you may need explicit `shrink_to_fit`.
...
**Real-World Analogy:** You have a backpack that can hold 10 kg. You're packing for a hike â€” every item you add takes up weight and space. Similarly, every variable, array, and recursive call consumes memory. Space complexity measures the *peak memory usage* of an algorithm.

**Definition:** Space complexity is the total amount of memory an algorithm needs relative to input size \( n \), including:
- **Input space** â€” storage for the input itself.
- **Auxiliary space** â€” extra memory for temporary variables, data structures, call stack.
...
**Pseudocode â€” Space Analysis of Factorial:**

```
// Recursive â€” O(n) space due to call stack
...
**Dry Run â€” Call Stack for factorialRecursive(5):**

| Call | n value | Stack depth | Memory used (approx) |
|------|---------|-------------|----------------------|
| factorialRecursive(5) | 5 | 1 | 1 frame |
| â†’ factorialRecursive(4) | 4 | 2 | 2 frames |
| â†’ â†’ factorialRecursive(3) | 3 | 3 | 3 frames |
| â†’ â†’ â†’ factorialRecursive(2) | 2 | 4 | 4 frames |
| â†’ â†’ â†’ â†’ factorialRecursive(1) | 1 | 5 | 5 frames (peak) |
| â†’ â†’ â†’ â†’ return 1 | - | 4 | unwinding |
| â†’ â†’ â†’ return 2 | - | 3 | |
| â†’ â†’ return 6 | - | 2 | |
| â†’ return 24 | - | 1 | |
| return 120 | - | 0 | |

Peak memory = 5 stack frames â†’ O(n) space.

**Demonstration Code:**

```cpp
// C++ â€” Space complexity comparison
#include <iostream>
using namespace std;

// O(n) space â€” recursive
int factRec(int n) {
    if (n <= 1) return 1;
    return n * factRec(n - 1);
}

// O(1) space â€” iterative
int factIter(int n) {
    int result = 1;
...
// O(n) auxiliary space â€” copy array
...
```python
# Python â€” Space complexity examples
...
# O(n) auxiliary space
def duplicate(arr):
    return arr[:]  # Creates new list â€” O(n) extra space
...
```java
// Java â€” Space complexity examples
...
**Complexity Analysis â€” WHY O(n) vs O(1):**

- **Recursive factorial:** Each call pushes a frame onto the call stack. n recursive calls â†’ n frames in memory at peak â†’ O(n) space.
- **Iterative factorial:** Only 2 integer variables exist regardless of n â†’ O(1) space.
...
| Floyd-Warshall | O(nÂ²) | 2D distance matrix |

**Advantages & Disadvantages:**

| Advantages | Disadvantages |
|------------|---------------|
| Prevents memory overflow in production | Modern machines have ample RAM â€” sometimes less critical than time |
...
- **n = 0:** Recursive factorial(0) uses 1 stack frame (base case) â€” still O(n) where n=0.
- **Very deep recursion (n > 10,000):** Stack overflow in most languages regardless of complexity class.
- **Shared references:** Two pointers to the same data don't double the space â€” only count unique allocations.
...
| Class | Name | n=10 | n=100 | n=10â¶ | Data Structure Examples |
|-------|------|------|-------|-------|------------------------|
...
| O(n) | Linear | 10 | 100 | 10â¶ | Linked List search, unsorted array search, Tree traversal |
| O(n log n) | Linearithmic | ~33 | ~664 | ~20Ã—10â¶ | Sorting (Merge sort, Heap sort), BST sort, building Segment Tree |
| O(nÂ²) | Quadratic | 100 | 10,000 | 10Â¹Â² | Nested loops, Bubble sort, Floyd-Warshall, adjacency matrix traversal |

**Key insight:** At n = 10â¶, O(nÂ²) requires 10Â¹Â² operations â€” roughly **17 minutes** at 1 GHz. O(n log n) takes ~20 million operations â€” about **20 ms**. That's a 50,000Ã— difference.
...
L = length of key for Trie; Î±(n) = inverse Ackermann (practically constant).
...
### Example 3: Logarithmic Complexity â€” Binary Search

```cpp
// C++ â€” O(log n) binary search
#include <iostream>
#include <vector>
using namespace std;
...
### Example 4: Recursive Complexity â€” Fibonacci (O(2â¿) vs O(n))
...
### Example 5: Space Complexity â€” Recursive vs Iterative (n = 5 factorial)
...
| 1 | factorial(5) | 5 | 1 |
| 2 | â†’ factorial(4) | 4 | 2 |
| 3 | â†’ â†’ factorial(3) | 3 | 3 |
| 4 | â†’ â†’ â†’ factorial(2) | 2 | 4 |
| 5 | â†’ â†’ â†’ â†’ factorial(1) | 1 | 5 (peak) |
| - | returns 1 â†’ 2 â†’ 6 â†’ 24 â†’ 120 | unwind | - |
...
- **Worst-case vs average-case**: Hash tables are O(1) average but O(n) worst-case â€” always know which guarantee your application needs.
...
1. **Count nested loops** â€” each nesting level typically multiplies by n (one loop = O(n), two loops = O(nÂ²)).
2. **Look for halving** â€” if the input shrinks by a factor each iteration (binary search), suspect O(log n).
3. **Check recursion** â€” count the number of recursive calls per level. Fibonacci makes 2 calls per level â†’ O(2â¿).
4. **Find the recurrence** â€” write T(n) = aÂ·T(n/b) + f(n), then apply the Master Theorem.
5. **Isolate the dominant term** â€” drop everything that grows slower than the fastest-growing term.

### Common Pitfalls

| Mistake | Why It's Wrong | Correct Approach |
|---------|----------------|------------------|
| "O(n + nÂ²) = O(n)" | nÂ² dominates n | O(nÂ²) |
...
| Saying O(2â¿) = O(nÂ·2â¿â»Â¹) | Both are O(2â¿) | Constants and lower terms dropped |
| Ignoring recursion depth for space | Recursion uses call stack memory | Include stack frames in space analysis |
| Claiming Î˜(n) when best case is O(1) | Theta requires matching upper AND lower bounds | Use O(n) worst-case, Î©(1) best-case |

### Classic Space-Time Tradeoff Questions

**Q: Find duplicates in an array â€” O(nÂ²) time O(1) space vs O(n) time O(n) space?**

The brute-force approach (nested loops) uses no extra memory but costs O(nÂ²). Using a HashSet trades O(n) extra memory for O(n) time:
```cpp
// O(n) time, O(n) space â€” using hash set
...
| Nested loops both 0 to n | O(nÂ²) |
| Loop divides by 2 each time | O(log n) |
| Loop with log n steps Ã— n inner | O(n log n) |
| Two recursive calls per level | O(2â¿) |
...
|---------------------|----------------------|----------------------------|
| **Google Search** | Inverted Index (HashMap-like) | O(1) keyword â†’ document lookup across billions of pages. Linear search would be impossible. |
| **Redis** | Hash Table, Skip List | O(1) key-value access, O(log n) sorted sets. Nanosecond latency at 100K+ ops/sec. |
| **PostgreSQL B-Tree Index** | B-Tree | O(log n) search/insert/delete for millions of rows. Log n for 10â¶ rows â‰ˆ 20 comparisons. |
...
| **DNS Resolution** | Trie (+ Hash Table cache) | O(L) domain lookup where L = domain length. Fast for 10â¹+ daily queries. |
...
| **Stock Exchange** | Binary Heap | O(log n) order insertion, O(1) min/max peek. 10â¶+ orders/sec at microsecond latency. |
...
| **Message Queue (Kafka)** | Queue (Circular Buffer) | O(1) enqueue/dequeue, O(1) sequential disk writes. 10â¶+ messages/sec throughput. |
...
|-----------|----------------------|--------------------------|---------|
| Lookup user by ID | Array O(n): 500Î¼s | HashMap O(1): 0.1Î¼s | 5000Ã— |
| Deduplicate cart items | Nested loop O(nÂ²): 25Î¼s | HashSet O(1): 0.5Î¼s | 50Ã— |
| Sort by price | Bubble O(nÂ²): 100Î¼s | Merge O(n log n): 10Î¼s | 10Ã— |
| Find cheapest item | Full scan O(n): 5Î¼s | Min-Heap O(1): 0.1Î¼s | 50Ã— |
| Validate promo codes | Linear O(n): 20Î¼s | Trie O(L): 0.5Î¼s | 40Ã— |

**Total:** Naive = 650Î¼s per order â†’ 650 ms for 1000 orders (cannot keep up). Optimized = 11.2Î¼s â†’ 11.2 ms. The optimized system handles **58Ã— more throughput** â€” complexity analysis made this possible.
...
| Mistake | Why It's Wrong | Correct Approach |
|---------|----------------|------------------|
| Claiming O(n + nÂ²) = O(n) | nÂ² dominates n asymptotically | O(nÂ²) â€” keep only highest-order term |
| Saying O(2n) = O(2n) | Constants are dropped in asymptotic notation | O(n) â€” 2n is still linear growth |
| Confusing Î©(n) with worst-case | Î© describes lower bound, not worst-case input | Î©(1) is best-case for linear search; O(n) is worst-case |
| Ignoring recursion stack in space analysis | The call stack uses O(depth) memory | Always count stack frames â€” factorial recursive uses O(n) space |
| Assuming Î˜(n) when best-case is better | Î˜ requires matching upper AND lower bounds | Use O(n) worst-case, Î©(1) best-case â€” no tight bound exists |
| Saying "log n" without specifying base | In CS, log means logâ‚‚ unless stated | Binary search on 10â¶ takes logâ‚‚ 10â¶ â‰ˆ 20 steps |
| Forgetting that Big-O is an upper bound | O(nÂ²) is technically correct for linear search too | Always give the tightest meaningful bound |
...
```typescript
/**
 * Runtime complexity analyzer â€” measures execution time of functions
...
function masterTheorem(a: number, b: number, fExponent: number): string {
    ...
    if (fExponent < logBA - epsilon) {
        return `Î˜(n^${logBA.toFixed(2)})`; // Case 1
    } else if (Math.abs(fExponent - logBA) < epsilon) {
        return `Î˜(n^${logBA.toFixed(2)} log n)`; // Case 2
    } else if (fExponent > logBA + epsilon) {
        return `Î˜(n^${fExponent})`; // Case 3
    }
    return "Need regularity condition check";
}

// Examples:
console.log("T(n) = 2T(n/2) + n â†’", masterTheorem(2, 2, 1));    // Merge sort: Î˜(n log n)
console.log("T(n) = T(n/2) + 1 â†’", masterTheorem(1, 2, 0));     // Binary search: Î˜(log n)
console.log("T(n) = 2T(n/2) + 1 â†’", masterTheorem(2, 2, 0));    // Tree traversal: Î˜(n)
console.log("T(n) = 3T(n/2) + n â†’", masterTheorem(3, 2, 1));    // Karatsuba-like: Î˜(n^1.58)
```
...
10. **Which of the following is true about amortized analysis?**
    ...
    - b) It averages cost over a sequence of operations âœ“
    - c) It ignores the cost of resizing
    - d) It is the same as average-case analysis

11. **An algorithm runs in 2Î¼s for n=100 and 8Î¼s for n=200. Its complexity is likely:**
    - a) O(1)
    - b) O(log n)
    - c) O(n) âœ“
    - d) O(nÂ²)
...
14. **The inverse Ackermann function Î±(n) appears in which data structure's complexity?**
    - a) Hash Table
    - b) Disjoint Set Union âœ“
...
**15.** What is the worst-case time complexity of searching in a hash table with chaining where load factor Î± = n/m? Explain why.

**16.** Write a TypeScript function to verify whether a given code snippet runs in O(n), O(n log n), or O(nÂ²) by measuring execution time at different input sizes.
...
**19.** An algorithm has time complexity T(n) = 4T(n/2) + nÂ². Apply the Master Theorem and determine its complexity class.
...
**21.** Prove that for any comparison-based sorting algorithm, the worst-case time complexity is Î©(n log n).

**22.** Given two algorithms with complexities O(nÂ²) and O(n log n), for n = 10â¶, estimate how many times faster the O(n log n) algorithm is (assume each operation takes 1ns).

**23.** Implement a TypeScript decorator `@logComplexity` that automatically logs execution time for any method it decorates.

**24.** Write a brief analysis: Why does the "two-pointer technique" for finding a pair with given sum in a sorted array achieve O(n) time instead of O(nÂ²)?

**Answers for exercises 14-24:** (14) O(n) â€” the recurrence is T(n)=2T(n/2)+O(1), case 1 of Master Theorem gives Î˜(n). (15) O(1+Î±) average, O(n) worst-case. (16-24) Solutions available in instructor materials.
...
**Little-o** is like saying "strictly less than." If Big-O says "â‰¤ 100 kg," little-o says "< 100 kg" â€” the algorithm is *strictly* faster than the bound, not just equal to it. **Little-omega** is the strict greater-than counterpart.

### Definition

- **Little-o**: \( f(n) = o(g(n)) \) means for *every* constant \( c > 0 \), there exists \( n_0 > 0 \) such that \( 0 \le f(n) < c \cdot g(n) \) for all \( n \ge n_0 \). The bound does not hold for any constant â€” \( f(n) \) grows strictly slower than \( g(n) \).
...
|----------|---------|---------|
| \( f(n) = O(g(n)) \) | \( f \) grows no faster than \( g \) | \( n = O(n^2) \) âœ“ |
| \( f(n) = o(g(n)) \) | \( f \) grows strictly slower than \( g \) | \( n = o(n^2) \) âœ“, but \( n^2 \neq o(n^2) \) |
| \( f(n) = \Omega(g(n)) \) | \( f \) grows no slower than \( g \) | \( n^2 = \Omega(n) \) âœ“ |
| \( f(n) = \omega(g(n)) \) | \( f \) grows strictly faster than \( g \) | \( n^2 = \omega(n) \) âœ“, but \( n \neq \omega(n) \) |

### Dry Run â€” Comparing Growth Rates

| Function Pair | Big-O? | Little-o? | Why |
|---------------|--------|-----------|-----|
| \( f(n)=n, g(n)=n^2 \) | \( n = O(n^2) \) âœ“ | \( n = o(n^2) \) âœ“ | n grows slower; for c=1, n < nÂ² for n>1 |
| \( f(n)=2n, g(n)=n \) | \( 2n = O(n) \) âœ“ | \( 2n \neq o(n) \) âœ— | No c makes 2n < cn for all n (pick c=3? 2n<3n holds, but need for *every* c) |
| \( f(n)=n^2, g(n)=n^2 \) | \( n^2 = O(n^2) \) âœ“ | \( n^2 \neq o(n^2) \) âœ— | Equal growth; cannot find c<1 to make nÂ² < cnÂ² |
...
### TypeScript â€” Growth Rate Classifier
...
function classifyGrowth(
    ...
    if (trend < 0.1) return "f = o(g) â€” f grows strictly slower";
    if (trend > 10) return "f = Ï‰(g) â€” f grows strictly faster";
    if (Math.abs(trend - 1) < 0.1) return "f = Î˜(g) â€” same growth rate";
    return "f = O(g) but not o(g) â€” needs more data";
}

// Example: Compare n vs nÂ²
const f = (n: number) => n;
...
15. **If f(n) = nÂ² and g(n) = nÂ³, which statement is true?**
    - a) f = o(g) âœ“
    - b) g = o(f)
    - c) f = Î˜(g)
    - d) f = Ï‰(g)
...
17. **The statement "n = o(nÂ²)" is:**
    - a) True âœ“
    - b) False
    - c) Neither true nor false
    - d) Only true for n â‰¥ 2

18. **For which pair does f(n) = Ï‰(g(n)) hold?**
    - a) f = n, g = nÂ²
    - b) f = nÂ², g = n âœ“
    - c) f = n, g = n
    - d) f = log n, g = n
...
Imagine you have a **jigsaw puzzle** with 10,000 pieces. Checking whether someone's proposed solution is correct is easy (O(n) â€” just look at the picture). But *finding* the solution from scratch is exponentially hard. NP-complete problems are exactly this: solutions are easy to verify, but hard to find.
...
flowchart TD
    subgraph "Complexity Zoo"
        P["<b>P</b><br/>Polynomial Time<br/>O(n), O(nÂ²), O(nÂ³)...<br/>Solved efficiently"]
        NP["<b>NP</b><br/>Nondeterministic Polynomial<br/>Verified in polynomial time<br/>SAT, TSP, Knapsack"]
        NPC["<b>NP-Complete</b><br/>Hardest in NP<br/>SAT â†’ 3SAT â†’ Vertex Cover<br/>â†’ Ham Path â†’ TSP"]
        NPH["<b>NP-Hard</b><br/>At least as hard as NP<br/>(may not be in NP)<br/>Halting Problem"]
        EXP["<b>EXP</b><br/>Exponential Time<br/>O(2â¿), O(n!)<br/>Intractable for large n"]
    end
...
| **EXP** | Exponential Time | Solvable in O(2^{n^k}) time | Generalized Chess (nÃ—n board) |

### The P vs NP Question

**The million-dollar question:** Is P = NP? If true, every problem whose solution can be quickly verified can also be quickly solved. Most computer scientists believe P â‰  NP.
...
### Dry Run â€” Reducing 3SAT to Vertex Cover

| Step | Transformation | Complexity |
|------|---------------|------------|
| 1 | Start with 3SAT formula Ï† with n variables, m clauses | O(n + m) |
| 2 | Create a variable gadget: for each variable x, create edge (x, Â¬x) | O(n) |
...
| 6 | Ï† is satisfiable iff graph has a vertex cover of size k | â€” |
...
| NP-Complete (small n) | O(2â¿) for n â‰¤ 30 | Branch and bound + pruning |
...
19. **Which is true about NP-Complete problems?**
    ...
    - b) They are the hardest problems in NP âœ“
    - c) They cannot be verified in polynomial time
    - d) They never have practical solutions

20. **If problem A reduces to problem B in polynomial time and B âˆˆ P, then:**
    - a) A âˆˆ P âœ“
    - b) A âˆˆ NP-Complete
    - c) B âˆˆ NP-Complete
    - d) A is unsolvable
...
27. **Explain why the Traveling Salesman Problem (TSP) optimization version is NP-Hard but the decision version ("Is there a tour of length â‰¤ k?") is NP-Complete.**
...
29. **For n=20, an O(2â¿) algorithm takes ~1 second. How long would it take for n=30? n=40? Explain why exponential algorithms become infeasible so quickly.**

30. **Implement a TypeScript function that checks whether a given graph coloring (assignment of colors to vertices) is valid â€” and explain why this verification is in P while finding the minimum coloring is NP-Hard.**
...
```mermaid
flowchart TD
    Start["Given a computational problem"] --> Q1["Is input size small<br/>(n â‰¤ 1000)?"]
    Q1 -->|Yes| Q2["Is exact solution needed?"]
    Q1 -->|No| Q3["Is the problem in P?"]
    
    Q2 -->|"Yes"| Exact["Use exact algorithm<br/>O(nÂ³) or better acceptable"]
    Q2 -->|"No"| Approx["Use approximation<br/>or heuristic"]
    
    Q3 -->|"Yes"| Poly["Use polynomial algorithm<br/>O(n), O(n log n), O(nÂ²)"]
    Q3 -->|"No (NP-Hard)"| Q4["How large is n?"]
    
    Q4 -->|"n â‰¤ 30"| Branch["Branch and bound<br/>with pruning"]
    Q4 -->|"n â‰¤ 500"| Approx2["Approximation algorithm<br/>with guarantee"]
    Q4 -->|"n â‰¥ 500"| Heuristic["Heuristic / Metaheuristic<br/>GA, SA, Local Search"]
    ...
    Choose --> DS["Map to optimal DS:<br/>â€¢ Array: O(1) access<br/>â€¢ Hash: O(1) lookup<br/>â€¢ Heap: O(log n) min<br/>â€¢ BST: O(log n) search"]
```
...
11. Compare O(1) space but O(nÂ²) time vs O(n) space but O(n) time for finding the first duplicate in an array. Which would you choose for a mobile app? For a server processing 1M requests/sec?

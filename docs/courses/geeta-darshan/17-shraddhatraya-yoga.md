...
```mermaid
flowchart TB
    A["Manushya: shraddhamay purush"]
    B["Shraddha: andar ki gahri parat"]
    C["Sattvic: jagrukta; prasannata"]
    D["Rajasic: bhaag-daud; chinta"]
    E["Tamasic: nidra; anchal; jadta"]
    F["Intake: bhojan; media; sangati; vichaar; vaani"]
    G["Guna: roz ka rang; karmon ka rang"]
    A --> B
    B --> C
    B --> D
    B --> E
...
```mermaid
flowchart TB
    S["Subah: aaj ka intake tay karo"]
    T["Har intake ko guna do: sattvic; rajasic; tamasic"]
    U["Din bhar: khaya; padha; suna; bola"]
    V["Shaam: din ka intake-log likho"]
    W["Kal ka plan: ek sattvic badlaav"]
    X["30 din baad: dekho; chetna ka rang badla ya nahin"]
    S --> T --> U --> V --> W --> X
```

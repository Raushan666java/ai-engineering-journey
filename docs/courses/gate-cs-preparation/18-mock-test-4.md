...
**Q16 (1 Mark):** If A is a 5Ã—5 matrix with det(A) = âˆ’3, then det(2A) = ?
...
**Q18 (1 Mark):** âˆ«â‚€^Ï€ sinÂ² x dx equals:
...
**Q19 (1 Mark):** If X ~ N(Î¼, ÏƒÂ²), then the distribution of Z = (X âˆ’ Î¼)/Ïƒ is:
...
**Q24 (2 Marks):** A continuous random variable X has PDF f(x) = kxÂ² for 0 â‰¤ x â‰¤ 2. The value of k is:
...
**Q25 (2 Marks):** Solve T(n) = T(âˆšn) + 1, with T(2) = 0. Take n = 2^{2^k}. T(n) = ?
...
**Q30 (1 Mark) [TOC]:** The language L = {aâ¿bâ¿câ¿ | n â‰¥ 0} belongs to which class?
...
**Q48 (2 Marks) [CN]:** A 10 Mbps CSMA/CD network has a maximum cable length of 2000 m. Signal propagation speed is 2Ã—10â¸ m/s. The minimum frame size in bits is:
...
**Q5:** All engineers are graduates (set E âŠ† G). Some graduates are unemployed (G âˆ© U â‰  âˆ…). This does not guarantee E âˆ© U â‰  âˆ…, so conclusion I does not follow. But "Some unemployed persons are graduates" is the same as "Some graduates are unemployed" (conversely), so II follows. Only II follows.

**Q6:** LCM(1,2,3,4,5,6) = LCM(2Â², 3, 5) = 4 Ã— 3 Ã— 5 = 60.

**Q7:** Pattern: each term = previous Ã— 2 + 1. 6Ã—2+1=13, 13Ã—2+1=27, 27Ã—2+1=55, 55Ã—2+1=111, 111Ã—2+1=223.
...
**Q30:** aâ¿bâ¿câ¿ requires counting and comparing three sequences. It cannot be recognized by a PDA (which has only one stack), so it's not context-free. It can be recognized by a Linear Bounded Automaton, so it is context-sensitive.
...
**Q38:** a*b* generates strings with any number of a's followed by any number of b's. Îµ, aab, and ab all follow this pattern. bba has b before a, which is not generated.
...
**Q40:** A 4-bit ripple counter has 2â´ = 16 distinct states (0000 through 1111).
...
**Q54:** A modulo-5 counter counts from 0 to 4 (5 states). Minimum flip-flops = âŒˆlogâ‚‚(5)âŒ‰ = âŒˆ2.32âŒ‰ = 3.

**Q55:** Packet size = 2000 bytes = 16000 bits. Transmission time = 16000 / 10â¹ = 16 Î¼s. RTT = 2 Ã— 20 ms = 40 ms = 40000 Î¼s. Utilization = T_tx / (T_tx + RTT) = 16 / (16 + 40000) â‰ˆ 16/40016 â‰ˆ 0.0004 = 0.04%.

Hmm but the answer key says B. Let me check if they use a different packet size interpretation. If packet = 1000 bytes = 8000 bits. At 1 Gbps: time = 8 Î¼s. Round trip propagation = 2 Ã— 20 ms = 40 ms = 40000 Î¼s. Efficiency = 8/(8 + 40000) = 8/40008 = 0.00019997 â‰ˆ 0.0002 = 0.02%. So answer should be A. Let me fix the answer key to A.

Wait, maybe they consider propagation delay differently. Or maybe they use 1000 bytes = 1000 Ã— 10 = 10000 bits for some reason? No.

Let me adjust to make the answer cleaner. With 2000 bytes: transmission = 16000/10â¹ = 16 Î¼s. Efficiency = 16/40016 = 0.0004 = 0.04%. That gives B. Let me change the packet size to 2000 bytes in the question.

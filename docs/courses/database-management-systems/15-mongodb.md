...
STEP 3: Server validates document size â‰¤ 16MB BSON limit
...
- The fanout factor (â‰ˆ 100-500 keys per node for 8KB pages) keeps tree height at 3-4 levels for billions of documents.
...
| 2      | Validate document (size â‰¤ 16MB)          | 112 bytes â€” OK   |
...
STEP 5: B-tree index traversed: root â†’ internal â†’ leaf node
        â‰ˆ 3-4 I/O operations for billions of documents
...
Index states during step 6:
  ...
  Step 6b: Check "alice.new@example.com" not in tree âœ“
...
| **Index lookup (equality)** | O(log n) | O(1) | B-tree height â‰ˆ log_f(n); f = fanout (â‰ˆ 500) |
...
|        | revenue=Î£(qty*price), units=Î£(qty)      |  revenue:500, units:2}     |
...
STEP 7: Secondary reports its oplog lag back to primary
        - Lag = primary's last optime âˆ’ secondary's last optime
...
| **Heartbeat** | O(VÂ²) network | O(1) | Every node heartbeats every other node |
...
// Chunks: [("AF",-âˆž) â†’ ("AF","user5")], [("AF","user5") â†’ ("IN","user100")], ...

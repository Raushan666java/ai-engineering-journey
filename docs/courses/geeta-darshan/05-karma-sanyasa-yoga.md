...
```mermaid
flowchart TB
    E["Din ke chaar prakar ke events"] --> P["Prashansa mili"]
    E --> N["Ninda mili"]
    E --> S["Sukh ka pal"]
    E --> D["Dukh ka pal"]
    P --> R1["Reaction ka score; 0 se 10 tak doori"]
    N --> R1
    S --> R1
    D --> R1
    R1 --> W["Hafta ka equanimity index"]
    W --> C["Osho ki tippani; kam doori matlab ego soya; zyada doori matlab jagruti ki parchhi"]
    style C fill:#4446e8,color:#fff
    style W fill:#1a1b23,color:#fff
```

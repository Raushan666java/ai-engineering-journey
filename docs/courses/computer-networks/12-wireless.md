...
|----------|-----------|------|------------|---------------|--------------|-------------|------------|-------------|----------------|
| 802.11a | WiFi 2 | 1999 | 5 | 20 MHz | 54 Mbps | 1Ãƒâ€”1 (no MIMO) | OFDM (64-subcarrier) | 52 data + 4 pilot | 800 ns |
| 802.11b | WiFi 1 | 1999 | 2.4 | 20 MHz | 11 Mbps | 1Ãƒâ€”1 (no MIMO) | DSSS (CCK) | N/A (DSSS) | N/A |
| 802.11g | WiFi 3 | 2003 | 2.4 | 20 MHz | 54 Mbps | 1Ãƒâ€”1 (no MIMO) | OFDM (64-subcarrier) | 52 data + 4 pilot | 800 ns |
| 802.11n | WiFi 4 | 2009 | 2.4/5 | 20/40 MHz | 600 Mbps | 4Ãƒâ€”4 | MIMO-OFDM (64-subcarrier) | 52/108 data | 400/800 ns |
| 802.11ac | WiFi 5 | 2013 | 5 | 20/40/80/160 MHz | 6.9 Gbps | 8Ãƒâ€”8 (MU-MIMO DL) | MIMO-OFDM (256-subcarrier) | 234/468 | 400/800 ns |
| 802.11ax | WiFi 6 | 2019 | 2.4/5/6 | 20/40/80/160 MHz | 9.6 Gbps | 8Ãƒâ€”8 (MU-MIMO DL+UL) | OFDMA (1024-QAM) | 234/468/936/1872 | 800/1600/3200 ns |
| 802.11be | WiFi 7 | 2024 | 2.4/5/6 | 20/40/80/160/320 MHz | 46 Gbps | 16Ãƒâ€”16 (CMU-MIMO) | 4096-QAM OFDMA | 468/936/1872/3744 | 800/1600/3200 ns |
...
**Why WiFi 7 reaches 46 Gbps**: 320 MHz Ãƒâ€” 16 streams Ãƒâ€” 4096-QAM (12 bits/symbol) Ãƒâ€” 5/6 code rate Ãƒâ€” OFDMA efficiency Ã¢â€°Ë† 46 Gbps. The 320 MHz channel alone (doubled from WiFi 6's 160 MHz) doubles throughput. 16Ãƒâ€”16 MIMO doubles it again vs WiFi 6's 8Ãƒâ€”8. 4096-QAM adds ~20% more bits per symbol than 1024-QAM.
...
| 802.11n | First MIMO (4Ãƒâ€”4), dual-band, 40 MHz channels | Legacy preamble reduces efficiency with mixed clients |
...
| 802.11be (WiFi 7) | 320 MHz, 4096-QAM, 16Ãƒâ€”16 MIMO, multi-link, 46 Gbps | Early adoption, expensive hardware, backward compatibility overhead |
...
- **SIFS (Short IFS)**: 10 Ã‚Âµs (802.11b/g) or 16 Ã‚Âµs (802.11a/n/ac/ax). Used for ACK, CTS, and data frame bursts.
- **DIFS (DCF IFS)**: 50 Ã‚Âµs (802.11b) or 34 Ã‚Âµs (802.11a/g). SIFS + 2 Ãƒâ€” slot time. Stations wait DIFS before starting backoff.
...
1. **Physical Carrier Sense**: Check if the medium is idle for DIFS duration (34 Ã‚Âµs for 802.11a/g).
...
3. **Backoff Countdown**: Decrement $B$ each idle slot time (9 Ã‚Âµs for 802.11a/g). If medium becomes busy, freeze $B$.
4. **Transmission**: When $B = 0$, transmit the frame.
5. **ACK Wait**: Wait for ACK within SIFS + ACK_Timeout (typically ~50 Ã‚Âµs).
...
Parameters: Slot time = 9 Ã‚Âµs, DIFS = 34 Ã‚Âµs, SIFS = 16 Ã‚Âµs, CW_min = 15, CW_max = 1023.
Data frame transmission time = 200 Ã‚Âµs (1500-byte frame at 54 Mbps). ACK = 30 Ã‚Âµs.

Three stations (A, B, C) all have a frame to send. The medium starts idle.

| Time (Ã‚Âµs) | Event | Station A Backoff | Station B Backoff | Station C Backoff | Medium State |
|-----------|-------|------------------|------------------|------------------|-------------|
| 0 | DIFS idle period starts | â†’ | â†’ | â†’ | IDLE |
| 34 | DIFS ends, all pick backoff | B=7 | B=3 | B=12 | IDLE |
| 43 | Slot 1 decrement (9 Ã‚Âµs) | 6 | 2 | 11 | IDLE |
...
| 277 | SIFS (16 Ã‚Âµs) | 4 (frozen) | â†’ | 9 (frozen) | IDLE (ACK) |
| 307 | ACK from B's AP | 4 (frozen) | â†’ | 9 (frozen) | IDLE |
| 341 | DIFS (34 Ã‚Âµs) after ACK | â†’ | â†’ | â†’ | IDLE |
...
Total time for 3 stations: 672 Ã‚Âµs. Without CSMA/CA (all transmitting simultaneously), all 3 frames would collide. CSMA/CA serialized the transmissions with ~100% channel utilization (minus overhead).
...
class CSMACASimulator {
  ...
  private readonly slotTime: number = 9;  // Âµs
  ...
  private step(): void {
    ...
    if (ready.length === 1) {
      ...
      console.log(`[T=${this.currentTime}Âµs] STA${s.id} transmits SUCCESS (frames=${s.framesSent})`);
    } else if (ready.length > 1) {
      for (const s of ready) {
        s.collisions++;
        s.cw = Math.min(s.cw * 2, this.cwMax);
        s.backoff = Math.floor(Math.random() * (s.cw + 1));
        console.log(`[T=${this.currentTime}Âµs] COLLISION STA${s.id} new CW=${s.cw} backoff=${s.backoff}`);
      }
      this.mediumBusy = true;
      this.busyUntil = this.currentTime + this.frameTime;
    }
  ...
  report(): void {
    ...
    console.log(`Time: ${this.currentTime}Âµs | Total sent: ${totalSent} | Collisions: ${totalColl}`);
...
/*
Output:
[T=...Âµs] STA... transmits SUCCESS (frames=1)
...
=== CSMA/CA REPORT ===
Time: 5000Âµs | Total sent: 3 | Collisions: 0
...
| Duration | 2 B | NAV reservation time (Ã‚Âµs) or STA ID (PS-Poll) |
...
2. **Receiver CTS**: Receiver waits SIFS (16 Ã‚Âµs), transmits CTS (14 bytes) to sender. CTS copies the Duration field, subtracting SIFS + CTS time. All other stations that hear CTS set their NAV.
...
**When RTS/CTS is used**: RTS threshold is typically 2346 bytes (disabled by default on many devices). For small frames, RTS/CTS overhead (RTS + CTS Ã¢â€°Ë† 34 bytes + 2Ãƒâ€”SIFS = 52 Ã‚Âµs plus contention) exceeds its benefit. RTS/CTS is recommended in dense or high-interference environments.
...
| Time (Ã‚Âµs) | Sender | Frame | Duration (Ã‚Âµs) | NAV at A | NAV at AP | NAV at C | Notes |
|-----------|--------|-------|--------------|----------|-----------|----------|-------|
...
Duration calculation: RTS duration = Data(338) + SIFS(16) + ACK(30) + 3Ãƒâ€”SIFS(48) = 400 Ã‚Âµs.
CTS duration = 400 - SIFS(16) - CTS_time(14B @ 54 Mbps Ã¢â€°Ë† 4 Ã‚Âµs) Ã¢â€°Ë† 370 Ã‚Âµs (rounding).

During the entire exchange (0-370 Ã‚Âµs), Station C defers because its NAV > 0. The hidden terminal collision is avoided.
...
**Non-RTS/CTS Hidden Terminal**: Without RTS/CTS, hidden stations A and C transmit data frames simultaneously. Both frames collide at the AP. Neither A nor C receives an ACK. Both double CW and retry. They may collide again if they pick the same backoff value (probability Ã¢â€°Ë† 1/CW). With CW=15, P(same backoff) Ã¢â€°Ë† 6.7%.
...
- Time is divided into 625 Ã‚Âµs slots (1600 slots/second).
...
- Bluetooth 5: 2 Mbps (LE 2M PHY), 4Ãƒâ€” range, advertising extensions, mesh networking.
...
**OFDMA in LTE Downlink**: 15 kHz subcarrier spacing. Resource Block = 12 subcarriers Ãƒâ€” 7 OFDM symbols (normal CP) = 84 REs. For 20 MHz bandwidth: 100 RBs available. Each RB carries 84 symbols. With 64-QAM (6 bits/symbol) and code rate 3/4: 84 Ãƒâ€” 6 Ãƒâ€” 0.75 = 378 bits per RB per 0.5 ms slot.
...
| mMTC | â†’ | â†’ | â†’ | 1M/kmÃ‚Â² | IoT sensors, smart city |

**Flexible Numerology**: Subcarrier spacing = $15 \times 2^\mu$ kHz.

| $\mu$ | SCS (kHz) | Symbol Duration (Ã‚Âµs) | Slot Duration (ms) | Max BW (MHz) | Use Case |
|-------|-----------|---------------------|-------------------|-------------|----------|
...
| MIMO | Up to 8Ãƒâ€”8 DL, 4Ãƒâ€”4 UL | Up to 64Ãƒâ€”64 (massive MIMO) |
...
| Energy efficiency | Reference | 100Ãƒâ€” better per bit (target) |
...
| Handover | Roaming between APs (Ã¢â€°Ë†100 ms) | Connection transfer (Ã¢â€°Ë†3 s) | Seamless handover (<50 ms 5G) |
...
|-------------|-------------|-----|
| File transfer at close range (home) | WiFi (802.11ac/ax) | 100-1000Ãƒâ€” faster than Bluetooth |
...
- **Spatial multiplexing**: Multiple data streams transmitted simultaneously over the same frequency â†’ throughput increases linearly with stream count (up to 8Ãƒâ€” in WiFi 6, 16Ãƒâ€” in WiFi 7, 64Ãƒâ€” in 5G).
...
Real-world benefit: In a crowded office, OFDMA handles many small packets efficiently. A 20 MHz OFDMA transmission can serve 9 devices simultaneously instead of serializing them (9Ãƒâ€” latency reduction for small frames).
...
**LoRaWAN**: Long-range (2-15 km), sub-GHz (868/915 MHz), ultra-low power (10+ years on battery). Data rate: 0.25-50 kbps. Use case: smart city parking sensors that report occupancy every 5 minutes. Each transmission costs ~0.01 Ã‚ÂµAh. A single gateway covers an entire city district.
...
| 802.11n | 2009 | 600 Mbps | 4Ãƒâ€”4 | 40 MHz | MIMO-OFDM | Dual-band, MIMO |
| 802.11ac | 2013 | 6.9 Gbps | 8Ãƒâ€”8 | 160 MHz | MU-MIMO | 5 GHz Gigabit WiFi |
| 802.11ax (WiFi 6) | 2019 | 9.6 Gbps | 8Ãƒâ€”8 | 160 MHz | OFDMA + MU-MIMO | Efficient dense deployment |
| 802.11be (WiFi 7) | 2024 | 46 Gbps | 16Ãƒâ€”16 | 320 MHz | 4096-QAM, multi-link | Ultra-high throughput |
...
| AP placement follows cell tower principles â€” hexagonal grid with overlap | Space APs so cell edges have -65 dBm RSSI; 15-20% overlap for seamless handoff |
...
|----------|--------|-------------|
| Q1 | B | Wireless transceivers are half-duplex â€” transmitting drowns out incoming signals, making collision detection impossible; CSMA/CA prevents collisions proactively |
...
4. Network slicing creates isolated end-to-end logical networks with dedicated resources and SLAs on shared physical infrastructure. It solves the problem of different services (eMBB, uRLLC, mMTC) having conflicting requirements â€” each slice gets tailored QoS, security, and management.
...
8. TIM (Traffic Indication Map) is a bitmap in each beacon frame indicating which stations have buffered data at the AP. Stations wake at each beacon interval to check the TIM â€” if their bit is set, they send PS-Poll to retrieve buffered frames.
...
10. eMBB (enhanced Mobile Broadband: 20 Gbps, video/AR/VR), uRLLC (ultra-Reliable Low-Latency: 1 ms, 99.999%, industrial/autonomous), mMTC (massive Machine Type: 1M devices/kmÂ², IoT sensors).
</details>
...
11. Offered load = 20 Ã— 50 Ã— 1500 Ã— 8 = 12 Mbps. The channel at 300 Mbps can handle this easily (4% utilization). Under CSMA/CA, throughput is close to the offered load since contention is low. Saturation occurs above ~200 Mbps offered load.

12. For 100 MHz: 100 RBGs Ã— 12 subcarriers Ã— 14 symbols Ã— 64 MIMO layers Ã— 6 bits/symbol Ã— 7/8 code rate â‰ˆ 5.6 Gbps (simplified; actual includes control overhead). Formula: N_RB Ã— N_sc Ã— N_sym Ã— N_layers Ã— log2(M) Ã— code_rate Ã— 1000 slots/s.

13. Cell circumference = 2Ï€ Ã— 500 = 3141 m. At 60 km/h = 16.67 m/s, crossing time = 3141/16.67 â‰ˆ 188 s. TAU rate per user = speed/(TA diameter) per sec. For 20 cells per TA and 1000 users/cell, total users in TA = 20,000. Paging load = 20,000 Ã— 0.1 calls/hour / 3600 = 0.56 pages/second/cell.

14. See the dry run trace in section 12.4.5 for the full simulation: B wins at 61 Âµs, A retransmits with CW=31 (backoff chosen uniformly from 0-31), and the sequence continues. The trace shows collisions â‰ˆ (1/CW) probability per contention round.

15. RTS/CTS overhead: RTS(20B) + CTS(14B) + 2Ã—SIFS(32Âµs) = 34 bytes + 32 Âµs. At 54 Mbps, 34 bytes = 5 Âµs. Total overhead â‰ˆ 37 Âµs. Without RTS/CTS, each collision costs a full data frame (1500 bytes â‰ˆ 222 Âµs). With 3 stations and avg 2 retries, collision overhead = 3 Ã— 222 Ã— 2 = 1332 Âµs vs RTS/CTS overhead = 37 Âµs per successful exchange. RTS/CTS becomes beneficial at high contention.
</details>

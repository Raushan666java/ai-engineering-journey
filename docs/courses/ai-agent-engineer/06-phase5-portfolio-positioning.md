...
**Live projects:**
- ?? [RAG Memory API](https://rag-demo.apexpillar.tech) â€” document Q&A with cited sources
- ?? [LangGraph Agent Pipeline](https://github.com/yourname/purvanchal-langgraph) â€” multi-node agent state machine
- ?? [Booking Module (FastAPI)](https://github.com/yourname/booking-api) â€” async real-estate booking port

**Stack:** Python Â· FastAPI Â· LangGraph Â· CrewAI Â· MCP Â· ChromaDB Â· Qdrant Â· Docker Â· Redis

?? [LinkedIn](https://linkedin.com/in/yourprofile) Â· [Upwork](https://upwork.com/freelancers/yourprofile)
```
...
Â¦   +- Simple ? LLM Router pattern (just if-else tool dispatch)
Â¦   +- Complex ? LangGraph (checkpoints, human-in-loop, cycles)
Â¦
+- Multi-agent team
Â¦   +- Role-based, protoype fast ? CrewAI
Â¦   +- Enterprise, .NET ? Microsoft Agent Framework (MAF)
Â¦

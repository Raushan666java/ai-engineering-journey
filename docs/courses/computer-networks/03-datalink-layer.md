...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/03-datalink-layer/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/03-datalink-layer/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/03-datalink-layer/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/03-datalink-layer/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/03-datalink-layer/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/03-datalink-layer/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
| Error Detection | CRC-32 catches all bursts Ã¢â€°Â¤ 32 bits | Use CRC for link-layer integrity; checksums (Internet) are weaker but simpler |
...
stateDiagram-v2
    ...
    Checking --> Idle : zero remainder â†’ accept
    Checking --> Error : non-zero remainder â†’ reject
    ...
    state Shifting {
        ...
        XOR --> ShiftIn : moduloâ€‘2 subtract
    }
```
...
| 12 | 0x7E | End flag â†’ deliver payload | `[41,42,7E,43,44,7D,45,46]` Ã¢Å“â€œ |
...
| **Worst-case expansion** | None | 2Ãƒâ€” (payload all 0x7E) | 1.2Ãƒâ€” (payload all 1s) |
...
Two-dimensional parity arranges data in a matrix (rows Ãƒâ€” columns) and computes parity for each row and each column.

Example: 4Ãƒâ€”4 data matrix
```
...
| 2D parity | O(n) | O(Ã¢Ë†Å¡n) | Row and column parity buffers |
...
| Verification sum: 0x0102 + 0x0304 + 0x0506 + 0x0708 + 0xEFEB | | 0x1 0000 â†’ wrap: 0x0000 + 0x0001 = 0xFFFF Ã¢Å“â€œ |
...
#### Worked Example: CRC-3 with G(x) = xÃ‚Â³ + x + 1 (binary 1011)
...
| Operation | Time | Space | Why |
|-----------|------|-------|-----|
| CRC computation (software, bit-at-a-time) | O(nÃ‚Â·k) | O(1) | Each byte requires k iterations of bit shifts |
| CRC computation (lookup table) | O(n) | O(256Ã‚Â·k) bits | Precomputed table trades memory for speed |
...
| Aspect | Detail |
|--------|--------|
| Advantage | Detects ALL bursts of length Ã¢â€°Â¤ k (where k = degree of generator) |
...
Compute parity:
- p1 covers {1,3,5,7}: p1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 Ã¢Å â€¢ 0 = 0 â†’ p1 = 1
- p2 covers {2,3,6,7}: p2 Ã¢Å â€¢ 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 = 0 â†’ p2 = 0
- p4 covers {4,5,6,7}: p4 Ã¢Å â€¢ 0 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 = 0 â†’ p4 = 1
...
- p1 check: 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 = 1 â†’ fails (syndrome bit 0 = 1)
- p2 check: 0 Ã¢Å â€¢ 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 = 0 â†’ passes (syndrome bit 1 = 0)
- p4 check: 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 1 Ã¢Å â€¢ 0 = 1 â†’ fails (syndrome bit 2 = 1)
...
| Operation | Time | Space | Why |
|-----------|------|-------|-----|
| Encoding | O(rÃ‚Â·n) = O(log n Ã‚Â· n) | O(n) | For each parity bit r, scan all n positions |
| Syndrome calculation | O(rÃ‚Â·n) = O(log n Ã‚Â· n) | O(1) | Recompute parity for each of r groups |
...
**Why O(rÃ‚Â·n):** Each of the $r \approx \log_2 n$ parity groups covers roughly half the codeword. So each group requires scanning $n/2$ positions on average â†’ total $O(r \cdot n/2) = O(n \log n)$. For (7,4): r=3, n=7 â†’ 21 parity checks total.
...
| Selective Repeat | $2^{k-1}$ | Must satisfy SWS + RWS Ã¢â€°Â¤ $2^k$ to prevent window overlap |
...
| Bandwidth wasted per error | O(NÃ‚Â·L) | â†’ | Retransmits N frames Ãƒâ€” L bits each |
...
| Condition | Choose |
|-----------|--------|
| Low error rate (< 10Ã¢ÂÂ»Ã¢ÂÂ¶ BER) | Go-Back-N (simpler, lower overhead) |
...
| Window small (N Ã¢â€°Â¤ 4) | Go-Back-N (cost of full retransmission is low) |

The crossover point is roughly when retransmission cost (N Ãƒâ€” frame size) exceeds buffer cost (N Ãƒâ€” frame buffer at receiver).
...
A (7,4) Hamming code has minimum Hamming distance $d_{\min} = 3$. To correct up to $t$ errors, we need $d_{\min} \ge 2t + 1$. For $t=1$: $3 \ge 3$ Ã¢Å“â€œ. For $t=2$: $3 \ge 5$ Ã¢Å“â€”. To correct two errors, we need $d_{\min} = 5$, achieved by increasing redundancy. The extended Hamming (8,4) code has $d_{\min} = 4$ (detects 2, corrects 1). True double-error correction requires BCH codes or a longer Hamming code with more parity bits.

### Q7: Why 2Ã‚Â³Ã‚Â¹ - 1 maximum window for Go-Back-N but 2Ã‚Â³Ã¢ÂÂ° for Selective Repeat?
...
2. Station B responds with an ACK frame after a Short Interframe Space (SIFS, 10 Ã‚Âµs for 802.11g).
...
| CRC-32 | Polynomial division remainder | Detects all bursts Ã¢â€°Â¤ 32 bits | Ethernet, Wi-Fi |
...
| Category | Key Points |
|----------|------------|
| **Framing Methods** | Character count (fragile), Byte stuffing (variable overhead, PPP), Bit stuffing (Ã¢â€°Â¤20% overhead, HDLC) |
| **CRC-32 Properties** | Detects: all single-bit, all double-bit, odd-count errors, bursts Ã¢â€°Â¤ 32 bits |
...
| **Efficiency Rule** | Window Ã¢â€°Â¥ BDP in frames to achieve 100% link utilization |
...
**Background:** A remote research station in Antarctica communicates via a geostationary satellite link (RTT â‰ˆ 540 ms). The link operates at 50 Mbps with a bit error rate (BER) of $10^{-6}$. Frame size is 1500 bytes.

**Challenge:** Stop-and-Wait would achieve only 0.004 % utilization. Go-Back-N would retransmit an entire window (~4000 frames) on each error, wasting ~400 ms of capacity per error. At BER $10^{-6}$, each 1500-byte (12,000-bit) frame has a 1.2 % error probability â†’ errors are frequent.

**Solution â€” Selective Repeat:**
1. Window size = $\lceil (540 \times 10^{-3} \times 50 \times 10^6) / (1500 \times 8) \rceil = \lceil 27,000,000 / 12,000 \rceil = 2250$ frames
2. With 11-bit sequence numbers ($2^{10} = 1024 < 2250$, so $k = 11 \rightarrow 2^{10} = 2048 < 2250$, actually $k = 12 \rightarrow 2^{11} = 2048 < 2250$, so $k = 12$ with $2^{11} = 2048$ â€” still not enough, $k = 13 \rightarrow 2^{12} = 4096 \ge 2 \times 2250$ âœ“)
3. Receiver buffers out-of-order frames up to 2250 deep
4. Only corrupted frames are retransmitted

**Result:** Utilization â‰ˆ 92 % (compared to 0.004 % for Stop-and-Wait, 45 % for Go-Back-N at this error rate). The research station achieves sustained throughput of 46 Mbps instead of 2 Mbps.
...
| **Stop-and-Wait** is only efficient on very short links | Utilization = $T_{tx} / (T_{tx} + 2T_{prop})$ â€” if $T_{prop} \gg T_{tx}$, utilization drops to near zero |
| **Selective Repeat** for high-BER, high-delay links | Retransmits only lost frames â€” critical for wireless and satellite |
...
| **Frame size matters** | Larger frames â†’ better efficiency, but higher error probability per frame |
...
1. Byte stuffing escapes every occurrence of the flag byte in payload â†’ worst-case 100 % overhead (all flag bytes). Bit stuffing inserts a 0 after five consecutive 1s â†’ worst-case ~20 % overhead (payload all 1s).
2. A single-bit error of degree $i$ produces remainder $x^i \bmod G(x)$. If $G(x)$ has â‰¥ 2 terms, this remainder is non-zero â†’ always detected.
3. Detect $d$ errors: $d_{min} \ge d + 1$. Correct $c$ errors: $d_{min} \ge 2c + 1$.
4. GBN: $W \le 2^3 - 1 = 7$. SR: $W \le 2^{3-1} = 4$.
5. Acceptable when $T_{tx} \gg T_{prop}$ (short, low-delay links). Example: 1500 bytes at 1 Gbps â†’ $T_{tx} = 12\ \mu s$, useful only for sub-meter distances.
6. GBN: single cumulative ACK for all frames up to N. SR: individual ACK per frame.
7. Wi-Fi uses SIFS (10 Âµs) before ACK â€” too short for window-based operation. Stop-and-Wait with fast ACK works well for short-range radio.
8. Detects loopback â€” if a peer receives its own magic number, a loop exists.
</details>
...
9. Compute: pad data with 3 zeros â†’ 1010101000. Divide by 1011 using XOR. Remainder = 110.
10. Max throughput with SWS = 7, RTT = 50 ms. Window fills pipe? BDP = 50 ms Ã— 10 Mbps = 500,000 bits. Frame = 8000 bits. Need SWS = 500,000/8000 = 62.5 for full utilization â†’ SWS = 7 â†’ throughput = 7 Ã— 8000 / 0.05 = 1.12 Mbps.
11. 4Ã—16 matrix â†’ 4 row parity + 16 column parity + 1 corner = 21 bits overhead for 64 data bits. Double-bit error in same row: each row parity flips twice (returns to even), column parity bits detect the two columns â†’ error detected.
12. BDP = 2 Ã— 100 Âµs Ã— 10 Gbps = 2,000,000 bits = 166.67 frames Ã— 2 (for SR) â†’ W â‰ˆ 334. Sequence bits: $k$ such that $2^{k-1} \ge 334$ â†’ $k = 10$.
13. With 3-bit seq (mod 8), GBN max = 7. Sending frame 7 with window = 7 â†’ window full. After ACK for 0, window slides, can send frame 8 (seq 0).
14. Flag byte 0x7E = 1/256 = 0.39 %. Each escape = 2 bytes (0x7D + flag). Expected overhead = 1000 Ã— 0.0039 Ã— 2 â‰ˆ 7.8 bytes (0.78 %).
</details>

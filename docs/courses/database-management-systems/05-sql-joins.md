...
**Why Nested Loop is O(NÃ—M):** For each of the N left rows, we scan all M right rows. With 5 employees and 3 departments: 5Ã—3 = 15 condition evaluations. With 1M employees and 100K departments: 10Â¹Â¹ evaluations â€” catastrophic.
...
| **Duplicate join values** | Every pair is emitted â€” can cause row multiplication | Two employees in dept 20 Ã— one dept 20 row â†’ 2 rows |
...
| Algorithm | Time | Space | Why |
|-----------|------|-------|-----|
| Nested Loop | O(N Ã— M) | O(1) | N left Ã— M right â€” same as LEFT JOIN |
...
| Algorithm | Time | Space | Why |
|-----------|------|-------|-----|
| CROSS JOIN | O(N Ã— M) | O(1) extra | Every left Ã— every right â€” no shortcuts |
...
| **Large tables** | Result explosion | 10K Ã— 10K = 100M rows â€” can crash the server |
...
**Why O(NÂ²) for nested loop:** The same table appears twice. For each of N "employee" rows, we scan all N "manager" rows. 5 rows â†’ 25 comparisons. 1M rows â†’ 10Â¹Â² comparisons (impractical without hash).
...
| 3 | (3, Charlie, 20) | (10, Engineering) | NO (20â‰ 10) | Continue scanning | â€” |
...
| 8 | (5, Eve, 20) | (10, Eng) | NO (20â‰ 10) | Continue | â€” |
...
| INNER JOIN + DISTINCT | O(NÃ—M + sort) | O(N + M) | Join then sort for dedup â€” most expensive option |
...
| **Multiple matches** | Left row appears once (first match short-circuits) | Alice matches Engineering â†’ only Alice, not AliceÃ—N |
...
- NOT IN: **Dangerous** â€” `WHERE x NOT IN (1, NULL, 3)` returns zero rows because `x â‰  NULL` is UNKNOWN
...
**Answer:**
1. **Index join columns** â€” especially foreign keys. Index Nested Loop turns O(NÃ—M) into O(N log M)
...
- Nested Loop Join works for any condition but is O(NÃ—M) â€” best with small inner tables.

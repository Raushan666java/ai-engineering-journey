...
- **Joint Probability Distribution**: Specifies the probability of every possible combination of values for a set of random variables. For 3 boolean variables, the full joint has 2Ã‚Â³ = 8 entries.
...
We have two variables: Weather (W) Ã¢Ë†Ë† {Sunny, Rainy} and Mood (M) Ã¢Ë†Ë† {Happy, Sad}. Joint distribution P(W, M):
...
|--------|-----------|-----|
| **Space (full joint)** | O(dÃ¢ÂÂ¿) for n variables each with d values | Every combination must be stored â†’ exponential explosion |
| **Time (marginalize)** | O(dÃ¢ÂÂ¿) | Must sum over all other dimensions |
| **CPT representation (BN)** | O(n Ãƒâ€” dÃ¡ÂµÂ) where k = max parents | Only local parents; k &lt;< n in sparse graphs |

The full joint distribution is **impractical** beyond ~20 boolean variables (2Ã‚Â²Ã¢ÂÂ° Ã¢â€°Ë† 1 million entries). This is the **curse of dimensionality** and the primary motivation for Bayesian Networks.
...
P(spam | "FREE MONEY!!!") = P("FREE MONEY!!!" | spam) Ãƒâ€” P(spam) / P("FREE MONEY!!!")

= (0.80 Ãƒâ€” 0.60) / (0.80 Ãƒâ€” 0.60 + 0.05 Ãƒâ€” 0.40) = 0.48 / 0.50 = 0.96
...
7. Compute posterior: P(A|B) = P(B|A) Ãƒâ€” P(A) / P(B)
8. Return posterior
...
4. P(Positive) = (0.95 Ãƒâ€” 0.01) + (0.05 Ãƒâ€” 0.99) = 0.0095 + 0.0495 = 0.059
5. P(Disease | Positive) = 0.0095 / 0.059 Ã¢â€°Ë† 0.161
...
| 3 | numerator = 0.95 Ãƒâ€” 0.01 | 0.0095 |
| 4 | denominator = 0.95Ãƒâ€”0.01 + 0.05Ãƒâ€”0.99 | 0.059 |
...
| **Factorization** | P(X) = Ã¢Ë†Â P(XÃ¡ÂµÂ¢ | Parents(XÃ¡ÂµÂ¢)) | Reduces parameters from O(2Ã¢ÂÂ¿) to O(n Ãƒâ€” 2Ã¡ÂµÂ) |
...
**Steps:**
1. Choose an ordering of variables: XÃ¢â€šÂ, XÃ¢â€šâ€š, ..., XÃ¢â€šâ„¢ (causes before effects)
2. For each variable XÃ¡ÂµÂ¢:
   a. Identify a minimal set of parents from XÃ¢â€šÂ...XÃ¡ÂµÂ¢Ã¢â€šâ€¹Ã¢â€šÂ that directly influence XÃ¡ÂµÂ¢
   b. Add directed edges from each parent to XÃ¡ÂµÂ¢
   c. Define the CPT P(XÃ¡ÂµÂ¢ | Parents(XÃ¡ÂµÂ¢))
...
| Aspect | Complexity | Why |
|--------|-----------|-----|
| **Full joint parameters** | O(2Ã¢ÂÂ¿) | Exponential in number of variables |
| **BN parameters** | O(n Ãƒâ€” 2Ã¡ÂµÂ) where k = max parents | Exponential in max parents, not total variables |
| **Inference (exact)** | O(exp(treewidth)) | NP-hard in worst case; tractable for low treewidth |
| **Structure learning** | O(nÃ‚Â²) heuristic / NP-hard optimal | Each edge direction needs evaluation |
| **Memory (CPT storage)** | O(Ã¢Ë†â€˜ 2^{|parents(XÃ¡ÂµÂ¢)|}) | Sum of per-node CPT sizes |
...
1. **Network with Many Parents**: CPT size explodes. Solution: use canonical models like noisy-OR (each parent independently causes child; probabilities combine via P(child=True) = 1 - Ã¢Ë†Â(1 - pÃ¡ÂµÂ¢)).
...
- fÃ¢â€šÂ(B): P(B)
- fÃ¢â€šâ€š(E): P(E)
- fÃ¢â€šÆ’(A, B, E): P(A | B, E)
- fÃ¢â€šâ€ž(J, A): P(J | A)
- fÃ¢â€šâ€¦(M, A): P(M | A)

**Step 1 â†’ Restrict evidence M=true:**
- fÃ¢â€šâ€¦(M=true, A): A=F â†’ 0.01, A=T â†’ 0.70

**Step 2 â†’ Eliminate J:**
- Sum out J: P(J|A) summed over J = 1.0. Factor disappears.

**Step 3 â†’ Multiply fÃ¢â€šÆ’ Ãƒâ€” fÃ¢â€šâ€¦:**
- fÃ¢â€šâ€ (B, E, A) = fÃ¢â€šÆ’(A, B, E) Ãƒâ€” fÃ¢â€šâ€¦(M=true, A)

| B | E | A=F | A=T |
|---|---|-----|-----|
| F | F | 0.999Ãƒâ€”0.01=0.00999 | 0.001Ãƒâ€”0.70=0.00070 |
| F | T | 0.710Ãƒâ€”0.01=0.00710 | 0.290Ãƒâ€”0.70=0.20300 |
| T | F | 0.060Ãƒâ€”0.01=0.00060 | 0.940Ãƒâ€”0.70=0.65800 |
| T | T | 0.050Ãƒâ€”0.01=0.00050 | 0.950Ãƒâ€”0.70=0.66500 |

**Step 4 â†’ Eliminate A (sum over A):**
- fÃ¢â€šâ€¡(B=F, E=F) = 0.00999 + 0.00070 = 0.01069
- fÃ¢â€šâ€¡(B=F, E=T) = 0.00710 + 0.20300 = 0.21010
- fÃ¢â€šâ€¡(B=T, E=F) = 0.00060 + 0.65800 = 0.65860
- fÃ¢â€šâ€¡(B=T, E=T) = 0.00050 + 0.66500 = 0.66550

**Step 5 â†’ Multiply fÃ¢â€šÂ Ãƒâ€” fÃ¢â€šâ€š Ãƒâ€” fÃ¢â€šâ€¡:**
- fÃ¢â€šË†(B=F, E=F) = 0.999 Ãƒâ€” 0.998 Ãƒâ€” 0.01069 = 0.010654
- fÃ¢â€šË†(B=F, E=T) = 0.999 Ãƒâ€” 0.002 Ãƒâ€” 0.21010 = 0.000420
- fÃ¢â€šË†(B=T, E=F) = 0.001 Ãƒâ€” 0.998 Ãƒâ€” 0.65860 = 0.000657
- fÃ¢â€šË†(B=T, E=T) = 0.001 Ãƒâ€” 0.002 Ãƒâ€” 0.66550 = 0.000001

**Step 6 â†’ Eliminate E:**
- fÃ¢â€šâ€°(B=F) = 0.010654 + 0.000420 = 0.011074
- fÃ¢â€šâ€°(B=T) = 0.000657 + 0.000001 = 0.000658
...
| Aspect | Complexity | Why |
|--------|-----------|-----|
| **Variable Elimination** | O(n Ãƒâ€” exp(treewidth)) | Treewidth = size of largest intermediate factor |
| **Naive enumeration** | O(2Ã¢ÂÂ¿) | Sum over all assignments of hidden variables |
| **Likelihood Weighting** | O(N Ãƒâ€” n) for N samples | No exponential blowup, but approximate |
| **Gibbs Sampling** | O(N Ãƒâ€” n) per iteration | Mixing time can be exponential for pathological BNs |
...
|-----------------|:---:|:---:|:---:|---------|-----------|
| Variable Elimination | Exact | Ã¢Å“â€¦ | O(exp treewidth) | Low treewidth BNs | Sensitive to elimination order |
| Likelihood Weighting | Approx | Ã¢ÂÅ’ | O(N) | Rare evidence | Many samples needed |
| Rejection Sampling | Approx | Ã¢ÂÅ’ | O(N / P(e)) | Easy-to-satisfy evidence | Wastes samples on rejections |
| Gibbs Sampling (MCMC) | Approx | Ã¢ÂÅ’ | O(N Ãƒâ€” nodes) | High-dimensional posteriors | Burn-in and mixing issues |
...
If all paths between X and Y are blocked by Z, then X Ã¢Å¸â€š Y | Z.
...
**How:** P(Spam | Words) Ã¢Ë†Â P(Spam) Ã¢Ë†Â P(Word | Spam). Words are assumed conditionally independent given class.
...
|------|---------|----------|
| Joint Probability | P(A Ã¢Ë†Â§ B) = P(A|B)P(B) | Computing combined events |
| Bayes' Rule | P(A|B) = P(B|A)P(A) / P(B) | Diagnostic reasoning |
| Chain Rule | P(XÃ¢â€šÂÃ¢â‚¬Â¦XÃ¢â€šâ„¢) = Ã¢Ë†Â P(XÃ¡ÂµÂ¢ | XÃ¢â€šÂÃ¢â‚¬Â¦XÃ¡ÂµÂ¢Ã¢â€šâ€¹Ã¢â€šÂ) | Decomposing joint distributions |
| Law of Total Probability | P(B) = ÃŽÂ£ P(B|AÃ¡ÂµÂ¢)P(AÃ¡ÂµÂ¢) | Computing marginal likelihood |
...
|-----------|:---:|:---:|:---:|:---:|:---:|
| Bayes' Rule | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ |
| Bayesian Networks | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ |
| Naive Bayes | Ã¢Å“â€¦ | Ã¢ÂÅ’ | Ã¢Å“â€¦ | Ã¢ÂÅ’ | Ã¢Å“â€¦ |
| Variable Elimination | Ã¢ÂÅ’ | Ã¢ÂÅ’ | Ã¢ÂÅ’ | Ã¢ÂÅ’ | Ã¢Å“â€¦ |
| MCMC Sampling | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ | Ã¢Å“â€¦ |
| Kalman / Particle Filters | Ã¢ÂÅ’ | Ã¢Å“â€¦ | Ã¢ÂÅ’ | Ã¢Å“â€¦ | Ã¢Å“â€¦ |
...
- A) P(XÃ¢â€šÂÃ¢â‚¬Â¦XÃ¢â€šâ„¢) = Ã¢Ë†Â P(XÃ¡ÂµÂ¢ | Parents(XÃ¡ÂµÂ¢))
- B) P(XÃ¢â€šÂÃ¢â‚¬Â¦XÃ¢â€šâ„¢) = Ã¢Ë†Â P(XÃ¡ÂµÂ¢)
- C) P(A|B) = P(B|A)P(A)
- D) P(A Ã¢Ë†Â§ B) = P(A)P(B)
...
<details><summary>Answer&lt;/summary&gt;B) BNs factorize the joint distribution into local CPTs, requiring O(n Ãƒâ€” 2Ã¡ÂµÂ) parameters instead of O(2Ã¢ÂÂ¿) â†’ a massive saving when each node has few parents.</details>

---

> **Ã°Å¸â€™Â¡ Pro Tip:** When modeling real-world problems with Bayesian networks, focus on getting the qualitative structure (DAG) right â†’ conditional independence assumptions matter far more than precise probability numbers. Use noisy-OR/MAX for efficient parameterization of causal relationships.

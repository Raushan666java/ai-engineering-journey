...
> **ðŸ’¡ Pro Tip:** The heuristic function h(n) is the key design decision in informed search. A good heuristic (like Manhattan distance for the 8-puzzle) can reduce explored nodes by orders of magnitude compared to uninformed search.
...
> **âš ï¸ Warning:** An admissible heuristic never overestimates, but it can still lead to poor performance if it greatly underestimates. The closer h(n) is to the true cost h*(n), the fewer nodes A* expands.
...
|-----------|:---:|:---:|:---:|:---:|:---:|
| BFS | Uninformed | âœ… | âœ… (uniform) | No | O(b^d) |
| DFS | Uninformed | âŒ | âŒ | No | O(bm) |
| Uniform-Cost | Uninformed | âœ… | âœ… | No | O(b^{1+...}) |
| Greedy Best-First | Informed | âŒ | âŒ | Yes | O(b^m) |
| A* | Informed | âœ… | âœ… | Yes | O(b^d) |

## Quick Reference â€” Heuristic Properties

| Property | Definition | Implication |
|----------|-----------|-------------|
| Admissible | h(n) â‰¤ h*(n) never overestimates | A* tree-search is optimal |
| Consistent | h(n) â‰¤ c(n,a,n') + h(n') (triangle inequality) | A* graph-search is optimal |
| Dominance | hâ‚‚(n) â‰¥ hâ‚(n) for all n (both admissible) | hâ‚‚ dominates hâ‚ â€” expands fewer nodes |
| Effective Branching Factor | b* where N = 1 + b* + (b*)Â² + ... | Measures heuristic quality empirically |
...
| BFS | â¬œ | â¬œ | â¬œ | âœ… |
| DFS | â¬œ | â¬œ | âœ… | âœ… |
| Greedy Best-First | âœ… | â¬œ | â¬œ | âœ… |
| A* Search | âœ… | â¬œ | â¬œ | âœ… |
| Heuristic Design | â¬œ | âœ… | â¬œ | âœ… |

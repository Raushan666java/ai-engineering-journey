...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/09-dp-sequences/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/09-dp-sequences/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/09-dp-sequences/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/09-dp-sequences/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/algorithms/09-dp-sequences/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/algorithms/09-dp-sequences/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
Sequence DP problems model real-world tasks where order matters. When a spell checker suggests "receive" instead of "recieve", it uses edit distance â€” the minimum characters to insert, delete, or replace. When GitHub highlights lines added and removed in a pull request, it runs LCS (longest common subsequence) under the hood. When biologists align two DNA strands to find evolutionary mutations, they use sequence alignment DP â€” the same recurrence as edit distance, generalized with scoring matrices.

Without sequence DP, every comparison of two ordered sequences would require exponential brute force. The core insight â€” that the optimal solution for prefixes builds on optimal solutions for smaller prefixes â€” turns an \( O(3^{m+n}) \) problem into \( O(mn) \).
...
**Real-World Analogy:** Version control systems (git diff) compute the longest set of lines that remain unchanged between two file versions. The changed lines are everything else â€” insertions and deletions around the common core. Bioinformatics uses the same idea to compare DNA sequences: "ACTG" vs "ACG" shares the subsequence "ACG".

#### Algorithm Steps

1. Create a 2D DP table `dp` of size `(m+1) Ã— (n+1)` initialized to 0, where `m = len(X)` and `n = len(Y)`.
...
- `i=1 (A), j=1 (B)`: Aâ‰ B â†’ dp[1][1] = max(0,0) = 0
- `i=1 (A), j=4 (A)`: A=A â†’ dp[1][4] = dp[0][3] + 1 = 1
- `i=2 (B), j=1 (B)`: B=B â†’ dp[2][1] = dp[1][0] + 1 = 1
- `i=2 (B), j=5 (B)`: B=B â†’ dp[2][5] = dp[1][4] + 1 = 2
- `i=4 (B), j=5 (B)`: B=B â†’ dp[4][5] = dp[3][4] + 1 = 3
- `i=6 (A), j=4 (A)`: A=A â†’ dp[6][4] = dp[5][3] + 1 = 3
- `i=6 (A), j=6 (A)`: A=A â†’ dp[6][6] = dp[5][5] + 1 = 4
- `i=7 (B), j=5 (B)`: B=B â†’ dp[7][5] = dp[6][4] + 1 = 4

**Reconstruction traceback from (7,6):**
dp[7][6]=4 â†’ (7,5): Bâ‰ A, dp[6][5]=3 &lt; dp[7][6]=4 â†’ j-- â†’ (7,5): B=B â†’ take 'B', i=6, j=4
dp[6][4]=3 â†’ (6,4): A=A â†’ take 'A', i=5, j=3
dp[5][3]=2 â†’ (5,3): Dâ‰ C, dp[4][3]=1 &lt; dp[5][3]=2 â†’ j-- â†’ (5,2): Dâ‰ B, dp[4][2]=1 < dp[5][2]=2 â†’ j-- â†’ (5,1): Dâ‰ B, dp[4][1]=1 = dp[5][1]=1 â†’ j-- â†’ (5,0): stop
Backtrack from dp[4][1]: (4,1): B=B â†’ take 'B', i=3, j=0 â†’ stop

LCS = "BCAB" (or alternatively "BCBA" through different equal choices â€” both length 4).
...
#### Complexity Analysis â€” With WHY

- **Time: O(mn)** â€” Each of the (m+1)Ã—(n+1) cells is computed exactly once with O(1) work per cell. This is optimal because the LCS problem has Î©(mn) possible prefix pairs to consider.
- **Space: O(mn)** â€” The full 2D table stores (m+1)Ã—(n+1) integers. Each cell depends on three neighbors (top, left, diagonal), so a full table is needed for reconstruction.
- **Why not faster?** LCS is in the class of sequence alignment problems with a known Î©(mn) lower bound in the comparison model. No sub-quadratic algorithm exists for arbitrary alphabets.
...
**Time: O(mn), Space: O(n)** â€” Only two rows of length n+1 are kept, reducing O(mn) space to O(n).
...
| Handles arbitrary-length strings | Not efficient for very long strings (>10âµ characters) |
...
**Real-World Analogy:** In the card game patience (solitaire), you arrange cards into piles where each pile is decreasing. The number of piles equals the length of the LIS. In finance, LIS models the longest uptrend in a stock price â€” finding the maximum number of days the price can increase in sequence without requiring consecutive days.
...
| 3 | 5 | 0,1,2 | 10&lt;5? No, 9<5? No, 2<5? Yes â†’ dp[3]=max(1,1+dp[2])=max(1,2)=2 | 1 | 2 |
| 4 | 3 | 0,1,2,3 | 10&lt;3? No, 9<3? No, 2<3? Yes â†’ dp[4]=max(1,1+1)=2, 5<3? No | 1 | 2 |
| 5 | 7 | 0-4 | 2&lt;7 â†’ dp[5]=max(1,1+1)=2, 5<7 â†’ max(2,1+2)=3, 3<7 â†’ max(3,1+2)=3 | 1 | 3 |
| 6 | 101 | 0-5 | 7&lt;101 â†’ dp[6]=max(1,1+3)=4 | 1 | 4 |
| 7 | 18 | 0-6 | 7&lt;18 â†’ max(1,1+3)=4, 101<18? No | 1 | 4 |

Final `dp = [1, 1, 1, 2, 2, 3, 4, 4]`, LIS length = 4.

#### \( O(n \log n) \) â€” Patience Sorting
...
|---|----------------------|--------|------------|
| 10 | [] â†’ i=0 = len | append 10 | [10] |
| 9 | lower_bound([10], 9) â†’ 0 &lt; len | replace tails[0]=9 | [9] |
| 2 | lower_bound([9], 2) â†’ 0 &lt; len | replace tails[0]=2 | [2] |
| 5 | lower_bound([2], 5) â†’ 1 = len | append 5 | [2, 5] |
| 3 | lower_bound([2,5], 3) â†’ 1 &lt; len | replace tails[1]=3 | [2, 3] |
| 7 | lower_bound([2,3], 7) â†’ 2 = len | append 7 | [2, 3, 7] |
| 101 | lower_bound([2,3,7], 101) â†’ 3 = len | append 101 | [2, 3, 7, 101] |
| 18 | lower_bound([2,3,7,101], 18) â†’ 3 &lt; len | replace tails[3]=18 | [2, 3, 7, 18] |
...
// O(n^2) version â€” also reconstructs the subsequence
...
- **O(nÂ²) time:** For each of n elements, scan up to n previous elements â€” total O(nÂ²) comparisons. A[0..i-1] must each be checked because LIS ending at i can extend any previous subsequence.
- **O(n log n) time:** Binary search on the tails array (size â‰¤ n) for each of n elements gives O(log n) per element. The tails array stays sorted by construction â€” each replacement preserves sorted order, enabling binary search.
...
- **Space:** O(n) for both versions â€” dp array or tails array.

#### Space Optimization

The O(nÂ²) DP naturally uses O(n) space (single dp array). No further optimization needed.
...
|----------|------------|---------------|
| **O(nÂ²) DP** | Simple, intuitive, easy to reconstruct subsequence | Slow for n > 10â´ |
| **O(n log n)** | Fast, handles n up to 10â¶ | Harder to understand; `tails` is not the actual subsequence |
...
**Real-World Analogy:** Spell checkers suggest "accommodate" when you type "acommodate" â€” the edit distance is 1 (insert one 'c'). Autocorrect on smartphones uses edit distance to rank candidate corrections. In computational biology, the Needleman-Wunsch algorithm generalizes edit distance to align DNA sequences with affine gap penalties, enabling scientists to quantify evolutionary divergence between species.

#### Algorithm Steps

1. Create a 2D DP table `dp` of size `(m+1) Ã— (n+1)`.
...
3. Loop `i` from 1 to `m` and `j` from 1 to `n`:
   ...
   - Else, `dp[i][j] = 1 + min(dp[i-1][j], dp[i][j-1], dp[i-1][j-1])`:
     - `dp[i-1][j] + 1` â†’ delete X[i-1] (or equivalently insert into Y)
     - `dp[i][j-1] + 1` â†’ insert Y[j-1] into X
     - `dp[i-1][j-1] + 1` â†’ replace X[i-1] with Y[j-1]
4. Return `dp[m][n]`.
...
- `i=1 (C), j=1 (C)`: C=C â†’ dp[1][1] = dp[0][0] = 0
- `i=1 (C), j=2 (A)`: Câ‰ A â†’ min(dp[0][2]=2, dp[1][1]=0, dp[0][1]=1) + 1 = 1 (replace Câ†’CA? Actually delete C and insert A: dp[1][2] = 1)
- `i=1 (C), j=3 (R)`: Câ‰ R â†’ min(dp[0][3]=3, dp[1][2]=1, dp[0][2]=2) + 1 = 2
- `i=2 (A), j=1 (C)`: Aâ‰ C â†’ min(dp[1][1]=0, dp[2][0]=2, dp[1][0]=1) + 1 = 1
- `i=2 (A), j=2 (A)`: A=A â†’ dp[2][2] = dp[1][1] = 0
- `i=2 (A), j=3 (R)`: Aâ‰ R â†’ min(dp[1][3]=2, dp[2][2]=0, dp[1][2]=1) + 1 = 1
- `i=3 (T), j=1 (C)`: Tâ‰ C â†’ min(dp[2][1]=1, dp[3][0]=3, dp[2][0]=2) + 1 = 2
- `i=3 (T), j=2 (A)`: Tâ‰ A â†’ min(dp[2][2]=0, dp[3][1]=2, dp[2][1]=1) + 1 = 1
- `i=3 (T), j=3 (R)`: Tâ‰ R â†’ min(dp[2][3]=1, dp[3][2]=1, dp[2][2]=0) + 1 = 1
...
Edit Distance = dp[3][3] = 1 (replace 'T' with 'R': CAT â†’ CAR).
...
- **Time: O(mn)** â€” Each cell in the (m+1)Ã—(n+1) table requires O(1) work (a comparison and up to three lookups). This is optimal because any algorithm must at least consider each pair of prefix positions.
- **Space: O(mn)** for the full table. **O(n)** with 2-row optimization.
- **Why three operations?** Delete, insert, and replace are the minimal set that can transform any string into any other. Remove any operation and some strings become unreachable (e.g., without replace, "CAT"â†’"CAR" requires 2 operations instead of 1).
...
**Time: O(mn), Space: O(n).** Only two rows are kept â€” the previous row and the current row being filled.
...
| X="ABC", Y="CBA" | 2 | Reverse: replace Bâ†’B? Actually Aâ†”C â†’ delete A, insert C, delete C, insert A... min=2 (replace Aâ†”C, keep B) |
| X="KITTEN", Y="SITTING" | 3 | Replace Kâ†’S, replace Eâ†’I, insert G |
...
**Real-World Analogy:** When a deep learning framework multiplies a sequence of weight matrices and input tensors, the order of multiplication dramatically affects computation speed. For example, multiplying a 10Ã—100 matrix by a 100Ã—5 matrix by a 5Ã—50 matrix costs 7500 multiplications if grouped as (10Ã—100 Ã— 100Ã—5) Ã— 5Ã—50, but 75,000 if grouped as 10Ã—100 Ã— (100Ã—5 Ã— 5Ã—50) â€” a 10Ã— difference. Compilers use this optimization for tensor operations.

#### Algorithm Steps

1. Let `p[0..n]` be the dimensions where matrix `A_i` has dimensions `p[i-1] Ã— p[i]`.
2. Create a 2D DP table `dp` of size `n Ã— n` initialized to 0, and a `split` table for reconstruction.
...
Dimensions `p = [5, 4, 6, 2, 7]` â†’ matrices: Aâ‚(5Ã—4), Aâ‚‚(4Ã—6), Aâ‚ƒ(6Ã—2), Aâ‚„(2Ã—7), n=4.

**Chain length L=2:**

| (i,j) | k=1 | k=2 | k=3 | dp[i][j] |
|-------|-----|-----|-----|----------|
| (1,2) | 0+0+5Â·4Â·6=120 | â€” | â€” | 120 |
| (2,3) | â€” | 0+0+4Â·6Â·2=48 | â€” | 48 |
| (3,4) | â€” | â€” | 0+0+6Â·2Â·7=84 | 84 |

**Chain length L=3:**

- (1,3): i=1, j=3
  - k=1: dp[1][1]+dp[2][3] + p[0]Â·p[1]Â·p[3] = 0 + 48 + 5Â·4Â·2 = 40 + 48 ... wait: 0 + 48 + 5Â·4Â·2 = 48 + 40 = 88
  - k=2: dp[1][2]+dp[3][3] + p[0]Â·p[2]Â·p[3] = 120 + 0 + 5Â·6Â·2 = 120 + 60 = 180
  - min = 88, split = 1

- (2,4): i=2, j=4
  - k=2: dp[2][2]+dp[3][4] + p[1]Â·p[2]Â·p[4] = 0 + 84 + 4Â·6Â·7 = 84 + 168 = 252
  - k=3: dp[2][3]+dp[4][4] + p[1]Â·p[3]Â·p[4] = 48 + 0 + 4Â·2Â·7 = 48 + 56 = 104
  - min = 104, split = 3

**Chain length L=4:**

- (1,4): i=1, j=4
  - k=1: dp[1][1]+dp[2][4] + p[0]Â·p[1]Â·p[4] = 0 + 104 + 5Â·4Â·7 = 104 + 140 = 244
  - k=2: dp[1][2]+dp[3][4] + p[0]Â·p[2]Â·p[4] = 120 + 84 + 5Â·6Â·7 = 204 + 210 = 414
  - k=3: dp[1][3]+dp[4][4] + p[0]Â·p[3]Â·p[4] = 88 + 0 + 5Â·2Â·7 = 88 + 70 = 158
  - min = 158, split = 3
...
Minimum cost = 158 multiplications. Parenthesization: `(Aâ‚ Ã— (Aâ‚‚ Ã— Aâ‚ƒ)) Ã— Aâ‚„` at split=3 for (1,4), and within (2,4) at split=3 â†’ `(Aâ‚ Ã— (Aâ‚‚ Ã— Aâ‚ƒ)) Ã— Aâ‚„`.
...
def print_parens(split, i, j):
    if i == j:
        return f"A{i}"
    k = split[i][j]
    return f"({print_parens(split, i, k)} Ã— {print_parens(split, k + 1, j)})"
```
...
- **Time: O(nÂ³)** â€” Three nested loops: chain length L (n iterations), start index i (n iterations per L), split point k (up to n iterations). Total â‰ˆ Î£_{L=2}^{n} (n-L+1)Â·(L-1) = O(nÂ³).
- **Space: O(nÂ²)** â€” The dp table stores nÃ—n entries. The split table also O(nÂ²) for reconstruction.
- **Why O(nÂ³)?** Each subproblem (i,j) tries all possible split points k between i and j-1. There are O(nÂ²) subproblems Ã— O(n) splits each = O(nÂ³). This is optimal for general matrix chains.
- **Chu-Liu-Edmonds lower bound:** No algorithm can do better than O(nÂ²) because the output itself (split choices) is O(nÂ²) entries.

#### Space Optimization

The dp table can't be compressed to O(n) easily because interval DP needs all subproblem lengths. However, we only need one split table for reconstruction â€” the dp table can be stored as-is.

#### Advantages & Disadvantages

| Advantages | Disadvantages |
|------------|---------------|
| Optimal parenthesization (not a heuristic) | O(nÂ³) time is slow for large n |
...
| p=[5, 4, 6, 2] | 88 | Three matrices, better to group (Aâ‚(Aâ‚‚Aâ‚ƒ)) |
...
**Real-World Analogy:** NLP pipelines split text into meaningful chunks. Palindrome partitioning models problems where you must segment a sequence into valid substrings â€” like splitting a compound word ("longest") into dictionary words ("long", "est") or aligning phonemes in speech recognition.

#### Algorithm Steps

**Step 1 â€” Precompute palindrome table:**

1. Create a boolean 2D table `isPal` of size `n Ã— n`.
...
S = "ababbbabbababa" (n=14) â€” simplified as S = "aab" (n=3) for illustration.

**Step 1 â€” Palindrome table for S = "aab":**
...
- L=2: i=0,j=1: S[0]=a=S[1]=a, j-i=1&lt;2 â†’ isPal[0][1]=true
- L=2: i=1,j=2: S[1]=aâ‰ S[2]=b â†’ isPal[1][2]=false
- L=3: i=0,j=2: S[0]=aâ‰ S[2]=b â†’ isPal[0][2]=false

**Step 2 â€” Minimum cuts for S = "aab":**
- i=0: isPal[0][0]=true â†’ dp[0]=0
- i=1: isPal[0][1]=true â†’ dp[1]=0
- i=2: isPal[0][2]=false â†’ check:
  - j=0: isPal[1][2]=false â†’ skip
  - j=1: isPal[2][2]=true â†’ dp[2]=min(INF, dp[1]+1) = min(INF, 0+1) = 1
...
- **Time: O(nÂ²)** â€” Palindrome precomputation is O(nÂ²) (nested loops over substring lengths and start positions). The cuts DP is also O(nÂ²) (each i checks all j &lt; i). Combined: O(nÂ²).
- **Space: O(nÂ²)** â€” The palindrome table isO(nÂ²) boolean entries. Without precomputation, checking isPal inside the cut loop would be O(nÂ³) â€” the precomputation is essential.
- **Why O(nÂ²) and not O(nÂ³)?** The palindrome expansion uses the recurrence `isPal[i][j]` = `(S[i] == S[j]) && isPal[i+1][j-1]` â€” this is O(1) per substring, avoiding the naive O(nÂ³) expand-from-center approach.

#### Space Optimization

The palindrome table can be computed on-the-fly using two-pointer expansion from each center (O(nÂ²) time, O(1) space per check), but this makes the cuts DP O(nÂ³) again. An alternative: use expand-around-center to fill isPal in O(nÂ²) with O(nÂ²) space is standard.
...
**Time: O(nÂ²), Space: O(n)** â€” Eliminates the isPal table entirely.

#### Advantages & Disadvantages

| Advantages | Disadvantages |
|------------|---------------|
| Clean separation of palindrome detection and cuts | O(nÂ²) space for the standard approach |
| O(nÂ²) time is optimal | Doesn't enumerate all partitions (only count) |
...
- Transition: Compare `A[i-1]` and `B[j-1]` â†’ match or mismatch
...
- Complexity: O(nÂ²) base, can sometimes optimize to O(n log n) with binary search
...
| Single-Sequence Partition | dp[i] | Palindrome Partition, Word Break | O(nÂ²) |
| Interval DP | dp[i][j] by length | Matrix Chain, Burst Balloons | O(nÂ³) |
| Subsequence | dp[i] | LIS O(nÂ²) / O(n log n) | O(nÂ²)/O(n log n) |

---

## Interview Corner

### LIS in O(n log n) â€” The Patience Sorting Trick
...
**Variation â€” Non-decreasing:** Use `upper_bound` (C++) or `bisect_right` (Python) instead of `lower_bound` / `bisect_left`.

**Variation â€” Maximum Sum Increasing Subsequence:** Use O(nÂ²) DP with `dp[i] = A[i] + max(dp[j])` for `A[j] < A[i]`.
...
**Example:** S = "BBABCBCAB" â†’ reverse = "BACBCBABB" â†’ LCS length = 7 ("BABCBCB" or "BBABCBB").
...
**Space complexity:** O(nÂ²) time, O(nÂ²) space. Can be optimized to O(n) space with 2-row technique.
...
| Max sum increasing subsequence | Subsequence DP | O(nÂ²) DP | dp[i] = A[i] + max(dp[j]) for A[j] &lt; A[i] |
...
Git also offers patience diff, histogram diff, and minimal diff â€” all built on LCS/edit distance variants. GitHub uses these to display familiar green/red line highlights.
...
The **Smith-Waterman algorithm** (local alignment) modifies this to find the best-aligning substring pair (score never goes below 0) â€” used to find conserved functional regions.
...
**Why LCS and not string matching:** Renaming variables (aâ†’b, xâ†’y) changes the literal text but not the tokenized pattern `IDENT ASSIGN IDENT PLUS IDENT SEMI`. LCS on the tokenized sequence catches this.
...
| LPS | 2D (i,j) | dp[i][j] = LPS length in S[i..j] | match â†’ +2, no match â†’ max split | O(nÂ²) |

### Quick Reference

| Category | Key Points |
|----------|------------|
| **Two-Sequence DP** | LCS, Edit Distance, SCS â€” 2D table, O(mn) time |
| **Single-Sequence DP** | LIS â€” 1D or patience sorting, O(n log n) |
| **Interval DP** | Matrix Chain, LPS â€” loop by length L, then start i |
...
| **Space Optimization** | LCS, Edit Distance: 2 rows â†’ O(n). LIS: already O(n). |

...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/ai-agent-engineer/07-day-by-day-plan/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
|-------|-----------------|-----------------|
| 0 â€” Backend Gap-Fill | Redis docs, FastAPI docs | Real Python tutorials |
| 1 â€” Python + FastAPI | FastAPI official tutorial, Pydantic docs | TestDriven.io FastAPI course |
| 2 â€” LLM + RAG | Anthropic Cookbook, OpenAI Cookbook | Pinecone RAG guide |
| 3 â€” Agents | LangGraph docs, CrewAI docs, MCP spec | Simon Willison's LLM tools posts |
| 4 â€” Production Hardening | Docker docs, GitHub Actions docs | Grafana + Prometheus docs |
| 5 â€” Portfolio + Job Search | dev.to, Upwork guides | LinkedIn optimization guides |

## Common Pitfalls

1. **Phase skipping**: Each phase builds on the previous one â€” do not jump ahead without passing the checkpoint
...
Each day has a **topic**, an **action** (what to do), and a **done condition** (how to know you're done). If a day takes longer, stretch it â€” the buffer weeks absorb overruns.
...
| Sun | Rest or optional â€” read ahead in next week's content |
...
## Week 1 â€” Phase 0: Backend Gap-Fill
...
| 2 | Redis pub/sub + OpenAPI | Read 0.2â€“0.3, build pub/sub demo | Working publisher + subscriber in Python |
...
## Week 2 â€” Phase 1: Python + FastAPI (Part 1)

| Day | Topic | Action | Done Condition |
|-----|-------|--------|----------------|
| 1 | Type hints + dataclasses vs Pydantic | Read 1.1â€“1.2, annotate 10 functions | Can write `list[dict[str,int]]` without checking |
| 2 | Context managers + comprehensions | Read 1.3â€“1.4, write custom DB context mgr | Working context manager, 1-liner comprehensions |
...
| 5 | FastAPI DI + middleware | Read 1.7â€“1.8, wire `get_current_user` | 3 endpoints with DI chain |
| 6 | Background tasks + Pydantic v2 | Read 1.9â€“1.10, add model-level validator | Validator rejects invalid data |
| 7 | Pydantic settings + review | Read 1.11, move config to pydantic-settings | `.env` loads into typed config |

---

## Week 3 â€” Phase 1: Python + FastAPI (Part 2)

| Day | Topic | Action | Done Condition |
|-----|-------|--------|----------------|
| 1 | AsyncIO fundamentals | Read 1.12, compare event loop to Node.js | Can explain `await` in JS terms |
| 2 | asyncio.gather + httpx | Read 1.13â€“1.14, rewrite 3 calls as concurrent | Measurable speedup |
...
| 6 | Booking module port â€” schema + models | Write SQLAlchemy models + Pydantic schemas | Models match Laravel schema |
| 7 | Booking module port â€” endpoints | Wire CRUD endpoints | All endpoints respond correctly |

---

## Week 4 â€” Phase 2: LLM + RAG (Part 1)

| Day | Topic | Action | Done Condition |
|-----|-------|--------|----------------|
| 1 | Tokens + BPE tokenization | Read 2.1, estimate 5 paragraphs | Within 20% without tool |
| 2 | Context windows + embeddings | Read 2.2â€“2.3, run embedding comparison | Can explain cosine similarity concretely |
...
| 6 | Vector search math + HNSW | Read 2.7â€“2.8, understand cosine vs dot product | Can explain HNSW speed/accuracy tradeoff |
| 7 | RAG architecture | Read 2.9, draw full pipeline from memory | Every arrow labeled, no gaps |

---

## Week 5 â€” Phase 2: LLM + RAG (Part 2)

| Day | Topic | Action | Done Condition |
|-----|-------|--------|----------------|
| 1 | Chunking strategies | Read 2.10, test 3 strategies on your data | Can explain a real fixed-size failure mode |
| 2 | Re-ranking + vector DB comparison | Read 2.11â€“2.12, write comparison table | Honest 1-paragraph comparison written |
...
| 5 | RAG demo â€” ingestion pipeline | Build chunk + embed + store pipeline | Documents ingested and searchable |
| 6 | RAG demo â€” query endpoint | Build retrieve + re-rank + generate | Returns cited answers |
| 7 | RAG demo â€” deploy + review | Deploy to VPS, test edge cases | Public URL returning answers |

---

## Week 6 â€” Phase 3: Agents (Part 1)
...
## Week 7 â€” Phase 3: Agents (Part 2)
...
| 7 | Agent evaluation + cost | Read 3.12â€“3.13, evaluate your agent | 2 test cases pass, cost calculated |

---

## Week 8 â€” Phase 3: Agents (Part 3, Buffer)
...
| 3 | Purvanchal orchestration â€” design | Design state graph for existing n8n flow | Graph diagram matches flow |
| 4 | Purvanchal â€” build Part 1 | Implement main nodes + edges | Core path executes |
| 5 | Purvanchal â€” build Part 2 | Add conditional routing + persistence | Full pipeline runs end to end |
| 6 | Purvanchal â€” test + refine | Add error handling, edge cases | Passes 5 test scenarios |
| 7 | Buffer / catch-up | Finish any incomplete exercises | All Phase 3 checkboxes verified |

---

## Week 9 â€” Phase 4: Production Hardening (Part 1)
...
| 6 | Metrics + health checks | Read 4.6â€“4.7, expose /metrics and /healthz | Prometheus can scrape metrics |
| 7 | Secrets management | Read 4.8, move all keys to .env | Startup fails if var is missing |

---

## Week 10 â€” Phase 4: Production Hardening (Part 2)
...
## Week 11 â€” Phase 5: Portfolio (Part 1)
...
## Week 12 â€” Phase 5: Portfolio (Part 2) + Interview Prep

| Day | Topic | Action | Done Condition |
|-----|-------|--------|----------------|
| 1 | Technical interview prep | Read 5.7, run through 10 mock Q&A | Can explain RAG pipeline from memory |
| 2 | Salary negotiation + pricing | Read 5.8â€“5.9, set your rates | Written rate card + minimum acceptable |
...
Weeks 8 and 10 have built-in buffer days. These are the most important weeks in the plan â€” they determine whether you fall behind or stay on track.
...
1. **Never skip a day** â€” even 30 min of focused work counts. Consistency beats intensity.
2. **Done > perfect** â€” a working prototype deployed today is worth more than a polished one next week.
3. **Phase checkpoints are non-negotiable** â€” if you can't pass a phase checkpoint, do not start the next phase. Use buffer days to close the gap.
4. **Log your velocity** â€” after each week, note how many days you actually completed vs planned. This helps you adjust the remaining weeks realistically.
5. **If you're consistently overrunning, reduce scope** â€” the goal is 3 working portfolio projects and a job search, not completing every exercise perfectly.
...
const FULL_SCHEDULE: WeekSchedule[] = [
  {
    ...
    days: [
      ...
      { day: 2, dayOfWeek: "Tue", topic: "Redis pub/sub + OpenAPI", action: "Read 0.2â€“0.3, build pub/sub demo", doneCondition: "Working publisher + subscriber", estimatedMinutes: 180, isBuffer: false },
  ...
  },
  {
    ...
    days: [
      { day: 1, dayOfWeek: "Mon", topic: "Type hints + dataclasses vs Pydantic", action: "Read 1.1â€“1.2, annotate 10 functions", doneCondition: "Write list[dict[str,int]] without checking", estimatedMinutes: 180, isBuffer: false },
      { day: 2, dayOfWeek: "Tue", topic: "Context managers + comprehensions", action: "Read 1.3â€“1.4, write custom DB context mgr", doneCondition: "Working context manager", estimatedMinutes: 180, isBuffer: false },
      ...
      { day: 5, dayOfWeek: "Fri", topic: "FastAPI DI + middleware", action: "Read 1.7â€“1.8, wire get_current_user", doneCondition: "3 endpoints with DI chain", estimatedMinutes: 240, isBuffer: false },
      { day: 6, dayOfWeek: "Sat", topic: "Background tasks + Pydantic v2", action: "Read 1.9â€“1.10, add model-level validator", doneCondition: "Validator rejects invalid data", estimatedMinutes: 180, isBuffer: false },
  ...
  // Weeks 3â€“12 follow same pattern; omitted for brevity
];
...
tracker.logSession({
  ...
  actualMinutes: 190,
  completed: true,
  notes: "Pub/sub demo works â€” confirmed message loss without subscriber",
});
...
flowchart TD
    ...
    CHECK -->|"Yes â€” make progress"| WORK["Work Session<br/>2-4 hours"]
    CHECK -->|"No â€” stuck"| SIMPLIFY["Build simplest version<br/>Skip research, iterate later"]
    SIMPLIFY --> WORK
...
**Day 1 â€” Redis as Cache vs Queue**
Read the Redis documentation on SETEX (caching with TTL) and RQ (Redis Queue for background jobs). Build two small scripts: one using `redis-py` to cache API responses with `SETEX`, another using `rq` to enqueue a background email-sending job. Compare when each is appropriate â€” caching for read-heavy, latency-sensitive data; queues for write-heavy, durable operations.

**Day 2 â€” Redis Pub/Sub + OpenAPI**
...
**Day 4 â€” Rate Limiting + API Versioning**
...
**Day 5 â€” Microservices vs Monolith**
Read about the microservices pattern (independent deployability, bounded contexts, polyglot persistence) vs monoliths (simplicity, shared-nothing within process, easier transactions). Write a one-page analysis of your ApexERP project â€” would it benefit from microservices or should it stay monolithic?

**Day 6 â€” Idempotency Keys + WebSocket**

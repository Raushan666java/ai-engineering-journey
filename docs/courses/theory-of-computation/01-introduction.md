...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/01-introduction/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/01-introduction/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/01-introduction/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/01-introduction/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/01-introduction/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/01-introduction/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
**Cartesian product** A Ã— B = {(a,b) | a ? A, b ? B} is the set of all ordered pairs. This is the foundation for transition functions in automata.

### Relations and Functions


A **relation** R ? A Ã— B is a set of ordered pairs. A **function** f: A ? B is a relation where each a ? A maps to exactly one b ? B.

```text
f: N ? N, f(n) = nÂ²  // total function
g: N ? N, g(n) = 1/n  // partial function (undefined at n=0)
```

A function is **injective** (one-to-one), **surjective** (onto), or **bijective** (both). Bijections establish that two sets have the same **cardinality**. Countably infinite sets (N, Q) can be listed; uncountably infinite sets (R, ??(N)) cannot â€” this distinction drives undecidability.

### Graphs and Trees


A **directed graph** G = (V, E) consists of vertices V and edges E ? V Ã— V. A **tree** is a connected acyclic graph. Automata are labeled directed graphs where vertices are states and edges are transitions.
...
S2 = {a, b, c, â€¦, z}     // lowercase letters
S3 = {0, 1, 2, â€¦, 9}     // decimal digits
```
...
```text
L1 = {e, 0, 1, 00, 01, 10, 11, â€¦}  = S^*    (all binary strings)
...
| 1 | Context-Sensitive | Linear Bounded Automaton | aAÃŸ ? a?ÃŸ | {a^n b^n c^n} |
| 0 | Unrestricted | Turing Machine | a ? ÃŸ | {a^n | n is prime} |
...
Productions have the form aAÃŸ ? a?ÃŸ where ? ? e. A non-terminal A can be replaced only in the context of its surrounding strings a and ÃŸ. These correspond to linear bounded automata.
...
Productions have the form a ? ÃŸ where |a| = |ÃŸ|. No restrictions. These correspond exactly to Turing machines in generative power.
...
function recognizerForHalting(program: string, input: string): string {
  // This function cannot exist â€” proven by diagonalization
...
1. **Automata Theory** â€” finite and infinite-state machines that model computation. Covers DFA, NFA, PDA, Turing machines, and their language classes.

2. **Computability Theory** â€” what can and cannot be computed. Explores the halting problem, reductions, and the limits of algorithmic solvability.

3. **Complexity Theory** â€” how efficiently problems can be solved. Studies time and space bounds, the P vs NP question, and classification of problems by difficulty.
...
1. "Is graph G connected?" â€” **Decision** (yes/no answer)
2. "Find the shortest path from s to t." â€” **Optimization** (best among many)
3. "Multiply two matrices." â€” **Function** (compute output)
4. "Does program P halt on input x?" â€” **Decision** (yes/no, also undecidable)
...
2. **The Chomsky hierarchy gives a complexity roadmap.** When designing a parser or recognizer, choose the weakest grammar class that can express your language â€” regular for tokenization, context-free for syntax, context-sensitive for semantic analysis.
...
function cantorDiagonalization(): void {
  ...
  // at the corresponding diagonal position â€” exactly Cantor's proof.
  console.log("Diagonal language constructed â€” not in the original list.");
}
```
...
Formal language theory is not merely an abstract mathematical exercise â€” it has profound practical implications for computing:
...
// LBA â€” equal counts of all three
```
...
GÃ¶del's **Incompleteness Theorems** showed that any sufficiently powerful formal system contains statements that can neither be proved nor disproved within the system. This shattered Hilbert's dream of a complete, consistent axiomatization of all mathematics and laid the groundwork for undecidability.
...
- **1960sâ€“70s:** Cook, Karp, and Levin develop NP-completeness theory.
- **1970sâ€“80s:** Hartmanis, Stearns, and others develop computational complexity theory.
- **1990sâ€“2000s:** Interactive proofs (Goldwasser, Micali, Rackoff), PCP theorem, quantum computation.
- **2010sâ€“present:** Deep learning, LLMs, and the renewed philosophical debate about what constitutes "understanding" in computation â€” echoing Turing's original questions.
...
timeline
    ...
    1931 : GÃ¶del's Incompleteness Theorems
...
- **Is the human mind a computer?** This question, at the heart of the philosophy of AI, remains unresolved. GÃ¶del's theorems have been used (controversially) to argue that human mathematical intuition transcends formal computation.
...
function haltingDetector(program: string, input: string): boolean {
  // Hypothetical â€” this cannot exist
...
function classifyChomsky(productions: Production[]): ChomskyType {
  ...
  for (const p of productions) {
    ...
    // Type-1 (CSG): aAÃŸ ? a?ÃŸ with |?| = 1 (non-decreasing)
...
// ----------------------------------------------
// Formal Language Type Checker â€” validates that
...
class LanguageTypeChecker {
  ...
  // No constraints â€” any production form is allowed.
...
// ----------------------------------------------
// Chomsky Hierarchy Visualizer â€” generates a
...
class ChomskyHierarchyRenderer {
  static render(): string[] {
    return [
      "+------------------------------------------+",
      "Â¦     Chomsky Hierarchy (Classification)  Â¦",
      "Â¦------------------------------------------Â¦",
      "Â¦  Type-0: Unrestricted                    Â¦",
      "Â¦    ? Recursively enumerable languages    Â¦",
      "Â¦    ? Recognized by Turing machines       Â¦",
      "Â¦         ?                                Â¦",
      "Â¦  Type-1: Context-Sensitive               Â¦",
      "Â¦    ? Recognized by LBA                   Â¦",
      "Â¦    ? A context ? B context substitutions Â¦",
      "Â¦         ?                                Â¦",
      "Â¦  Type-2: Context-Free                    Â¦",
      "Â¦    ? Recognized by PDA                   Â¦",
      "Â¦    ? A ? a (single nonterminal LHS)      Â¦",
      "Â¦         ?                                Â¦",
      "Â¦  Type-3: Regular                         Â¦",
      "Â¦    ? Recognized by DFA / NFA             Â¦",
      "Â¦    ? A ? aB | a (right-linear)           Â¦",
      "+------------------------------------------+"
    ];
  }
}
...
- **Three pillars** â€” automata, computability, complexity â€” build on each other
...
9. Prove that if a language L is decidable, then its complement LÂ¯ is also decidable. What happens if L is only recognizable?
...
- **Sipser, Michael.** *Introduction to the Theory of Computation* (3rd ed.). Chapters 0â€“1 provide an excellent introduction to mathematical preliminaries and the Chomsky hierarchy.
- **Hopcroft, John E., Motwani, Rajeev, and Ullman, Jeffrey D.** *Introduction to Automata Theory, Languages, and Computation* (3rd ed.). Chapters 1â€“2 cover basic concepts and the regular/context-free classification.
...
- **Davis, Martin, Sigal, Ron, and Weyuker, Elaine J.** *Computability, Complexity, and Languages* (2nd ed.). A deeper exploration of the mathematical foundations including recursive function theory and the Âµ-recursive functions.

...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/09-turing/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/09-turing/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/09-turing/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/09-turing/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/theory-of-computation/09-turing/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/theory-of-computation/09-turing/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
A **Turing machine** is a 7-tuple (Q, ÃŽÂ£, ÃŽâ€œ, ÃŽÂ´, qÃ¢â€šâ‚¬, q_accept, q_reject) where:

- **Q** is a finite set of states.
- **ÃŽÂ£** is the input alphabet (does not contain the blank symbol).
- **ÃŽâ€œ** is the tape alphabet (ÃŽÂ£ Ã¢Å â€š ÃŽâ€œ, includes blank symbol Ã¢ÂÂ£).
- **ÃŽÂ´: Q Ãƒâ€” ÃŽâ€œ â†’ Q Ãƒâ€” ÃŽâ€œ Ãƒâ€” {L, R}** is the transition function.
- **qÃ¢â€šâ‚¬ Ã¢Ë†Ë† Q** is the start state.
- **q_accept Ã¢Ë†Ë† Q** is the accepting state.
- **q_reject Ã¢Ë†Ë† Q** is the rejecting state (q_reject Ã¢â€°Â  q_accept).

A transition ÃŽÂ´(q, a) = (r, b, L) means:
- In state q, reading symbol a on the tape,
- Write symbol b, move the head left, and go to state r.

**Computation:** Starting with the input on the tape (head at leftmost symbol), the TM repeatedly applies ÃŽÂ´ until it enters q_accept (accepts) or q_reject (rejects). The TM may also **loop forever** (never halt).
...
Example: `qÃ¢â€šâ‚¬ 0101` means state qÃ¢â€šâ‚¬, tape contains "0101", head at the first 0.

The start configuration on input w is: qÃ¢â€šâ‚¬ w.
...
A Turing machine M **accepts** string w if there is a sequence of configurations CÃ¢â€šâ‚¬, CÃ¢â€šÂ, Ã¢â‚¬Â¦, CÃ¢â€šâ€“ where:
- CÃ¢â€šâ‚¬ is the start configuration for w.
- Each CÃ¡ÂµÂ¢ yields CÃ¡ÂµÂ¢Ã¢â€šÅ Ã¢â€šÂ via ÃŽÂ´.
- CÃ¢â€šâ€“ is an accepting configuration.
...
1. **Marking symbols:** Use tape symbols with dots (e.g., ÃƒÂ¡) to mark already-processed symbols.
...
ÃŽÂ´: Q Ãƒâ€” ÃŽâ€œÃ¡ÂµÂ â†’ Q Ãƒâ€” ÃŽâ€œÃ¡ÂµÂ Ãƒâ€” {L, R}Ã¡ÂµÂ
...
**Proof sketch:** Use a single tape with "tracks" separated by a delimiter #. Each track stores the content of one tape. A special marker (Ã¡Â¸Æ’) indicates the position of each tape's head. Simulating one step of the k-tape machine may require sweeping the entire tape to find all head positions, making the simulation potentially slow but correct.
...
ÃŽÂ´: Q Ãƒâ€” ÃŽâ€œ â†’ P(Q Ãƒâ€” ÃŽâ€œ Ãƒâ€” {L, R})
...
### Example 8.1: TM for L = { aÃ¢ÂÂ¿bÃ¢ÂÂ¿cÃ¢ÂÂ¿ | n Ã¢â€°Â¥ 0 }
...
State design:
- qÃ¢â€šâ‚¬: Initial â†’ find first a, mark as X, go to qÃ¢â€šÂ
- qÃ¢â€šÂ: Finding b â†’ scan right over a, Y; mark first b as Y, go to qÃ¢â€šâ€š
- qÃ¢â€šâ€š: Finding c â†’ scan right over b, Z; mark first c as Z, go to qÃ¢â€šÆ’
- qÃ¢â€šÆ’: Return left â†’ scan left over X, Y, Z, a, b, c to leftmost; go to qÃ¢â€šâ‚¬
- qÃ¢â€šâ€ž: Verification â†’ check all symbols are X, Y, Z
- q_accept, q_reject
...
```
State qÃ¢â€šâ‚¬: move right to end of input
  ÃŽÂ´(qÃ¢â€šâ‚¬, 0) = (qÃ¢â€šâ‚¬, 0, R)
  ÃŽÂ´(qÃ¢â€šâ‚¬, 1) = (qÃ¢â€šâ‚¬, 1, R)
  ÃŽÂ´(qÃ¢â€šâ‚¬, Ã¢ÂÂ£) = (qÃ¢â€šÂ, Ã¢ÂÂ£, L)  -- reached end, start incrementing

State qÃ¢â€šÂ: increment current digit
  ÃŽÂ´(qÃ¢â€šÂ, 0) = (qÃ¢â€šâ€š, 1, L)  -- 0â†’1, done
  ÃŽÂ´(qÃ¢â€šÂ, 1) = (qÃ¢â€šÂ, 0, L)  -- 1â†’0, carry
  ÃŽÂ´(qÃ¢â€šÂ, Ã¢ÂÂ£) = (qÃ¢â€šâ€š, 1, L)  -- overflow: 1000... â†’ 1000...1

State qÃ¢â€šâ€š: move to start and halt
  ÃŽÂ´(qÃ¢â€šâ€š, 0) = (qÃ¢â€šâ€š, 0, L)
  ÃŽÂ´(qÃ¢â€šâ€š, 1) = (qÃ¢â€šâ€š, 1, L)
  ÃŽÂ´(qÃ¢â€šâ€š, Ã¢ÂÂ£) = (q_accept, Ã¢ÂÂ£, R)
```

Trace for "1011" (11): qÃ¢â€šâ‚¬1011 â†’ * â†’ 1011 qÃ¢â€šÂ (at Ã¢ÂÂ£) â†’ 101 qÃ¢â€šÂ 1 â†’ 10 qÃ¢â€šÂ 01 â†’ 1 qÃ¢â€šÂ 001 â†’ qÃ¢â€šâ€š 1100 â†’ * â†’ q_accept 1100 (12).

### Example 8.3: TM for Palindrome Recognition

Language: L = { w Ã¢Ë†Ë† {0,1}* | w = wÃŠâ‚¬ }.

**Strategy:**
1. Compare first and last symbols â†’ if they match, erase both and repeat.
2. If only ÃŽÂµ or one symbol remains, accept.

Transitions (sketch):
- qÃ¢â€šâ‚¬: Read first symbol. If 0 â†’ replace with X, go to qÃ¢â€šÂ (looking for 0 at end). If 1 â†’ replace with X, go to qÃ¢â€šâ€š. If blank â†’ accept.
- qÃ¢â€šÂ: Scan right to end, ignoring 0,1. At blank, move left. If 0 â†’ replace with X, go to qÃ¢â€šÆ’. If 1 or X â†’ reject.
- qÃ¢â€šâ€š: Symmetric to qÃ¢â€šÂ for 1.
- qÃ¢â€šÆ’: Scan left to beginning (past X's, 0's, 1's). At X â†’ move right to next unprocessed symbol, go to qÃ¢â€šâ‚¬.
...
2. Mark head positions with dots: #0Ã¡Â¸Æ’1#1Ã‡Å¸0#.
...
If any assignment satisfies the formula, the NTM accepts. The DTM simulation would try all 2Ã¢ÂÂ¿ assignments exponentially.
...
2. **TM design is programming at its most basic.** Designing a TM forces you to think about state, tape operations, and control flow at the most primitive level â€” analogous to programming in assembly language without a stack or registers.
...
class TuringMachine {
  ...
  static haltChecker(): { halts: boolean; reason: string } {
    ...
    return {
      halts: false,
      reason: "The halting problem is undecidable â€” no TM can determine if another TM halts"
    };
  }
}
...
// -----------------------------------------------------
// Busy Beaver Runner â€” simulates the classic Busy Beaver
...
class BusyBeaverRunner {
  ...
  static knownWinners(): string[] {
    return [
      ...
      "S(6) = 3.5Ã—10Â¹6Â²67 (astronomical â€” not maximal)",
      "",
...
1. Design a TM that recognizes L = { 0Ã¢ÂÂ¿1Ã¢ÂÂ¿ | n Ã¢â€°Â¥ 0 }.
...
7. Design a TM that recognizes L = { w Ã¢Ë†Ë† {a,b}* | w = wÃŠâ‚¬ } (palindromes).
8. Show formally that a TM with a doubly infinite tape is equivalent to a standard TM.
9. Design a 2-tape TM to recognize { aÃ¢ÂÂ¿bÃ¢ÂÂ¿cÃ¢ÂÂ¿ | n Ã¢â€°Â¥ 0 } and then simulate it on a single tape.
...
13. Design a TM that recognizes the language { aÃ¢ÂÂ¿ | n is a prime number }.
14. Prove that the simulation of an NTM by a DTM may require exponential time (show a language that an NTM decides in O(n) time but requires ÃŽÂ©(2Ã¢ÂÂ¿) time on a DTM).
...
Without the UTM concept, computers would be fixed-function devices â€” each machine dedicated to a single computation.
...
This is not a theorem â€” it's a claim about the nature of computation. It has been remarkably resilient:
...
Quantum computing challenges this thesis â€” Shor's algorithm factors in polynomial time on a quantum computer, but no polynomial-time TM factoring algorithm is known. Whether quantum computers violate the extended thesis remains an open question.

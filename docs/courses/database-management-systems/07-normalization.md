...
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/hero.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/hero.svg" alt="Chapter Banner: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/handwritten-notes.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/handwritten-notes.svg" alt="Handwritten Notes: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/sticky-notes.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/sticky-notes.svg" alt="Sticky Notes: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/visual-explanation.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/visual-explanation.svg" alt="Visual Explanation: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/architecture.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/architecture.svg" alt="Architecture: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/workflow.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/workflow.svg" alt="Workflow: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/mindmap.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/mindmap.svg" alt="Mind Map: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/comparison.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/comparison.svg" alt="Comparison: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/cheatsheet.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/cheatsheet.svg" alt="Cheat Sheet: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/interview-quiz.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/interview-quiz.svg" alt="Quiz Card: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
<a href="../../../assets/images/lessons/database-management-systems/07-normalization/social-card.svg" target="_blank" rel="noopener">
  <img src="../../../assets/images/lessons/database-management-systems/07-normalization/social-card.svg" alt="Social Card: Chapter 7: Normalization in Database Management Systems" width="18%" style="margin:2px;border:1px solid #e2e8f0;border-radius:6px;">
</a>
</div>
<!-- End Image Gallery -->
...
|-------|-------------|-------------------|
| **Functional Dependencies** | X -> Y means X determines Y â€” the foundation of all normalization | Find all FDs first; normalization follows automatically |
...
> **One-Sentence Takeaway:** Normalization systematically removes data redundancy by decomposing tables through 1NF -> 2NF -> 3NF -> BCNF â€” think of it as "each column depends on the key, the whole key, and nothing but the key."
...
|-------------|-----------|--------------------------|-------------|
| **Insertion Anomaly** | Cannot insert a fact because the primary key requires another fact that is missing | Adding course CS102 requires a student_id â€” but no student is enrolled yet | New entities cannot be recorded independently |
...
| **Update Anomaly** | Changing a fact requires updating multiple rows; inconsistency if any are missed | Dr. Smith moves to Room 201 â€” all 50 DBMS rows must be updated | Data inconsistency and maintenance overhead |
...
**Insertion Anomaly â€” Detailed:** We cannot add a new course (CS102, "Data Structures") unless a student enrolls in it. The primary key is (student_id, course_id), so we would need a student_id, which we do not have for a new course. This forces a dummy or null student_id, violating entity integrity.

**Update Anomaly â€” Detailed:** If "Dr. Smith" (instructor for "DBMS") moves to office "Room 201," we must update every row where Dr. Smith teaches. If there are 50 students in DBMS, we must update 50 rows. A single missed update creates inconsistency â€” some rows say Room 101, others say Room 201. How do we know which is correct?

**Deletion Anomaly â€” Detailed:** If the last student drops "DBMS" and we delete their enrollment row, we lose the information that "DBMS" is taught by "Dr. Smith" in "Room 101." The course and instructor information is gone forever â€” collateral damage from removing a student enrollment.

**Redundancy â€” Quantified:** The instructor name and office are repeated for every student in each course. With 50 students per course and 100 courses, instructor info appears 5000 times when it should appear 100 times. This wastes space, slows queries, and causes the update anomaly.
...
Think of a recipe: `dish_name -> ingredients, cooking_time, difficulty`. If you know the dish name, you uniquely know the ingredients and cooking time. This is a functional dependency (FD) â€” the dish name *functionally determines* the ingredients.

Now suppose two different recipes produce the same dish (e.g., two ways to make pasta carbonara). Then `dish_name -> ingredients` is FALSE â€” the same dish name gives different ingredients. FDs are constraints that must hold for *every possible* instance of the relation, not just the current data.
...
**Trivial Functional Dependencies â€” Details:**
- `student_id -> student_id` (trivial â€” always true)
- `student_id, name -> student_id` (trivial â€” right side subset of left)
- `student_id -> name` (non-trivial)
- `student_id -> student_id, name` (trivial even though it includes name? Actually `student_id, name` is not a subset of `{student_id}`, so NO â€” this is non-trivial because `name` is not in `{student_id}`. But `{student_id, name} -> {student_id}` IS trivial.)

**Edge Case â€” Trivial FDs in Closure:** When computing closure, trivial FDs are always true but never add new attributes. They are noise in the computation â€” we skip them because they contribute nothing.
...
|------|------|-----------------|---------|-------------|
| 1 | **Reflexivity** | If Y subset of X, then X -> Y | {A, B} -> A | Subset determines itself â€” obvious but formally necessary |
...
**Circular Dependencies:** FDs like {A -> B, B -> C, C -> A} create a cycle. In this case, A, B, and C are all candidate keys â€” each determines all others through the chain.

**Empty Left Side:** Is {} -> Y valid? This means "every tuple has the same Y value" â€” a constant constraint. For example, {} -> gender would mean all employees have the same gender. This is rare but valid.
...
| Aspect | Description |
|--------|-------------|
| **Advantage: Soundness** | Every derived FD is guaranteed to hold â€” no false positives |
| **Advantage: Completeness** | All implied FDs can be derived â€” no false negatives |
| **Advantage: Simplicity** | Only 3 rules to remember â€” easy to apply manually |
...
#### Closure Algorithm â€” Step-by-Step
...
#### Detailed Trace Table â€” Example 1
...
| 1 | {A, B, C} | CD -> E | Y={C,D} subset of {A,B,C}: NO (D missing) | â€” | {A, B, C} |
| 1 | {A, B, C} | B -> D | Y={B} subset of {A,B,C}: YES | D | {A, B, C, D} |
| 1 | {A, B, C, D} | E -> A | Y={E} subset of {A,B,C,D}: NO | â€” | {A, B, C, D} |
| 2 | {A, B, C, D} | A -> BC | YES â€” already in closure | â€” | {A, B, C, D} |
| 2 | {A, B, C, D} | CD -> E | Y={C,D} subset of {A,B,C,D}: YES | E | {A, B, C, D, E} |
| 2 | {A, B, C, D, E} | B -> D | YES â€” already in closure | â€” | {A, B, C, D, E} |
| 2 | {A, B, C, D, E} | E -> A | YES â€” already in closure | â€” | {A, B, C, D, E} |
| 3 | {A, B, C, D, E} | (all) | No new FDs apply | â€” | {A, B, C, D, E} |

**Result: A+ = {A, B, C, D, E}** (all attributes â€” A is a superkey)

**Compute B+:**

| Iteration | closure (before) | FD Checked | Y subset of closure? | Attributes Added | closure (after) |
|-----------|-----------------|------------|---------------------|-----------------|-----------------|
| 1 | {B} | A -> BC | Y={A} subset of {B}: NO | â€” | {B} |
| 1 | {B} | CD -> E | Y={C,D} subset of {B}: NO | â€” | {B} |
| 1 | {B} | B -> D | Y={B} subset of {B}: YES | D | {B, D} |
| 1 | {B, D} | E -> A | Y={E} subset of {B,D}: NO | â€” | {B, D} |
| 2 | {B, D} | (all) | No new FDs apply | â€” | {B, D} |

**Result: B+ = {B, D}** (not a superkey)

#### Detailed Trace Table â€” Example 2
...
| 1 | {A,B,C,D,E,F} | E -> A | YES â€” A already in closure | â€” | {A,B,C,D,E,F} |
| 2 | {A,B,C,D,E,F} | (all) | No new attributes | â€” | {A,B,C,D,E,F} |
...
| Aspect | Description |
|--------|-------------|
| **A: Algorithmic** | Simple iterative algorithm â€” always terminates |
...
**Empty Attribute Set:** Computing {} closure â€” start with empty set. Only FDs with empty left side (constant constraints) will add attributes. Result is typically {} unless there are constant constraints.
...
**Step 5:** A candidate key is minimal â€” remove any attribute that is not essential.
...
#### Detailed Dry Run â€” Finding Candidate Keys
...
| 1 | C | LH | C -> D (left only) â€” never on right |
...
- Remove A: {C}+ = {C, D, E} â€” not a key. A is needed.
- Remove C: {A}+ = {A, B, F} â€” not a key. C is needed.
...
Imagine a library catalog that stores (shelf_id, book_title, author_name, section_name). The shelf_id + book_title tells you the exact spot. But author_name depends only on book_title â€” it repeats on every shelf where that book sits. 2NF says: if your key is composite (shelf_id + book_title), every non-key column must depend on the WHOLE key, not just part of it. Author_name depends on book_title alone â€” that is a partial dependency. Move it out.
...
**FDs identified:**
- `student_id -> student_name` (partial dependency â€” depends on part of PK)
- `course_id -> course_name` (partial dependency)
- `student_id, course_id -> grade` (full dependency â€” depends on whole PK)

**Checking 2NF:**

| Non-key Attribute | Depends on Which Key Part? | Partial? | Verdict |
|------------------|---------------------------|----------|---------|
| student_name | student_id (proper subset) | YES â€” violates 2NF | Must remove |
| grade | (student_id, course_id) â€” whole key | NO â€” full dependency | Keep |
| course_name | course_id (proper subset) | YES â€” violates 2NF | Must remove |

**Decomposition:**

R1(student_id, student_name, address, major) â€” PK: student_id
R2(course_id, course_name, credits) â€” PK: course_id
R3(student_id, course_id, grade) â€” PK: (student_id, course_id)

#### Dry Run Trace â€” Complete 2NF Decomposition
...
- student_id -> student_name: Alice repeats for S1 twice â†’ update anomaly if she changes name
- course_id -> course_name: DBMS repeats twice â†’ update anomaly if course name changes
...
#### Detailed Dry Run â€” 3NF Decomposition
...
**Decomposition:**
R1(order_id, order_date, customer_id) â€” PK: order_id
R2(customer_id, customer_name) â€” PK: customer_id
...
**Step 3: Check 1NF â€” PASS (all values atomic)**
...
#### Detailed Dry Run â€” BCNF Decomposition
...
**Step 2:** Check BCNF:
- course_id -> instructor_name: Is course_id a superkey? NO (course_id+ = {course_id, instructor_name} â€” does not contain student_id). VIOLATION.

**Step 3:** Decompose using violating FD course_id -> instructor_name:
- R1 = {course_id, instructor_name} â€” PK: course_id
- R2 = {student_id, course_id} â€” PK: (student_id, course_id)
...
- Is C a superkey? C+ = {C, B} â€” does NOT contain A. NO.
...
A decomposition of R into R1 and R2 is **lossless** (or non-additive) if joining R1 and R2 always produces the original R â€” no spurious tuples, no missing tuples.

**Formally:** For every instance r of R: r = Ï€_R1(r) â‹ˆ Ï€_R2(r)
...
| **Acceptable?** | YES â€” required | NO â€” must be avoided |
...
#### Detailed Example â€” Lossless vs Lossy Join
...
**Formally:** For decomposition Ï = {R1, R2, ..., Rn} and FD set F:
The projection of F onto Ri is: Ï€_Ri(F) = {X -> Y in F+ | X union Y is subset of Ri}
The decomposition preserves F if (Ï€_R1(F) union Ï€_R2(F) union ... union Ï€_Rn(F))+ = F+

#### Example â€” Non-Dependency-Preserving Decomposition
...
```python
"""
Normalization Toolkit â€” Attribute Closure and Normalization Checker
"""
...
class SchemaAnalyzer:
    ...
    def _find_violations(self) -> List[str]:
        ...
        for fd in self.fds:
            ...
            if not is_superkey:
                ...
                violations.append(
                    f"BCNF: {fd} â€” LHS {set(fd.lhs)} not a superkey, "
                ...
                if not is_superkey:
                    for attr in fd.rhs:
                        if attr not in self.prime_attrs:
                            violations.append(
                                f"Would-be BCNF: {fd} â€” but in 3NF because "
    ...
    def report(self) -> str:
        ...
        if self.violations:
            ...
            lines.append("No violations â€” schema is fully normalized.")
...
#### Example â€” Overlapping Keys Affecting BCNF

R(A, B, C) with FDs {AB -> C, C -> B}:
- Candidate keys: {A, B}, {A, C}
- BCNF for C -> B: Is C a superkey? C+ = {C, B} â€” no A. NOT a superkey.
...
| **When to prefer** | Default â€” preserves all constraints | High-integrity systems willing to sacrifice FDs |
...
#### Q3: Denormalization â€” when and why?
...
**Interview Answer:** "Lossless decomposition guarantees joining the tables recovers exactly the original rows â€” no spurious tuples. Lossy creates phantom rows. The test: if the common attribute is a key in at least one component, the join is lossless. Lossless is mandatory â€” lossy decompositions corrupt data."
...
**Interview Answer:** "Yes. The classic example: R(A, B, C) with {AB -> C, C -> B}. Keys: {A,B}, {A,C}. FD C -> B â€” C is not a superkey, but B is prime. 3NF allows it; BCNF does not. The 3NF relaxation for prime RHS attributes is exactly what makes this possible."

#### Q8: Explain the 3NF synthesis algorithm

**Interview Answer:** "Four steps: (1) Compute minimal cover â€” decompose RHS, remove redundant LHS attributes, remove redundant FDs. (2) Group FDs by left-hand side, creating a schema with LHS union RHS for each group. (3) If no schema contains a candidate key, add one. (4) Remove redundant schemas that are subsets of others. The result is lossless and dependency-preserving."

#### Q9: Why is BCNF decomposition not dependency-preserving?

**Interview Answer:** "BCNF decomposition splits relations on violating FDs. After splitting, an FD whose attributes span multiple decomposed relations cannot be enforced locally. Example: R(student_id, course_id, instructor) with course_id -> instructor. After decomposing to R1(course_id, instructor) and R2(student_id, course_id), the FD (student_id, course_id) -> instructor is lost â€” it requires a join to enforce."
...
|---------|------------------------|
| **AUTO_INCREMENT** | Surrogate keys â€” natural candidate keys need UNIQUE constraints |
...
| **DOMAIN types** | Enforce atomicity at type level â€” stronger 1NF |
| **CHECK constraints** | Validate attribute format |
| **Partial/covering indexes** | Reduce JOIN cost â€” less need for denormalization |
...
```
Is every value atomic? â”€â”€â”€NOâ”€â”€â”€> Apply 1NF
       â”‚
      YES
       â”‚
Is the key single-attr? â”€â”€â”€NOâ”€â”€â”€> Check partial dependencies
       â”‚                              â”‚
      YES                        Has partial FDs?
       â”‚                         YESâ”€â”€â”€> Decompose to 2NF
       â”‚                           â”‚
       â”‚                          NO
       â”‚                           â”‚
       â”‚                           â–¼
       â”‚                    Check transitive deps on non-key attrs
       â”‚                              â”‚
       â”‚                     Has transitive? â”€â”€YESâ”€â”€> Decompose to 3NF
       â”‚                              â”‚
       â”‚                             NO
       â”‚                              â”‚
       â”‚                              â–¼
       â”‚                     Check FDs where LHS is not superkey
       â”‚                              â”‚
       â”‚                     Any violation? â”€â”€YESâ”€â”€> Consider BCNF (may lose FDs)
       â”‚                              â”‚
       â”‚                             NO
       â”‚                              â”‚
       â–¼                              â–¼
Check MVDs (4NF) â”€â”€> Schema is BCNF âœ“ (or 3NF if BCNF not possible)
```
...
```typescript
// ============================================================
// Functional Dependency Analyzer â€” TypeScript
// ============================================================
  ...
  checkNormalForm(): string {
    ...
    for (const fd of this.fds) {
      ...
      if (!isSuperkey) {
        ...
        if (!allPrime) {
          return '1NF (violates BCNF and 3NF â€” FD ' + this.fdToString(fd) + ' has non-prime RHS and LHS not a superkey)';
        }
      }
    }
    // Check 3NF violation
    for (const fd of this.fds) {
      ...
      if (!isSuperkey) {
        return '3NF (BCNF violated by FD ' + this.fdToString(fd) + ' but RHS is prime â€” 3NF holds)';
      }
    }
    return 'BCNF';
  }
...
- Attribute closure (X+) finds all attributes determined by X â€” the single most useful tool for normalization. Used to find keys, check FDs, and verify normal forms.
...
- 1NF eliminates non-atomic values and repeating groups â€” prerequisite for all relational operations.
- 2NF eliminates partial dependencies on composite keys â€” relevant only for tables with composite primary keys.
- 3NF eliminates transitive dependencies on non-key attributes â€” removes most practical redundancy.
- BCNF requires every FD left side to be a superkey â€” the strongest form based on FDs but may lose dependency preservation.
- Lossless decomposition guarantees data recoverability â€” mandatory for all decompositions.
- Dependency preservation ensures FDs can be enforced locally â€” 3NF synthesis guarantees both lossless join AND dependency preservation.
- The 3NF synthesis algorithm (minimal cover, group by LHS, add key, remove redundants) is the standard approach.
- BCNF is ideal but 3NF is practical â€” in real systems, we target BCNF but accept 3NF when BCNF would lose important constraints.
- Denormalization is intentional redundancy for performance â€” apply only after profiling demonstrates a bottleneck.
...
- **7.3:** Armstrong's axioms (reflexivity, augmentation, transitivity) form a sound and complete FD inference system â€” all six rules derive from just these three.
- **7.4:** Attribute closure (X+) finds all attributes determined by X â€” the key tool for finding keys, checking FDs, and normalization.
- **7.5:** Candidate keys are minimal superkeys â€” found by classifying attributes into four categories and testing closure.
- **7.6:** 1NF requires atomic attribute values and no repeating groups â€” enables all relational operations.
- **7.7:** 2NF eliminates partial dependencies â€” non-key attributes must depend on the entire composite key.
- **7.8:** 3NF eliminates transitive dependencies â€” a non-key attribute must not determine another non-key attribute.
- **7.9:** BCNF requires every FD left side to be a superkey â€” stronger than 3NF but may lose dependency preservation.
- **7.10:** Lossless decomposition guarantees JOIN recovers original rows â€” tested by checking if the common attribute is a key in one component.
- **7.11:** Dependency preservation means all FDs enforceable on decomposed tables without JOINs â€” 3NF synthesis guarantees this.
- **7.12-7.17:** C++ and Python implementations automate closure, key finding, 3NF/BCNF decomposition, and schema analysis.
- **7.18:** Multiple candidate keys affect normalization â€” partial dependencies must be checked against ALL candidate keys.

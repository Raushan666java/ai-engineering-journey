...
1. Implement a tracer that creates a trace tree with 3-5 spans (agent → retrieve → llm → format → output). Record timing and report total duration.
...
5. Implement an anomaly detector using both IQR and moving average Z-score methods. Test with a dataset containing 3 anomalous values and evaluate detection accuracy.

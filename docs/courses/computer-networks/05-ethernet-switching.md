...
<section class="lesson-visuals" aria-label="Visual learning resources">
  ...
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/05-ethernet-switching/handwritten-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/05-ethernet-switching/handwritten-notes.png" alt="Handwritten notes" loading="lazy">
    <span><strong>Handwritten notes</strong>Condensed notes for deliberate review.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/05-ethernet-switching/sticky-notes.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/05-ethernet-switching/sticky-notes.png" alt="Sticky-note revision" loading="lazy">
    <span><strong>Sticky-note revision</strong>Fast recall prompts for revision.</span>
  </a>
  <a class="lesson-visual-card" href="../../assets/images/lessons/computer-networks/05-ethernet-switching/visual-explanation.png" target="_blank" rel="noopener">
    <img src="../../assets/images/lessons/computer-networks/05-ethernet-switching/visual-explanation.png" alt="Visual concept guide" loading="lazy">
...
|-------|------|-------------|
| **Preamble** | 7 bytes | Alternating 1 and 0 bits (10101010 Ãƒâ€” 7). Enables receiver NIC to synchronize clock recovery before the frame arrives. |
...
| **Length/Type** | 2 bytes | If value Ã¢â€°Â¤ 1500 (0x05DC): indicates payload length in bytes (IEEE 802.3). If value Ã¢â€°Â¥ 1536 (0x0600): indicates EtherType (DIX Ethernet). Common EtherTypes: 0x0800 (IPv4), 0x0806 (ARP), 0x86DD (IPv6), 0x8100 (802.1Q VLAN tag). |
| **Payload + Pad** | 46Ã¢â‚¬â€œ1500 bytes | Network-layer PDU (e.g., IP packet). If payload &lt; 46 bytes, padding zeros are added to meet the 64-byte minimum frame size. |
...
5. **Type/Length interpretation** â†’ The 2-byte field is read. If Ã¢â€°Â¤ 1500, it is a length field (802.3); the receiver expects exactly that many payload bytes. If Ã¢â€°Â¥ 1536, it is an EtherType; the payload length is inferred from the frame size minus headers.
6. **Payload extraction** â†’ The data portion (46Ã¢â‚¬â€œ1500 bytes) is passed to the upper-layer protocol indicated by EtherType.
...
| 10 | Protocol | Dispatch payload | Length Ã¢â€°Â¤ 1500 â†’ 802.3 frame |
...
| CRC-32 protection | Detects all single-bit, double-bit, and burst errors Ã¢â€°Â¤ 32 bits | No error correction â†’ corrupted frames are silently dropped |
...
| EtherType ambiguity | Some NICs misclassify length vs type when value is 1500Ã¢â‚¬â€œ1535 | Modern switches treat values > 1500 as EtherType per IEEE 802.3x |
...
| 40 Gigabit | 802.3ba | 2010 | 40 Gbps | SMF/MMF, Cat6a (CX) | 100 m (CX), 10 km (SMF) | 4Ãƒâ€”10G lanes, QSFP form factor |
| 100 Gigabit | 802.3ba | 2010 | 100 Gbps | SMF/MMF | 40 km (SMF) | 10Ãƒâ€”10G or 4Ãƒâ€”25G lanes, CFP/QSFP28 |
| 200 Gigabit | 802.3bs | 2017 | 200 Gbps | SMF/MMF | 2 km (MMF), 10 km (SMF) | PAM4, 4Ãƒâ€”50G lanes |
| 400 Gigabit | 802.3bs | 2017 | 400 Gbps | SMF/MMF | 2 km (MMF), 10 km (SMF) | 16Ãƒâ€”25G or 8Ãƒâ€”50G PAM4, QSFP-DD/OSFP |
...
| NRZ â†’ PAM4 transition | 400 Gbps | 8Ãƒâ€”50G PAM4 lanes replaced 16Ãƒâ€”25G NRZ lanes for better density |
...
**Store-and-forward switching.** The switch receives the entire frame, checks the FCS for errors, and then forwards. This ensures no corrupted frames propagate but adds latency proportional to frame size. Latency = frame_size / link_speed. For a 1500-byte frame on 1 Gbps: 1500Ãƒâ€”8/1e9 = 12 Ã‚Âµs.

**Cut-through switching.** The switch begins forwarding before the complete frame arrives â†’ typically after reading only the destination MAC address (first 6 bytes of the frame, 14 bytes from preamble start). Latency is typically &lt; 10 Ã‚Âµs regardless of frame size, but damaged frames are forwarded. Two variants: fast-forward (forwards after dst MAC) and fragment-free (forwards after first 64 bytes).
...
| Latency | Lowest (repeats bits immediately) | Low (Ã‚Âµs) | Higher (ms, due to IP lookup) |
...
Consider a 4-port switch (ports 1Ã¢â‚¬â€œ4). Initially the MAC table is empty. Hosts A, B, C connected on ports 1, 2, 3 respectively. Host D connected on port 4. The switch processes frames in sequence:
...
| T=7 | Aâ†’B frame | 1 | A | B | {Aâ†’1, Bâ†’2, Câ†’3, Dâ†’4} | Refresh A; B known on port 2 Ã¢â€°Â  1 â†’ forward only port 2 |
| T=8 | Bâ†’A frame | 2 | B | A | {Aâ†’1, Bâ†’2, Câ†’3, Dâ†’4} | Refresh B; A known on port 1 Ã¢â€°Â  2 â†’ forward only port 1 |
| T=9 | Aâ†’A frame (self) | 1 | A | A | {Aâ†’1, Bâ†’2, Câ†’3, Dâ†’4} | Refresh A; A known on port 1 == 1 â†’ filter (drop) |

After T=6, all four hosts are learned. After T=7Ã¢â‚¬â€œ8, frames between known hosts are forwarded precisely to the correct port â†’ no flooding. After T=9, a self-addressed frame is filtered because the source and destination port match (the destination is on the same segment as the source).
...
| Frame flooding | O(p) + O(f) | O(pÃ‚Â·f) | Must replicate frame to up to p-1 ports. Each copy costs O(f) bandwidth. |
...
| Cut-through | Minimal latency (< 10 Ã‚Âµs) | Forwards corrupted frames and runt frames |
...
class SwitchForwardingTable {
  ...
  learn(mac: string, port: number): void {
    ...
    console.log(`Learned: ${mac} â†’ port ${port}`);
  }
...
console.log(sw.processFrame("AA:BB:CC:DD:EE:02", "AA:BB:CC:DD:EE:01", 2)); // Learn 02â†’2, forward 01â†’1
console.log(sw.processFrame("AA:BB:CC:DD:EE:02", "AA:BB:CC:DD:EE:02", 2)); // Filter (same port)
console.log(sw.processFrame("AA:BB:CC:DD:EE:03", "AA:BB:CC:DD:EE:01", 3)); // Learn 03â†’3, forward 01â†’1
...
4. **Destination resolution** â†’ The bridge looks up D in the MAC table.
   - If D is found on port Q (Q Ã¢â€°Â  P): forward frame to port Q only.
...
Four-port bridge, hosts AÃ¢â‚¬â€œF on ports 1Ã¢â‚¬â€œ4 (A and B on port 1 via hub, C and D on port 2, E on port 3, F on port 4).
...
| Flood to p ports | O(p Ã‚Â· f) | O(p Ã‚Â· f) | Each of p egress ports gets a frame copy of size f |
...
In a triangle topology of three switches (AÃ¢â‚¬â€œB, BÃ¢â‚¬â€œC, CÃ¢â‚¬â€œA connected), a broadcast frame from any host would circulate forever:
...
| Operation | Time Complexity | Space Complexity | Why |
|-----------|---------------|-----------------|-----|
| Root bridge election | O(bÃ‚Â²) | O(1) | Every bridge sends BPDUs; worst case b bridges exchange b BPDUs each |
...
| Self-configuring | Requires no manual path planning | Non-optimal path selection; best path Ã¢â€°Â  shortest latency path |
...
class STPSimulator {
  ...
  simulateFailure(failedBridgeId: number): void {
    console.log(`\nBridge ${failedBridgeId} fails â€” re-electing...`);
...
net.addBridge(3, 4096, "00:11:22:33:44:03"); // lower priority â†’ should be root
...
net.simulateFailure(3); // Root fails â†’ re-election
```
...
- **PCP** (3 bits): Priority Code Point â†’ 802.1p class of service (0Ã¢â‚¬â€œ7). Used for QoS.
- **DEI** (1 bit): Drop Eligible Indicator. When set, frame may be dropped during congestion.
- **VID** (12 bits): VLAN Identifier (1Ã¢â‚¬â€œ4094; 0 = priority only, 4095 = reserved).
...
| VLAN-scoped MAC lookup | O(1) avg | O(mÃ‚Â·v)* | Hash key is (VLAN, MAC) pair; m MACs Ãƒâ€” v VLANs |
| Tag insertion | O(f) | O(1) | Shift frame payload by 4 bytes; update FCS |
| Tag removal | O(f) | O(1) | Remove 4 bytes; update FCS |
| Flood within VLAN | O(pÃ‚Â·f) | O(f) | Replicate to up to p ports in same VLAN |
...
class VLANManager {
  ...
  private vlanPortMap: Map<number, number[]> = new Map(); // port â†’ VLAN list
...
| Cut-Through | After dst MAC (14B) | No | Forwarded | Forwarded | Lowest (< 10Ã‚Âµs) | HPC, low-latency trading |
...
| Latency | ~1 Ã‚Âµs | 5-50 Ã‚Âµs | 100 Ã‚Âµs - 5 ms |
...
| 100GbE | 100 Gbps | SMF/MMF | 4Ãƒâ€”25G lanes |
| 400GbE | 400 Gbps | SMF/MMF | 8 lanes Ãƒâ€” 50 Gbps PAM4 |
...
**Background:** A fintech company operates a data center with 1000 servers across 25 racks (40 servers/rack). Requirements: (a) low latency for trading applications (< 10 Âµs switch latency), (b) 10:1 oversubscription ratio max, (c) separate VLANs for production, staging, and management traffic.

**Design â€” Leaf-Spine Topology:**
1. **Leaf layer (ToR switches):** 25 switches (one per rack), each with 48Ã—10G server ports, 4Ã—40G uplinks to spines. Each leaf is a L2 switch with 802.1Q trunk uplinks.
2. **Spine layer:** 4 switches, each with 48Ã—40G ports (total 192 uplink ports â€” each leaf connects to all 4 spines for redundancy).
...
- Downlink capacity per leaf: 48 Ã— 10G = 480 Gbps
- Uplink capacity per leaf: 4 Ã— 40G = 160 Gbps
- Oversubscription ratio: 480/160 = 3:1 (within the 10:1 requirement)

**STP considerations:** In a leaf-spine with 4 spines, MSTP creates multiple spanning tree instances â€” one per VLAN group. VLAN 100 and VLAN 200 can use different spines as root, balancing load across all 4 spines.

**Outcome:** 3:1 oversubscription, < 5 Âµs leaf-to-spine latency, VLAN isolation between environments, and full redundancy (any single spine failure reduces uplink capacity by 25 % but does not disconnect any server).

## Practical Takeaways

| Takeaway | Application |
|----------|------------|
| **Ethernet frame format** has been stable for 40+ years | Always validate FCS when capturing packets â€” corrupted frames should never pass a switch |
...
| **MAC table size** determines scalability | A 48-port switch with 1000 MACs per port needs ~48K entries â€” ensure hardware TCAM supports this |
...
| **Leaf-spine topology** provides predictable latency and scale | Each leaf connects to every spine â€” no oversubscription at spine if N Ã— leaf_uplink â‰¤ spine_ports |
...
3. Cut-through starts forwarding after reading only the Dst MAC (first 6 bytes of frame) â€” it does not wait for the full frame or check FCS, so corrupted frames are forwarded.
4. STP prevents loops in redundant bridge topologies by blocking selected ports to create a loop-free tree.
5. 12 bits â†’ 2^12 - 2 = 4094 usable VLANs (VLAN 0 and 4095 reserved).
6. Preamble (7B, sync), SFD (1B, start), Dst MAC (6B), Src MAC (6B), Type/Len (2B), Payload (46-1500B), FCS (4B, CRC-32).
7. STP: Blocking â†’ Listening â†’ Learning â†’ Forwarding (30-50s). RSTP: Discarding â†’ Learning â†’ Forwarding (1-3s); adds alternate/backup port roles.
8. Hash of L2/L3/L4 fields maps each flow to the same link consistently â€” preventing reordering within a flow.
...
11. After learning Aâ†’2, Bâ†’2, Câ†’2, the frame to A arrives on port 5. Dst = A is found on port 2 â†’ forward to port 2 only.
...
13. BDP = 2 Ã— T_prop Ã— R. T_prop = 100 km Ã— 1 Âµs/km = 100 Âµs. BDP = 2 Ã— 100 Ã— 10^-6 Ã— 10^10 = 2 Ã— 10^6 bits = 2,000,000 bits. Frames = 2,000,000 / (1500 Ã— 8) = 166.7 frames.
14. VLAN 10: ports 1-6, VLAN 20: ports 7-12, int gi0/24 switchport trunk allowed vlan 10,20, switchport mode trunk.
15. Compute hash for each flow: flow1 hash(AA:BB:CC:DD:EE:01, ...) â†’ link 2, flow2 â†’ link 0, flow3 â†’ link 3.
</details>
...
16. **Design:** 5000 servers / 40 per rack = 125 ToRs. 125 ToRs Ã— 4 uplinks = 500 uplinks. 4 core switches Ã— 48 ports = 192 uplink capacity. Oversubscription = 5000 Ã— 10G / (4 Ã— 48 Ã— 40G) = 50 Tbps / 7.68 Tbps â‰ˆ 6.5:1. **Modification:** Increase to 8 core switches (96 ports Ã— 40G = 3.84 Tbps per core â†’ 15.36 Tbps total) â†’ oversubscription = 50/15.36 â‰ˆ 3.25:1. Or use 100G uplinks on ToRs with 8 core switches.
</details>

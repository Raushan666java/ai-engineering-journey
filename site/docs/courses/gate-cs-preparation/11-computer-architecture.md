...
1. **Fetch:** PC → MAR → memory → MDR → IR, PC â†Â� PC + 1
...
| GPRs (General Purpose Regs) | word Ãâ€â€� N | Operands, addresses, temporaries |
...
| Autoincrement | EA = R; R â†Â� R + 1 | `LW R1, (R2)+` | String ops |
| Autodecrement | R â†Â� R − 1; EA = R | `LW R1, −(R2)` | Stack push |
| Scaled indexed | EA = base + (index Ãâ€â€� scale) | `LW R1, arr(R2Ãâ€â€�4)` | Word arrays |
...
- `ADD X` → ACC â†Â� ACC + M[X]
- Second operand is always accumulator

**Two-address (CISC):**
- `ADD R1, R2` → R1 â†Â� R1 + R2 (destructive)
- `MOV R1, R2` → R1 â†Â� R2

**Three-address (RISC):**
- `ADD R1, R2, R3` → R1 â†Â� R2 + R3 (non-destructive)
...
- Value = (integer part) + (fractional part) Ãâ€â€� 2^(−n)
...
**Value = (−1)^S Ãâ€â€� 1.M Ãâ€â€� 2^(E−127)**
...
|----------|----------|---------|
| 0 | 0 | ±0 (signed zero) |
| 0 | ≠ 0 | Denormalized: (−1)^S Ãâ€â€� 0.M Ãâ€â€� 2^(−126) |
| 1–254 | anything | Normalized: (−1)^S Ãâ€â€� 1.M Ãâ€â€� 2^(E−127) |
| 255 | 0 | ±Infinity |
| 255 | ≠ 0 | NaN (Not a Number) |

**Range:** ±1.18 Ãâ€â€� 10^(−38) to ±3.4 Ãâ€â€� 10^(38)
...
**Value = (−1)^S Ãâ€â€� 1.M Ãâ€â€� 2^(E−1023)**

**Bias:** 1023
**Range:** ±2.23 Ãâ€â€� 10^(−308) to ±1.79 Ãâ€â€� 10^(308)
...
| Largest normalized | ≈ 3.4 Ãâ€â€� 10^38 | ≈ 1.79 Ãâ€â€� 10^308 |
| Smallest positive normalized | ≈ 1.18 Ãâ€â€� 10^(−38) | ≈ 2.23 Ãâ€â€� 10^(−308) |
| Machine epsilon (1 + ε > 1) | 2^(−23) ≈ 1.19 Ãâ€â€� 10^(−7) | 2^(−52) ≈ 2.22 Ãâ€â€� 10^(−16) |
...
2. Multiply mantissas (1.M Ãâ€â€� 1.M)
...
- Delay = n Ãâ€â€� t_FA (where t_FA = delay of one full adder)
...
**GATE tip:** When pipelines are given with branch penalties and data hazard stalls, compute effective CPI = 1 + stall_rate Ãâ€â€� stall_cycles.
...
```
Effective Access Time (EAT) = Hit time + Miss rate Ãâ€â€� Miss penalty
...
- Effective bandwidth = m Ãâ€â€� single-bank bandwidth (with aligned sequential access)

**GATE tip:** Interleaving reduces the effective memory access time for sequential access patterns. For k banks, the time to access k consecutive words is T + (k−1) Ãâ€â€� t, where T is first-word access time and t is per-word access time.
...
```
EAT = TLB_hit_rate Ãâ€â€� (TLB_hit_time + cache_access_time)
...
| Formula | Expression |
|---------|------------|
| AMAT (single-level) | Hit time + Miss rate Ãâ€â€� Miss penalty |
| AMAT (two-level) | L1 hit + L1 miss Ãâ€â€� (L2 hit + L2 miss Ãâ€â€� L2 miss penalty) |
| Miss penalty | = Memory access time (or L2 hit time for L1) |
| CPI with cache misses | CPI_execution + (Miss rate Ãâ€â€� Miss penalty / instructions) |
| Speedup | (1 + old_stall_cycles) / (1 + new_stall_cycles) |
| Average memory time with interleaving | T_first + (N−1) Ãâ€â€� t_per_word |
...
**Example:** 90% parallelizable → max speedup = 1/0.1 = 10Ãâ€â€� (even with 1000 processors)

### 9.6 Gustafson's Law (Scaled Speedup)


```
Scaled speedup = S + P Ãâ€â€� (1 − S)
```
...
| Metric | Formula |
|--------|---------|
| CPU Time | = Instructions Ãâ€â€� CPI Ãâ€â€� Clock cycle time |
| CPU Time | = Instructions Ãâ€â€� CPI / Clock rate |
| MIPS | = Instruction count / (Execution time Ãâ€â€� 10^6) |
| MIPS | = Clock rate / (CPI Ãâ€â€� 10^6) |
| CPI | = Σ (CPI_i Ãâ€â€� Frequency_i) |
...
| Actual speedup | = k Ãâ€â€� (1 + stall_cycles) |
| Effective CPI | = 1 + stall_rate Ãâ€â€� stall_cycles |
| Branch penalty | = misprediction_rate Ãâ€â€� branch_frequency Ãâ€â€� penalty_cycles |
| Data hazard penalty | = hazard_rate Ãâ€â€� hazard_stalls |
| Pipeline efficiency | = Speedup / k Ãâ€â€� 100% |
...
| Parameter | Formula |
|-----------|---------|
| Cache size | C = Number of blocks Ãâ€â€� Block size |
...
|----------|----------------|-----------------|
| Format | (−1)^S Ãâ€â€� 1.M Ãâ€â€� 2^(E−127) | (−1)^S Ãâ€â€� 1.M Ãâ€â€� 2^(E−1023) |
| Bias | 127 | 1023 |
| Max exponent (biased) | 254 | 2046 |
| Min exponent (biased) | 1 | 1 |
| Max value | 2^(254−127) Ãâ€â€� (2 − 2^(−23)) ≈ 3.4Ãâ€â€�10^38 | ≈ 1.8Ãâ€â€�10^308 |
| Min positive normalized | 2^(−126) ≈ 1.18Ãâ€â€�10^(−38) | 2^(−1022) ≈ 2.23Ãâ€â€�10^(−308) |
| Smallest denormalized | 2^(−149) ≈ 1.4Ãâ€â€�10^(−45) | 2^(−1074) ≈ 4.94Ãâ€â€�10^(−324) |
| Machine epsilon | 2^(−23) ≈ 1.19Ãâ€â€�10^(−7) | 2^(−52) ≈ 2.22Ãâ€â€�10^(−16) |
...
| Metric | Formula |
|--------|---------|
| Page table size | = (Virtual address space / Page size) Ãâ€â€� PTE size |
| TLB reach | = TLB entries Ãâ€â€� Page size |
| Effective memory access (with TLB) | = TLB_hit Ãâ€â€� (TLB_time + Mem_time) + TLB_miss Ãâ€â€� (TLB_miss_penalty) |
| Interleaving bandwidth | = Words per cycle = Number of banks (on sequential access) |
| DRAM refresh overhead | = (Refresh_cycles / Refresh_interval) Ãâ€â€� 100% |
...
Alternatively: 64 KB = 32768 words Ãâ€â€� 20 ns/word = 655.36 μs
...
AMAT = L1_hit_time + L1_miss_rate Ãâ€â€� (L2_hit_time + L2_miss_rate Ãâ€â€� Mem_time)
...
Format A: 4 opcode + 2 Ãâ€â€� 6 = 16 → Only 4+12=16 → all 16 bits used → 2^4 = 16 instructions possible
...
Average CPI = 0.40Ãâ€â€�1 + 0.20Ãâ€â€�2 + 0.15Ãâ€â€�1 + 0.25Ãâ€â€�4
            = 0.40 + 0.40 + 0.15 + 1.00
            = 1.95
...
Total time = 100 + 7Ãâ€â€�10 = 170 ns (without interleaving it'd be 100 + 7Ãâ€â€�10 = 170 as well... wait, without interleaving it's all one bank)
...
Without interleaving: T_A + (N−1) Ãâ€â€� T_w
...
- Time per N words = T_A + ((N−1)/m) Ãâ€â€� ... no
...
So total = 100 + 7 Ãâ€â€� 10 = 170 ns

vs without interleaving: 100 + 7 Ãâ€â€� 10 = 170 ns... they're the same?!
...
BUT with interleaving, the effective bandwidth is 4Ãâ€â€� higher if banks work in parallel. The difference shows up in the access time to the first word vs. burst rates.
...
**Answer:** 100 + 7 Ãâ€â€� 10 = 170 ns (with 4-way interleaving → same total time for this sequential pattern due to bus-limited transfer)
...
Total (worst case) = 4 KB + 2^10 Ãâ€â€� 4 KB = 4 KB + 4 MB ≈ 4 MB
...
EAT = 0.95 Ãâ€â€� (10 + 100) + 0.05 Ãâ€â€� (10 + 200 + 100)
...
Normalize: 1.000000000000000000000011 Ãâ€â€� 2^1
Exponent = 128 = 10000000
...
DMA cycles needed = 4 Ãâ€â€� 10^6 cycles/s
...
If we assume write hit rate same as read: 95% → 0.30 Ãâ€â€� 0.95 = 0.285 write hits/instruction
Write hit traffic: 0.285 Ãâ€â€� 8 = 2.28 bytes/instruction

Write misses: 0.30 Ãâ€â€� 0.05 = 0.015
...
| Largest normalized single | ≈ 3.4 Ãâ€â€� 10^38 |
...
*End of Chapter 11. This chapter covers all GATE-relevant topics in Computer Organization & Architecture based on the 2010â€â€�"2025 GATE exam pattern.*
...
| 2 | A direct-mapped cache with 64 blocks and 32-byte blocks uses how many bits for the index? | A) 5, B) 6, C) 7, D) 8 | **B** (log₂64=6) |
...
<p><strong>Solution:</strong> Sign=1 (negative). 5.75 = 101.11₂ = 1.0111×2². Exponent=2+127=129=10000001₂. Mantissa=0111000...0. Full: 1 10000001 01110000000000000000000 = 0xC0B80000.</p>
</details>
...
<p><strong>Solution:</strong> AMAT = 0.9×2 + 0.1×(2+50) = 1.8 + 5.2 = 7.0ns. Using formula: AMAT = Hit Time + Miss Rate × Miss Penalty = 2 + 0.1×50 = 7ns.</p>
</details>
...
<p><strong>Solution:</strong> Clock period = max(2,3,4,2,2) = 4ns. Non-pipelined = 2+3+4+2+2=13ns. Ideal speedup = 13/4 = 3.25. With pipeline overhead, actual ≈ 3.0-3.25.</p>
</details>
...
<p><strong>Solution:</strong> Total blocks = 64KB/32B = 2048. Sets = 2048/4 = 512. Index bits = log₂(512) = 9. Tag bits = 32-9-5(offset) = 18.</p>
</details>

<details>
<summary><strong>Exercise 5:</strong> Booth's Algorithm</summary>
<p>Multiply 6 (multiplicand) × -3 (multiplier) using Booth's algorithm with 4-bit representation.</p>
<p><strong>Solution:</strong> 6=0110, -3=1101. Booth: Initialize A=0, Q=1101, Q₋â‚Â�=0. Steps: (10): A=A-Q=0110... Wait let me retrace. 4-bit: 6=0110, -3=1101(2's). A=0000, Q=1101, Q-1=0, M=0110. Cycles: 0: Q₀Q₋â‚Â�=10 → A=A-M=1010. 1: shift ASR: 1101 0110 1. Q₀Q₋â‚Â�=01 → A=A+M=0011. 2: shift: 0001 1011 0. Q₀Q₋â‚Â�=10 → A=A-M=1011. 3: shift: 1101 1101 1. Q₀Q₋â‚Â�=11 → no op. Shift: 1110 1110 1. Result in A:Q = 11101110 = -18 (correct: 6×-3=-18).</p>
</details>

---

*End of Chapter 11 â€â€� Computer Organization & Architecture. Extended with summary, TypeScript implementations, quiz, and exercises.*

...
```
Total Error = BiasÃ‚Â² + Variance + Irreducible Error
...
**Answer:** **MSE** (Mean Squared Error) penalizes large errors more heavily. **RMSE** (Root MSE) is in the same units as the target. **MAE** (Mean Absolute Error) is less sensitive to outliers. **RÃ‚Â²** (coefficient of determination) measures variance explained â†’ 1.0 is perfect, 0.0 means no better than mean prediction.
...
```php
// Entropy = -ÃŽÂ£ p(i) * logÃ¢â€šâ€š(p(i))
// For a 50/50 split: -(0.5*logÃ¢â€šâ€š(0.5) + 0.5*logÃ¢â€šâ€š(0.5)) = 1.0
...
// After TF-IDF:  "the" weight Ã¢â€°Ë† 0.01 (appears in all docs)
//                "quantum" weight Ã¢â€°Ë† 0.85 (rare word, highly relevant)
```
...
**Answer:** Detection methods: Z-score (points beyond 3ÃÆ’), IQR (below Q1-1.5*IQR or above Q3+1.5*IQR), Isolation Forest, or domain-specific rules. Handling: remove (if measurement error), cap/winsorize (clamp to percentile), or transform (log scale reduces outlier impact).

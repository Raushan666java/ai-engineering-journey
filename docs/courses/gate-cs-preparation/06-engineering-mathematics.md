...
**GATE 2022 (CS) → Q4:** Evaluate `âˆ«_0^{Ï€/2} sin^3(x) cos^2(x) dx`.
...
**GATE 2024 (CS) → Q6:** If `f(x,y) = x^2y + xy^2`, find `âˆ‚Â²f/âˆ‚xâˆ‚y` at `(1,2)`.
...
**GATE 2022 (CS) → Q10:** Does `âˆ«_0^âˆž e^{-x} sin(x) dx` converge? If yes, find its value.
...
**GATE 2023 (CS) → Q5:** If `X ~ N(Î¼=10, ÏƒÂ²=4)`, find `P(8 < X < 12)`.
...
**GATE 2023 (CS) → Q12:** If `E[X] = 2` and `Var(X) = 4`, find an upper bound on `P(|X - 2| â‰¥ 4)` using Chebyshev's inequality.
...
**GATE 2021 (CS) → Q3:** Solve recurrence: `a_n = 3a_{n-1} - 2a_{n-2}` for `n â‰¥ 2`, with `a_0 = 1, a_1 = 3`.
...
**GATE 2022 (CS) → Q4:** In the group `(Z_7 - {0}, Ã—)` under multiplication modulo 7, find the order of element `3`.
...
**GATE 2023 (CS) → Q5:** Check if `(p → q) âˆ§ (q → r) → (p → r)` is a tautology.
...
**GATE 2024 (CS) → Q6:** If `|A| = 5, |B| = 3`, and `|A âˆª B| = 7`, find `|A âˆ© B|`.
...
**GATE 2020 (CS) → Q2:** Using Simpson's 1/3 rule with `h = Ï€/4`, approximate `âˆ«_0^{Ï€/2} sin(x) dx`.
...
**GATE 2024 (CS) → Q6:** Use trapezoidal rule with `n = 4` to approximate `âˆ«_0^1 e^x dx`. Compare with exact value.
...
**GATE 2011 (CS) → Q52:** Let `A` be a 3Ã—3 matrix with eigenvalues 1, -1, and 2. Find the trace of `A^3`.
...
- Eigenvalues: Î» = 0, 5
- For Î» = 0: `(A - 0I)v = [[1,2],[2,4]]v = 0` → `vâ‚ + 2vâ‚‚ = 0` → eigenvector `(-2, 1)`
- For Î» = 5: `(A - 5I)v = [[-4,2],[2,-1]]v = 0` → `-4vâ‚ + 2vâ‚‚ = 0` → eigenvector `(1, 2)`
- Two distinct eigenvalues → two LI eigenvectors
- **Answer: 2**

---

**GATE 2013 (CS) → Q54:** If `A = [[3, -2], [2, -1]]`, find `AÂ²`.
...
- Actually: after Râ‚‚ → Râ‚‚ - Râ‚ and Râ‚ƒ → Râ‚ƒ - Râ‚:
...
- Unique solution when `det(A) â‰  0` → `k â‰  3`
- **Answer: `k â‰  3`**

---

**GATE 2015 (CS) → Q56:** If `A` is a 3Ã—3 matrix with `det(A) = 2`, find `det(adj(A))`.
...
**GATE 2015 (CS) → Q57:** Let `P` be a 2Ã—2 matrix with eigenvalues 1 and 4. Find the trace of `Pâ»Â¹`.
...
- `det(Aáµ€A) = det(I)` → `det(Aáµ€)Â·det(A) = 1` → `det(A)Â² = 1`
- So `|A| = det(A) = Â±1`
- **Answer: Â±1**

---

**GATE 2017 (CS) → Q60:** How many solutions does `Ax = 0` have if `A` is `mÃ—n` with `rank(A) = r < n`?
...
- Given `det(2A) = 48` → `4 Â· det(A) = 48` → `det(A) = 12`
...
**Solution:**
- `f(2) = 0`. `lim_{x → 2â»} f(x) = lim_{x → 2âº} f(x) = 0` → continuous
- LHD: `lim_{h → 0â»} (f(2+h) - f(2))/h = lim_{h → 0â»} |h|/h = -1`
- RHD: `lim_{h → 0âº} (f(2+h) - f(2))/h = lim_{h → 0âº} |h|/h = 1`
- LHD â‰  RHD → not differentiable
- **Answer: (B)**

---

**GATE 2012 (CS) → Q65:** Evaluate `âˆ«_0^{Ï€/2} cos x / (1 + sin x) dx`.
...
**GATE 2013 (CS) → Q66:** Find the value of `âˆ«_{-a}^{a} xÂ³ dx`.
...
**GATE 2014 (CS) → Q67:** If `f(x) = xÂ² - 4x + 5`, find the absolute minimum on `[0, 3]`.
...
**GATE 2015 (CS) → Q68:** `lim_{x → 0} (sin x - x)/xÂ³ = ?`

**Solution:**
- Form `0/0`. Apply L'Hopital three times:
- L1: `lim (cos x - 1)/(3xÂ²)` → still `0/0`
...
**GATE 2016 (CS) → Q70:** Evaluate `âˆ«_0^âˆž e^{-x} dx`.

**Solution:**
- `âˆ«_0^âˆž e^{-x} dx = [-e^{-x}]_0^âˆž = lim_{b → âˆž} (-e^{-b}) - (-e^0)`
- `= 0 + 1 = 1`
- **Answer: 1**

---

**GATE 2017 (CS) → Q71:** The function `f(x) = xÂ³ - 3x + 1` on `[0, 1]` satisfies Rolle's theorem for what value of `c âˆˆ (0, 1)`?
...
**GATE 2012 (CS) → Q75:** If `X âˆ¼ Binomial(n, p)` with mean 4 and variance 2, find `n`.
...
**GATE 2013 (CS) → Q76:** Two events `A` and `B` have `P(A) = 0.3`, `P(B) = 0.4`, `P(Aâˆ©B) = 0.1`. Find `P(A|B)`.
...
**GATE 2016 (CS) → Q81:** `P(A) = 0.5`, `P(B) = 0.4`, `P(AâˆªB) = 0.7`. Are A and B independent?

**Solution:**
- `P(AâˆªB) = P(A) + P(B) - P(Aâˆ©B)`
- `0.7 = 0.5 + 0.4 - P(Aâˆ©B)` → `P(Aâˆ©B) = 0.2`
...
**GATE 2017 (CS) → Q82:** If `E[X] = 2` and `E[XÂ²] = 13`, find `Var(X)`.
...
**GATE 2018 (CS) → Q84:** For Bayes' theorem: `P(Aâ‚|B) = P(B|Aâ‚)P(Aâ‚) / Î£ P(B|Aâ±¼)P(Aâ±¼)`. If `P(Aâ‚) = 0.6`, `P(Aâ‚‚) = 0.4`, `P(B|Aâ‚) = 0.3`, `P(B|Aâ‚‚) = 0.2`, find `P(Aâ‚|B)`.
...
**GATE 2010 (CS) → Q85:** How many edges does a complete graph `Kâ‚…` have?
...
- `2^{h+1} - 1 â‰¥ 15` → `2^{h+1} â‰¥ 16` → `h+1 â‰¥ 4` → `h â‰¥ 3`
- Minimum height = 3
- **Answer: 3**

---

**GATE 2012 (CS) → Q87:** How many distinct Hamiltonian cycles does `Kâ‚„` have?
...
- Check transitivity: if `aRb` and `bRc` then `aRc`
  - `(3,2) âˆˆ R` and `(2,1) âˆˆ R` → `(3,1) âˆˆ R` âœ“
...
**GATE 2018 (CS) → Q94:** Negate: `âˆ€x âˆƒy (P(x, y) → Q(x))`.

**Solution:**
- Negation of `âˆ€x âˆƒy (P → Q)`:
- `Â¬âˆ€x âˆƒy (P → Q) â‰¡ âˆƒx Â¬âˆƒy (P → Q)`
- `â‰¡ âˆƒx âˆ€y Â¬(P → Q)`
- `â‰¡ âˆƒx âˆ€y (P âˆ§ Â¬Q)` (since `P → Q â‰¡ Â¬P âˆ¨ Q`, negation is `P âˆ§ Â¬Q`)
...
**Solution:**
- Newton-Raphson: `x_{n+1} = x_n - f(x_n)/f'(x_n)`
- If `f'(xâ‚€) = 0`, the denominator is zero → method fails (division by zero)
- Method diverges or fails to converge
- **Answer: Method fails (division by zero)**

---

**GATE 2013 (CS) → Q96:** Using bisection method on `f(x) = xÂ² - 2` in `[1, 2]`, after one iteration, the new interval is:
...
**GATE 2015 (CS) → Q98:** Using trapezoidal rule with `n = 4`, approximate `âˆ«â‚€Â¹ xÂ² dx`.
...
**GATE 2017 (CS) → Q99:** Gauss-Seidel iteration for `4xâ‚ + xâ‚‚ = 9`, `xâ‚ + 4xâ‚‚ = 6` starting from `(0,0)`. Find after 2 iterations.
...
| 13 | **Matrix multiplication order** | `(AB)áµ€ = Báµ€Aáµ€` → reverse order. Never write `(AB)áµ€ = Aáµ€Báµ€`. Also `(AB)â»Â¹ = Bâ»Â¹Aâ»Â¹`. |
...
| **Relationship** | `n→âˆž, p→0, np=Î»` → Poisson | `Î»→âˆž` → approx Normal | → |
...
| `lim_{x→0} (eË£ - 1)/x` | 1 |
| `lim_{x→0} (1 + x)^{1/x}` | `e` |
| `lim_{x→âˆž} (1 + 1/x)Ë£` | `e` |
| `lim_{x→0} (aË£ - 1)/x` | `ln a` |
| `lim_{x→0} (sinâ»Â¹ x)/x` | 1 |
| `lim_{x→âˆž} x^{1/x}` | 1 |
